...
description: >
  The Nextstep way of converting a plan into executable tasks. Use whenever
  you are asked to plan, scope, or break down work inside a Nextstep company.
  Industry-agnostic guidance on how to translate a plan into assigned issues
  with the right specialty, dependencies, and parallelization so Nextstep's
  executor can pick up the work — it does not prescribe a plan format. Pair
  with the `Nextstep` skill, which covers the mechanics of writing the plan
  document and reassigning the issue.
---

# Nextstep — Converting Plans to Tasks

A companion skill for turning a plan into executable Nextstep work. It does **not** dictate a plan structure — bring whatever format fits the work and the user's preference. It tells you _how_ to translate that plan into issues so that the rest of Nextstep works for you.

For the **mechanics** of recording a plan (issue document with key `plan`, comment links, approval gating, who to reassign back to), follow the _Planning_ section of the `Nextstep` skill. This skill covers planning method, not the API surface.
...
- **Use the dependency tree.** Nextstep's executor automatically starts any assigned task with no open blockers. Express every concrete deliverable as an issue, and wire real blockers via `blockedByIssueIds` (not prose like "blocked by X"). When `done`, dependents auto-wake.
...
- Not a replacement for the `Nextstep` skill's planning mechanics. Use both.
