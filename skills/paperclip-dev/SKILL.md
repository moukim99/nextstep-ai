...
required: false
description: >
  Develop and operate a local Nextstep instance — start and stop servers,
  ...
  Nextstep codebase itself or keep a running instance healthy.
---

# Nextstep Dev

This skill covers the day-to-day workflows for developing and operating a local Nextstep instance. It assumes you are working inside the Nextstep repo checkout with `origin` pointing to `git@github.com:nextstepai/nextstep.git`.
...
> **MANDATORY:** Before running any CLI command, building, testing, or managing worktrees, you MUST read `doc/DEVELOPING.md` in the Nextstep repo. It is the canonical reference for all `nextstepai` CLI commands, their options, build/test workflows, database operations, worktree management, and diagnostics. Do NOT guess at flags or options — read the doc first.
...
| Task | Command |
|------|---------|
| Start server (first time or normal) | `npx nextstepai run` |
...
| Back up database | `npx nextstepai db:backup` |
| Health check | `npx nextstepai doctor --repair` |
| Print env vars | `npx nextstepai env` |
| Trigger agent heartbeat | `npx nextstepai heartbeat run --agent-id <id>` |
| Install agent skills locally | `npx nextstepai agent local-cli <agent> --company-id <id>` |
...
Nextstep worktrees combine git worktrees with isolated Nextstep instances — each gets its own database, server port, and environment seeded from the primary instance.
...
- Starting a feature branch that needs its own Nextstep environment
- Running parallel agent work without cross-contaminating the primary instance
- Testing Nextstep changes in isolation before merging
...
| `worktree:list` | List worktrees and their Nextstep status |
...
npx nextstepai worktree:make my-feature --start-point origin/main

# 2. Move into the worktree (path printed by worktree:make) and source the environment
cd <worktree-path>
eval "$(npx nextstepai worktree env)"

# 3. Start the isolated Nextstep server
npx nextstepai run

# 4. Do your work

# 5. When done, merge history back if needed
npx nextstepai worktree:merge-history --from Nextstep-my-feature --to current --apply

# 6. Clean up
npx nextstepai worktree:cleanup my-feature
```

## Forks — Prefer Pushing to a User Fork

If the user has a personal fork of `nextstepai/nextstep` configured as a git remote, push your feature branches to **that fork** instead of creating branches on the main repo. This keeps the upstream branch list clean and matches the standard open-source contribution flow.

### Detect a fork remote

Before pushing or creating a PR, list remotes and check for one that points at a non-`nextstepai` GitHub fork:

```bash
git remote -v
```

Treat any remote whose URL points to `github.com:<user>/Nextstep` (or `github.com/<user>/Nextstep.git`) as the user's fork. Common names are `fork`, `<username>`, or `myfork`. The remote named `origin` or `upstream` that points at `nextstepai/nextstep` is the canonical upstream — do not push feature branches there if a fork exists.
...
```bash
gh pr create --repo nextstepai/nextstep --head <fork-owner>:<branch-name> ...
```
...
If `git remote -v` shows only `nextstepai/nextstep` remotes (no user fork), fall back to pushing branches to `origin` as before. Do NOT create a fork on the user's behalf — ask first.

### Keeping the fork up to date

The canonical remote that points at `nextstepai/nextstep` may be named `origin` **or** `upstream` depending on how the user set up the repo. Detect it the same way as in the "Detect a fork remote" step, then fetch and push from/with that remote so the sync works under either convention:

```bash
UPSTREAM_REMOTE=$(git remote -v | awk '/nextstepai\/Nextstep.*\(fetch\)/{print $1; exit}')
...
1. **CLI is the only interface to worktrees and databases.** All worktree and database operations MUST go through `npx nextstepai` / `pnpm nextstepai` commands. You MUST NOT:
   ...
   - Run `rm -rf` on any `.nextstep/`, `.Nextstep-worktrees/`, or `db/` directory
   - Directly manipulate embedded postgres data directories
   - Kill postgres processes by PID

2. **If a CLI command fails, stop and report.** Do NOT attempt workarounds. If `worktree:make`, `worktree reseed`, `worktree init`, `worktree:cleanup`, or any other `nextstepai` command fails:
   - Report the exact error message in your task comment
   - Set the task to `blocked`
   - Suggest running `npx nextstepai doctor --repair` or recreating the worktree from scratch
...
4. **Starting a dev server in a worktree requires setup first.** The correct sequence is:
   ...
   eval "$(npx nextstepai worktree env)"
   pnpm install && pnpm build
   npx nextstepai run          # or pnpm dev

   # If the worktree needs a fresh database:
   npx nextstepai worktree reseed --seed-mode full

   # If the worktree is broken beyond repair:
   npx nextstepai worktree:cleanup <name>
   npx nextstepai worktree:make <name> --seed-mode full
   ```
...
cd <worktree-path>
eval "$(npx nextstepai worktree env)"   # skip if using the primary instance
...
| Mistake | Fix |
|---------|-----|
| Server won't start | Run `npx nextstepai doctor --repair` to diagnose and auto-fix |
| Forgetting to source worktree env | Run `eval "$(npx nextstepai worktree env)"` after cd-ing into the worktree |
...
| Running agents against wrong instance | Verify `NEXTSTEP_API_URL` points to the correct port |
...
| Pushed feature branch to `nextstepai/nextstep` when a fork exists | Push to the user's fork remote instead — see "Forks" above |
