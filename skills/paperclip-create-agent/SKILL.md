...
description: >
  Create new agents in Nextstep with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# Nextstep Create Agent Skill
...
```sh
curl -sS "$NEXTSTEP_API_URL/api/agents/me" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY"
```

### 2. Discover adapter configuration for this Nextstep instance

```sh
curl -sS "$NEXTSTEP_API_URL/llms/agent-configuration.txt" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY"

# Then the specific adapter you plan to use, e.g. claude_local:
curl -sS "$NEXTSTEP_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY"
```
...
```sh
curl -sS "$NEXTSTEP_API_URL/api/companies/$NEXTSTEP_COMPANY_ID/agent-configurations" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY"
```
...
`skills/Nextstep-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/Nextstep-create-agent/references/baseline-role-guide.md`
...
```sh
curl -sS "$NEXTSTEP_API_URL/llms/agent-icons.txt" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY"
```
...
`skills/Nextstep-create-agent/references/draft-review-checklist.md`

### 8. Submit hire request

```sh
curl -sS -X POST "$NEXTSTEP_API_URL/api/companies/$NEXTSTEP_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY" \
...
- when the board approves, you will be woken with `NEXTSTEP_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$NEXTSTEP_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY"

curl -sS -X POST "$NEXTSTEP_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$NEXTSTEP_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$NEXTSTEP_API_URL/api/approvals/$NEXTSTEP_APPROVAL_ID" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY"

curl -sS "$NEXTSTEP_API_URL/api/approvals/$NEXTSTEP_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY"
```
...
- Template index and how to apply a template: `skills/Nextstep-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/Nextstep-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/Nextstep-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/Nextstep-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/Nextstep-create-agent/references/api-reference.md`
