---
name: Nextstep-create-plugin
description: >
  Create new Nextstep plugins with the current alpha SDK/runtime. Use when
...
# Create a Nextstep Plugin

Use this skill when the task is to create, scaffold, or document a Nextstep plugin.
...
node packages/plugins/create-Nextstep-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Nextstep repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.Nextstep-sdk/`:

```bash
pnpm --filter @nextstepai/create-Nextstep-plugin build
node packages/plugins/create-Nextstep-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Nextstep from an absolute local path during development
