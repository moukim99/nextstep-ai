...
```sh
curl -sS -H "Authorization: Bearer $NEXTSTEP_API_KEY" \
  "$NEXTSTEP_API_URL/api/issues/$NEXTSTEP_TASK_ID/heartbeat-context"
```
...
If `currentExecutionWorkspace` is `null`, the issue does not currently have a realized execution workspace. For child/follow-up work, create the child with `parentId` or use `inheritExecutionWorkspaceFromIssueId` so Nextstep preserves workspace continuity.

## Control Services

Prefer Nextstep-managed runtime service controls over manual `pnpm dev &` or ad-hoc background processes. These endpoints keep service state, URLs, logs, and ownership visible to other agents and the board.

```sh
# Start all configured services; waits for configured readiness checks.
curl -sS -X POST \
  -H "Authorization: Bearer $NEXTSTEP_API_KEY" \
  -H "X-Nextstep-Run-Id: $NEXTSTEP_RUN_ID" \
  -H "Content-Type: application/json" \
  "$NEXTSTEP_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/start" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Nextstep-Run-Id: $NEXTSTEP_RUN_ID" \
  -H "Content-Type: application/json" \
  "$NEXTSTEP_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/restart" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-Nextstep-Run-Id: $NEXTSTEP_RUN_ID" \
  -H "Content-Type: application/json" \
  "$NEXTSTEP_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/stop" \
  -d '{}'
```
...
When the Nextstep MCP tools are available, prefer these issue-scoped tools:

- `NextstepGetIssueWorkspaceRuntime` — reads `currentExecutionWorkspace` and service URLs for an issue.
- `NextstepControlIssueWorkspaceServices` — starts, stops, or restarts the current issue workspace services.
- `NextstepWaitForIssueWorkspaceService` — waits until a selected service is running and returns its URL when exposed.
