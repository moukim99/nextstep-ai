...
const PORT = Number(process.env.NEXTSTEP_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const NEXTSTEP_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-e2e-home-"));
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp NEXTSTEP_HOME.
  webServer: {
    command: `pnpm nextstepai onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active Nextstep home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      NEXTSTEP_HOME,
      NEXTSTEP_INSTANCE_ID: "playwright-e2e",
      NEXTSTEP_BIND: "loopback",
      NEXTSTEP_DEPLOYMENT_MODE: "local_trusted",
