...
const BASE = process.env.NEXTSTEP_E2E_BASE_URL ?? "http://127.0.0.1:3105";
const DATA_DIR = process.env.NEXTSTEP_E2E_DATA_DIR ?? process.env.NEXTSTEP_HOME;
const CONFIG_PATH = process.env.NEXTSTEP_E2E_CONFIG_PATH ?? path.resolve(process.cwd(), ".nextstep/config.json");
const BOOTSTRAP_SCRIPT_PATH = path.resolve(process.cwd(), "packages/db/scripts/create-auth-bootstrap-invite.ts");
const OWNER_PASSWORD = "Nextstep-owner-password";
const INVITED_PASSWORD = "Nextstep-invited-password";
...
const ownerUser: HumanUser = {
  name: "Owner User",
  email: `owner-${runId}@Nextstep.local`,
  password: OWNER_PASSWORD,
};
const invitedUser: HumanUser = {
  name: "Invited User",
  email: `invitee-${runId}@Nextstep.local`,
  password: INVITED_PASSWORD,
};

function createBootstrapInvite() {
  if (!DATA_DIR) {
    throw new Error("NEXTSTEP_E2E_DATA_DIR or NEXTSTEP_HOME is required for authenticated bootstrap tests");
  }
...
async function signUp(page: Page, user: HumanUser) {
  ...
  await expect(page.getByRole("heading", { name: "Sign in to Nextstep" })).toBeVisible();
  await page.getByRole("button", { name: "Create one" }).click();
  await expect(page.getByRole("heading", { name: "Create your Nextstep account" })).toBeVisible();
...
async function acceptBootstrapInvite(page: Page, inviteUrl: string) {
  ...
  await expect(page.getByRole("heading", { name: "Bootstrap your Nextstep instance" })).toBeVisible();
