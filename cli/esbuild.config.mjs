/**
 * esbuild configuration for building the nextstepai CLI for npm.
 *
 * Bundles all workspace packages (@nextstepai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @nextstepai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@nextstepai/")) {
      externals.add(name);
    }
  }
