...
export class NextstepApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-Nextstep-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Nextstep API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Nextstep server is not running, the configured URL is wrong, or the request is being blocked before it reaches Nextstep.",
    "",
    "Try:",
    "- Start Nextstep with `pnpm dev` or `pnpm nextstepai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Nextstep is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`NEXTSTEP_API_URL\`.`,
  );
  return lines.join("\n");
}
