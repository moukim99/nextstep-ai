...
function findContextFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".Nextstep", DEFAULT_CONTEXT_BASENAME);
...
export function resolveContextPath(overridePath?: string): string {
  ...
  if (process.env.NEXTSTEP_CONTEXT) return path.resolve(process.env.NEXTSTEP_CONTEXT);
