...
const DATA_DIR_OPTION_HELP =
  "Nextstep data directory root (isolates state from ~/.Nextstep)";

program
  .name("nextstepai")
  .description("Nextstep CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadNextstepEnvFile(options.config);
...
program
  ...
  .option("--run", "Start Nextstep immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Nextstep setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "Nextstep")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Nextstep")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Nextstep server API")
