...
import { NextstepConfigSchema, type NextstepConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".Nextstep", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveNextstepInstanceId());
}
...
export function readConfig(configPath?: string): NextstepConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = NextstepConfigSchema.safeParse(migrated);
  if (!parsed.success) {
