...
const JWT_SECRET_ENV_KEY = "NEXTSTEP_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Nextstep CLI commands",
...
export function resolveNextstepEnvFile(configPath?: string): string {
...
export function loadNextstepEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeNextstepEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readNextstepEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeNextstepEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeNextstepEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readNextstepEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeNextstepEnvEntries(next, filePath);
  return next;
}
