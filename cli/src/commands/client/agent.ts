...
function buildAgentEnvExports(input: {
  ...
  return [
    `export NEXTSTEP_API_URL='${escaped(input.apiBase)}'`,
    `export NEXTSTEP_COMPANY_ID='${escaped(input.companyId)}'`,
    `export NEXTSTEP_AGENT_ID='${escaped(input.agentId)}'`,
    `export NEXTSTEP_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Nextstep skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Nextstep skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveNextstepSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Nextstep skills directory. Expected ./skills in the repo checkout.",
              );
            }
