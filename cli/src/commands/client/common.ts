...
import { ApiRequestError, NextstepApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Nextstep config file")
    .option("-d, --data-dir <path>", "Nextstep data directory root (isolates state from ~/.Nextstep)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Nextstep API")
...
export function resolveCommandContext(
  ...
  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.NEXTSTEP_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.NEXTSTEP_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set NEXTSTEP_COMPANY_ID, or set context profile companyId via `nextstepai context set`.",
    );
  }

  const api = new NextstepApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.NEXTSTEP_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.NEXTSTEP_SERVER_PORT || "");
