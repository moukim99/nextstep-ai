...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "Nextstep");
      const connectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${embeddedHandle.port}/Nextstep`;
...
export async function disableAllRoutinesInConfig(
  ...
  const companyId =
    ...
    ?? nonEmpty(process.env.NEXTSTEP_COMPANY_ID)
    ?? null;
  if (!companyId) {
    throw new Error("Company ID is required. Pass --company-id or set NEXTSTEP_COMPANY_ID.");
  }
