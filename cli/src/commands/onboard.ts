...
type OnboardDefaults = Pick<NextstepConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or NEXTSTEP_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "NEXTSTEP_PUBLIC_URL",
  "DATABASE_URL",
  "NEXTSTEP_DB_BACKUP_ENABLED",
  "NEXTSTEP_DB_BACKUP_INTERVAL_MINUTES",
  "NEXTSTEP_DB_BACKUP_RETENTION_DAYS",
  "NEXTSTEP_DB_BACKUP_DIR",
  "NEXTSTEP_DEPLOYMENT_MODE",
  "NEXTSTEP_DEPLOYMENT_EXPOSURE",
  "NEXTSTEP_BIND",
  "NEXTSTEP_BIND_HOST",
  "NEXTSTEP_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "NEXTSTEP_ALLOWED_HOSTNAMES",
  "NEXTSTEP_AUTH_BASE_URL_MODE",
  "NEXTSTEP_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "NEXTSTEP_STORAGE_PROVIDER",
  "NEXTSTEP_STORAGE_LOCAL_DIR",
  "NEXTSTEP_STORAGE_S3_BUCKET",
  "NEXTSTEP_STORAGE_S3_REGION",
  "NEXTSTEP_STORAGE_S3_ENDPOINT",
  "NEXTSTEP_STORAGE_S3_PREFIX",
  "NEXTSTEP_STORAGE_S3_FORCE_PATH_STYLE",
  "NEXTSTEP_SECRETS_PROVIDER",
  "NEXTSTEP_SECRETS_STRICT_MODE",
  "NEXTSTEP_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<NextstepConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const instanceId = resolveNextstepInstanceId();
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.NEXTSTEP_PUBLIC_URL?.trim() ||
      process.env.NEXTSTEP_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.NEXTSTEP_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.NEXTSTEP_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.NEXTSTEP_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.NEXTSTEP_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.NEXTSTEP_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.NEXTSTEP_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.NEXTSTEP_ALLOWED_HOSTNAMES
    ? process.env.NEXTSTEP_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.NEXTSTEP_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.NEXTSTEP_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.NEXTSTEP_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.NEXTSTEP_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.NEXTSTEP_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.NEXTSTEP_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.NEXTSTEP_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.NEXTSTEP_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.NEXTSTEP_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.NEXTSTEP_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.NEXTSTEP_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.NEXTSTEP_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.NEXTSTEP_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.NEXTSTEP_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.NEXTSTEP_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "NEXTSTEP_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.NEXTSTEP_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "NEXTSTEP_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.NEXTSTEP_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "NEXTSTEP_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<NextstepConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" nextstepai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveNextstepInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: NextstepConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Nextstep install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("nextstepai configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("NEXTSTEP_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.NEXTSTEP_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("NEXTSTEP_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("NEXTSTEP_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: NEXTSTEP_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("nextstepai run")}`,
        `Reconfigure later: ${pc.cyan("nextstepai configure")}`,
        `Diagnose setup: ${pc.cyan("nextstepai doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.NEXTSTEP_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Nextstep setup is ready.");
    return;
  }
  ...
  let llm: NextstepConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@nextstepai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `nextstepai doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("nextstepai run")}`,
        `Then: ${pc.cyan("nextstepai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
