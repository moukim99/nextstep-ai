...
export const DEFAULT_WORKTREE_HOME = "~/.Nextstep-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".Nextstep");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: NextstepConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "Nextstep",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    NEXTSTEP_HOME: paths.homeDir,
    NEXTSTEP_INSTANCE_ID: paths.instanceId,
    NEXTSTEP_CONFIG: paths.configPath,
    NEXTSTEP_CONTEXT: paths.contextPath,
    NEXTSTEP_IN_WORKTREE: "true",
    ...(branding?.name ? { NEXTSTEP_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { NEXTSTEP_WORKTREE_COLOR: branding.color } : {}),
  };
}
