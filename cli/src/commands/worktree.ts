...
import { ensureAgentJwtSecret, loadNextstepEnvFile, mergeNextstepEnvEntries, readNextstepEnvEntries, resolveNextstepEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { NextstepConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printNextstepCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.NEXTSTEP_CONFIG;
...
const WORKTREE_NAME_PREFIX = "Nextstep-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.NEXTSTEP_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.NEXTSTEP_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromNextstepConfig(config: NextstepConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromNextstepConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.Nextstep${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".Nextstep", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".Nextstep", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.Nextstep"));
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readNextstepEnvEntries(resolveNextstepEnvFile(input.configPath));
  const homeDir = nonEmpty(envEntries.NEXTSTEP_HOME);
  const instanceId = nonEmpty(envEntries.NEXTSTEP_INSTANCE_ID);

  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local Nextstep instance. Expected NEXTSTEP_HOME and NEXTSTEP_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveExistingGitWorktree(selector: string, cwd: string): MergeSourceChoice | null {
  ...
  if (existsSync(directPath)) {
    return {
      ...
      hasNextstepConfig: existsSync(path.resolve(directPath, ".Nextstep", "config.json")),
...
async function ensureRepairTargetWorktree(input: {
  ...
  const currentConfigPath = path.resolve(currentRoot, ".Nextstep", "config.json");
  ...
  if (existing) {
    return {
      ...
      configPath: path.resolve(existing.worktree, ".Nextstep", "config.json"),
  ...
  return {
    rootPath: targetPath,
    configPath: path.resolve(targetPath, ".Nextstep", "config.json"),
    label: branchName,
    branchName,
    created: true,
  };
...
function resolveSourceConnectionString(config: NextstepConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://Nextstep:Nextstep@127.0.0.1:${port}/Nextstep`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: NextstepConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.NEXTSTEP_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.NEXTSTEP_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.NEXTSTEP_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.NEXTSTEP_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvFile = resolveNextstepEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readNextstepEnvEntries(sourceEnvFile);
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "Nextstep");
    }
    ...
    const adminConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "Nextstep");
    const targetConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${targetHandle.port}/Nextstep`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readNextstepEnvEntries(resolveNextstepEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.NEXTSTEP_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.NEXTSTEP_AGENT_JWT_SECRET);
  mergeNextstepEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { NEXTSTEP_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadNextstepEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Nextstep inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printNextstepCliBanner();
  p.intro(pc.bgCyan(pc.black(" nextstepai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printNextstepCliBanner();
  p.intro(pc.bgCyan(pc.black(" nextstepai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasNextstepConfig: existsSync(path.resolve(worktreePath, ".Nextstep", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printNextstepCliBanner();
  p.intro(pc.bgCyan(pc.black(" nextstepai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveNextstepEnvFile(configPath);
  const envEntries = readNextstepEnvEntries(envPath);
  const out = {
    NEXTSTEP_CONFIG: configPath,
    ...(envEntries.NEXTSTEP_HOME ? { NEXTSTEP_HOME: envEntries.NEXTSTEP_HOME } : {}),
    ...(envEntries.NEXTSTEP_INSTANCE_ID ? { NEXTSTEP_INSTANCE_ID: envEntries.NEXTSTEP_INSTANCE_ID } : {}),
    ...(envEntries.NEXTSTEP_CONTEXT ? { NEXTSTEP_CONTEXT: envEntries.NEXTSTEP_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasNextstepConfig)
      .map((choice) => path.resolve(choice.worktree, ".Nextstep", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readNextstepEnvEntries(resolveNextstepEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Nextstep once) before using worktree merge history.`,
      );
    }
...
function resolveRunningEmbeddedPostgresPid(config: NextstepConfig): number | null {
  if (config.database.mode !== "embedded-postgres") {
    return null;
  }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasNextstepConfig ? "Nextstep" : "no-Nextstep-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".Nextstep", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".Nextstep", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .nextstep/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasNextstepConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Nextstep worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasNextstepConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Nextstep worktrees were found. Run `nextstepai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Nextstep configs are the same. Choose different --from/--to worktrees.");
  }
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Nextstep configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Nextstep in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated Nextstep DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export async function worktreeRepairCommand(opts: WorktreeRepairOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(target.configPath)) {
    throw new Error("Source and target Nextstep configs are the same. Use --from-config/--from-instance to point repair at a different source.");
  }

  const targetConfig = existsSync(target.configPath) ? readConfig(target.configPath) : null;
  const targetEnvEntries = readNextstepEnvEntries(resolveNextstepEnvFile(target.configPath));
  const targetHasWorktreeEnv = Boolean(
    nonEmpty(targetEnvEntries.NEXTSTEP_HOME) && nonEmpty(targetEnvEntries.NEXTSTEP_INSTANCE_ID),
  );
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop Nextstep in ${target.rootPath} before repairing, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  if (runningTargetPid && opts.allowLiveTarget) {
    p.log.warning(`Proceeding even though the target embedded PostgreSQL appears to be running (pid ${runningTargetPid}).`);
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Nextstep instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Nextstep instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with Nextstep- if needed (created at ~/Nextstep-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: NEXTSTEP_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: NEXTSTEP_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source NEXTSTEP_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Nextstep instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Nextstep worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another Nextstep instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source NEXTSTEP_HOME used when deriving the source config")
  ...
  worktree
    .command("repair")
    .description("Create or repair a linked worktree-local Nextstep instance without touching the primary checkout")
    .option("--branch <name>", "Existing branch/worktree selector to repair, or a branch name to create under .nextstep/worktrees")
    .option("--home <path>", `Home root for worktree instances (env: NEXTSTEP_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source NEXTSTEP_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with Nextstep- if needed")
