...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@nextstepai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://Nextstep:Nextstep@127.0.0.1:${port}/Nextstep`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" Nextstep db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveNextstepInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "Nextstep";
