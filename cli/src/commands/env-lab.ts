...
import { resolveNextstepInstanceId, resolveNextstepInstanceRoot } from "../config/home.js";
...
export function resolveEnvLabSshStatePath(instanceId?: string): string {
  ...
  return path.resolve(
    resolveNextstepInstanceRoot(resolvedInstanceId),
    "env-lab",
    "ssh-fixture",
    "state.json",
  );
...
export async function envLabDoctorCommand(opts: { instance?: string; json?: boolean }) {
  ...
  p.log.message(`Cleanup: ${pc.dim("pnpm nextstepai env-lab down")}`);
}
...
export function registerEnvLabCommands(program: Command) {
  ...
  envLab
    ...
    .option("-i, --instance <id>", "Nextstep instance id (default: current/default)")
