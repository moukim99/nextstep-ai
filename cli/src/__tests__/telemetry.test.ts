...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.NEXTSTEP_HOME = path.join(root, "home");
    process.env.NEXTSTEP_INSTANCE_ID = "telemetry-test";
