...
function buildSourceConfig(): NextstepConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("Nextstep-pr-432")).toBe(
      path.resolve(os.homedir(), "Nextstep-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("Nextstep/pr-432")).toThrow(
  ...
  it("rewrites auth URLs only when they already include a port", () => {
    ...
    expect(rewriteLocalUrlPort("https://Nextstep.example", 3110)).toBe("https://Nextstep.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/Nextstep-feature",
      homeDir: "/tmp/Nextstep-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/Nextstep-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/Nextstep-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.NEXTSTEP_HOME).toBe(path.resolve("/tmp/Nextstep-worktrees"));
    expect(env.NEXTSTEP_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.NEXTSTEP_IN_WORKTREE).toBe("true");
    expect(env.NEXTSTEP_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.NEXTSTEP_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export NEXTSTEP_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  itEmbeddedPostgres("quarantines copied live execution state in seeded worktree databases", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-worktree-quarantine-");
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-secrets-"));
    const originalInlineMasterKey = process.env.NEXTSTEP_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.NEXTSTEP_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.NEXTSTEP_SECRETS_MASTER_KEY;
      delete process.env.NEXTSTEP_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.NEXTSTEP_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.NEXTSTEP_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.NEXTSTEP_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          NEXTSTEP_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.NEXTSTEP_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.NEXTSTEP_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".Nextstep-worktrees"),
      });

      const envPath = path.join(repoRoot, ".Nextstep", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("NEXTSTEP_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("NEXTSTEP_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/NEXTSTEP_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.NEXTSTEP_AGENT_JWT_SECRET;
      } else {
        process.env.NEXTSTEP_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-auth-seed-"));
      ...
      const worktreeHome = path.join(tempRoot, ".Nextstep-worktrees");
      const originalCwd = process.cwd();
      const sourceDb = await startEmbeddedPostgresTestDatabase("Nextstep-worktree-auth-source-");

      try {
        const sourceDbClient = createDb(sourceDb.connectionString);
        await sourceDbClient.insert(authUsers).values({
          id: "user-existing",
          email: "existing@Nextstep.ing",
          name: "Existing User",
          emailVerified: true,
          createdAt: new Date(),
          updatedAt: new Date(),
        ...
        const targetConfig = JSON.parse(
          fs.readFileSync(path.join(worktreeRoot, ".Nextstep", "config.json"), "utf8"),
        ) as NextstepConfig;
        ...
        try {
          const targetDb = createDb(
            `postgres://Nextstep:Nextstep@127.0.0.1:${targetConfig.database.embeddedPostgresPort}/Nextstep`,
          );
          const seededUsers = await targetDb.select().from(authUsers);
          expect(seededUsers.some((row) => row.email === "existing@Nextstep.ing")).toBe(true);
        } finally {
          await targetPg.stop();
        }
      } finally {
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".Nextstep-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".Nextstep", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Nextstep config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".Nextstep", "config.json");
    const originalCwd = process.cwd();
    const originalNextstepConfig = process.env.NEXTSTEP_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalNextstepConfig === undefined) {
        delete process.env.NEXTSTEP_CONFIG;
      } else {
        process.env.NEXTSTEP_CONFIG = originalNextstepConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".Nextstep", "config.json");
    const envPath = path.join(worktreeRoot, ".Nextstep", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "NEXTSTEP_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".Nextstep", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".Nextstep", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Nextstep instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".Nextstep-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalNextstepConfig = process.env.NEXTSTEP_CONFIG;
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `NEXTSTEP_HOME=${homeDir}`,
          `NEXTSTEP_INSTANCE_ID=${currentInstanceId}`,
          "NEXTSTEP_WORKTREE_NAME=existing-name",
          "NEXTSTEP_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`NEXTSTEP_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("NEXTSTEP_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("NEXTSTEP_WORKTREE_COLOR=\"#112233\"");
    } finally {
      process.chdir(originalCwd);
      if (originalNextstepConfig === undefined) {
        delete process.env.NEXTSTEP_CONFIG;
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".Nextstep-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `NEXTSTEP_HOME=${homeDir}\nNEXTSTEP_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`NEXTSTEP_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/Nextstep",
        targetRepoRoot: "/Users/example/Nextstep-pr-432",
        workspaceCwd: "/Users/example/Nextstep",
      }),
    ).toBe("/Users/example/Nextstep-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/Nextstep",
        targetRepoRoot: "/Users/example/Nextstep-pr-432",
        workspaceCwd: "/Users/example/Nextstep/packages/db",
      }),
    ).toBe("/Users/example/Nextstep-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "Nextstep-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("Nextstep-make-test", {
        seed: false,
        home: path.join(tempRoot, ".Nextstep-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".Nextstep", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".Nextstep", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-repair-primary-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".Nextstep", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".Nextstep", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when Nextstep metadata is missing", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-repair-current-"));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".Nextstep", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".Nextstep-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-worktree-repair-branch-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".Nextstep-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".Nextstep", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".Nextstep", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".Nextstep", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-worktree-routines-");
    const db = createDb(tempDb.connectionString);
    const companyId = randomUUID();
