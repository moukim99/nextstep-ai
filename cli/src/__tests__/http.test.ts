...
import { ApiConnectionError, ApiRequestError, NextstepApiClient } from "../client/http.js";

describe("NextstepApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new NextstepApiClient({
    ...
    expect(headers["x-Nextstep-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new NextstepApiClient({ apiBase: "http://localhost:3100" });
