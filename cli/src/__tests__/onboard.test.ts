...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-onboard-"));
  const runtimeRoot = path.join(root, "runtime");
  const configPath = path.join(root, ".Nextstep", "config.json");
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-onboard-fresh-"));
  return path.join(root, ".Nextstep", "config.json");
}
...
describe("onboard", () => {
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.NEXTSTEP_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as NextstepConfig;
  ...
  it("supports authenticated/private quickstart bind presets", async () => {
    const configPath = createFreshConfigPath();
    process.env.NEXTSTEP_TAILNET_BIND_HOST = "100.64.0.8";
  ...
  it("keeps tailnet quickstart on loopback until tailscale is available", async () => {
    const configPath = createFreshConfigPath();
    delete process.env.NEXTSTEP_TAILNET_BIND_HOST;
  ...
  it("ignores deployment env overrides during --yes quickstart", async () => {
    const configPath = createFreshConfigPath();
    process.env.NEXTSTEP_DEPLOYMENT_MODE = "authenticated";
