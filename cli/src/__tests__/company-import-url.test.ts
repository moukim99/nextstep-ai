...
describe("isGithubShorthand", () => {
  it("matches owner/repo/path shorthands", () => {
    expect(isGithubShorthand("nextstepai/companies/gstack")).toBe(true);
    expect(isGithubShorthand("nextstepai/companies")).toBe(true);
  });
...
describe("normalizeGithubImportSource", () => {
  it("normalizes shorthand imports to canonical GitHub sources", () => {
    expect(normalizeGithubImportSource("nextstepai/companies/gstack")).toBe(
      "https://github.com/nextstepai/companies?ref=main&path=gstack",
    );
  });

  it("applies --ref to shorthand imports", () => {
    expect(normalizeGithubImportSource("nextstepai/companies/gstack", "feature/demo")).toBe(
      "https://github.com/nextstepai/companies?ref=feature%2Fdemo&path=gstack",
    );
  });
  ...
  it("applies --ref to existing GitHub tree URLs without losing the package path", () => {
    expect(
      normalizeGithubImportSource(
        "https://github.com/nextstepai/companies/tree/main/gstack",
        "release/2026-03-23",
      ),
    ).toBe(
      "https://github.com/nextstepai/companies?ref=release%2F2026-03-23&path=gstack",
    );
  });
});
