...
function createBaseNextstepEnv(options: TestNextstepEnv) {
  ...
  env.NEXTSTEP_CONFIG = options.configPath;
  env.NEXTSTEP_HOME = options.NextstepHome;
  env.NEXTSTEP_INSTANCE_ID = options.instanceId;
  env.NEXTSTEP_CONTEXT = path.join(options.NextstepHome, "context.json");
  env.NEXTSTEP_AUTH_STORE = path.join(options.NextstepHome, "auth.json");
...
function createServerEnv(
  ...
  options: Omit<TestNextstepEnv, "configPath">,
) {
  const env = createBaseNextstepEnv({
    configPath,
    ...options,
  });
  ...
  env.SERVE_UI = "false";
  env.NEXTSTEP_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.NEXTSTEP_MIGRATION_AUTO_APPLY = "true";
  env.NEXTSTEP_UI_DEV_MIDDLEWARE = "false";

  return env;
}

function createCliEnv(options: TestNextstepEnv) {
  const env = createBaseNextstepEnv(options);
  delete env.DATABASE_URL;
  delete env.PORT;
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.NEXTSTEP_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.NEXTSTEP_MIGRATION_AUTO_APPLY;
  delete env.NEXTSTEP_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(
  args: string[],
  opts: TestNextstepEnv & { apiBase?: string; includeConfigArg?: boolean },
) {
  const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
  const cliArgs = ["--silent", "nextstepai", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `nextstepai run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("nextstepai company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  let NextstepHome = "";
  let cliShellHome = "";
  let NextstepInstanceId = "";
  let serverProcess: ServerProcess | null = null;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "Nextstep-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");
    NextstepHome = path.join(tempRoot, "Nextstep-home");
    cliShellHome = path.join(tempRoot, "shell-home");
    NextstepInstanceId = "company-cli-e2e";
    mkdirSync(NextstepHome, { recursive: true });
    mkdirSync(cliShellHome, { recursive: true });

    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-company-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["nextstepai", "run", "--config", configPath],
      {
        cwd: repoRoot,
        env: createServerEnv(configPath, port, tempDb.connectionString, {
          NextstepHome,
          instanceId: NextstepInstanceId,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    const expectedContextPath = path.join(NextstepHome, "context.json");
    const leakedContextPath = path.join(cliShellHome, ".Nextstep", "context.json");
    ...
    expect(readFileSync(path.join(exportDir, ".Nextstep.yaml"), "utf8")).toContain('schema: "Nextstep/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "Nextstep-demo"));
