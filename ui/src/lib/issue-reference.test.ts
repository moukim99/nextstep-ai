...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal Nextstep issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/nextstepai/nextstep/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/nextstepai/nextstep/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute Nextstep issue URLs so origin, port, and hash are not lost", () => {
