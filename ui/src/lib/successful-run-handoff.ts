import type { ActivityEvent, Issue, SuccessfulRunHandoffState } from "@nextstepai/shared";
...
export function isSuccessfulRunHandoffEscalationComment(text: string) {
  ...
  return trimmed === SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY
    || /^Nextstep exhausted the bounded successful-run handoff correction\b/i.test(trimmed);
}
