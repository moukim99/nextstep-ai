...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("Nextstep-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("Nextstep-demo/COMPANY.md");
  ...
  it("reads a Nextstep zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
        ".Nextstep.yaml": "schema: Nextstep/v1\n",
      },
      "Nextstep-demo",
    );
  ...
  it("reads standard DEFLATE zip archives created outside Nextstep", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("Nextstep-demo");
