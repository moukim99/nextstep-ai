...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("Nextstep-worktree-enabled") !== "true") return null;

  const name = readMetaContent("Nextstep-worktree-name");
  const color = normalizeHexColor(readMetaContent("Nextstep-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("Nextstep-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
