...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Nextstep", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Nextstep",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Nextstep");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Nextstep");
    expect(buildDocumentTitle([], null)).toBe("Nextstep");
  });
});
