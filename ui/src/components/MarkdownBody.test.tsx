...
describe("MarkdownBody", () => {
  ...
  it("renders user, agent, project, and skill mentions as chips", () => {
    const html = renderToStaticMarkup(
      <QueryClientProvider client={new QueryClient()}>
        <ThemeProvider>
          <MarkdownBody>
            {`[@Taylor](${buildUserMentionHref("user-123")}) [@CodexCoder](${buildAgentMentionHref("agent-123", "code")}) [@Nextstep App](${buildProjectMentionHref("project-456", "#336699")}) [/release-changelog](${buildSkillMentionHref("skill-789", "release-changelog")})`}
          </MarkdownBody>
        </ThemeProvider>
      </QueryClientProvider>,
    );
    ...
    expect(html).toContain("--Nextstep-mention-icon-mask");
    expect(html).toContain('href="/projects/project-456"');
    expect(html).toContain('data-mention-kind="project"');
    expect(html).toContain("--Nextstep-mention-project-color:#336699");
  ...
  it("linkifies bare issue identifiers in markdown text", () => {
    ...
    expect(html).toContain("Nextstep-markdown-issue-ref");
    expect(html).not.toContain("Nextstep-mention-chip--issue");
  });
  ...
  it("preserves absolute issue URLs as external links", () => {
    ...
    expect(html).not.toContain("Nextstep-markdown-issue-ref");
  });
  ...
  it("leaves wiki links as text unless explicitly enabled", () => {
    const html = renderMarkdown("See [[wiki/entities/Nextstep]].");

    expect(html).toContain("[[wiki/entities/Nextstep]]");
    expect(html).not.toContain('href="/wiki/page/wiki/entities/Nextstep.md"');
  });

  it("renders wiki links with a custom resolver when enabled", () => {
    const html = renderMarkdown(
      "See [[wiki/entities/Nextstep|Nextstep]] and [[wiki/entities/dotta-b]].",
      [],
      {
    ...
    expect(html).toContain('href="/wiki/page/wiki/entities/Nextstep.md"');
    expect(html).toContain('data-Nextstep-wiki-link="true"');
    expect(html).toContain('data-Nextstep-wiki-target="wiki/entities/Nextstep"');
    expect(html).toContain(">Nextstep</a>");
    expect(html).toContain('href="/wiki/page/wiki/entities/dotta-b.md"');
    expect(html).toContain(">wiki/entities/dotta-b</a>");
    expect(html).not.toContain("[[wiki/entities/Nextstep");
  });

  it("keeps wiki links as text when the custom resolver rejects them", () => {
    const html = renderMarkdown(
      "See [[wiki/entities/Nextstep]].",
      [],
      {
        enableWikiLinks: true,
    ...
    expect(html).toContain("[[wiki/entities/Nextstep]]");
    expect(html).not.toContain('data-Nextstep-wiki-link="true"');
    expect(html).not.toContain('href="/wiki/page/wiki/entities/Nextstep"');
  });

  it("does not render wiki links inside code spans or code blocks", () => {
    const html = renderMarkdown(
      "Inline `[[wiki/entities/Nextstep]]`.\n\n```md\n[[wiki/entities/dotta-b]]\n```",
      [],
      {
        enableWikiLinks: true,
        wikiLinkRoot: "/wiki/page",
  ...
  it("applies wrap-friendly styles to long inline content", () => {
    ...
    expect(html).toContain('class="Nextstep-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden');
  ...
  it("renders markdown tables in a horizontally scrollable region", () => {
    ...
    expect(html).toContain('class="Nextstep-markdown-table-scroll"');
  ...
  it("opens GitHub links in a new tab", () => {
    const html = renderMarkdown("[pr](https://github.com/nextstepai/nextstep/pull/4099)");
  ...
  it("prefixes GitHub markdown links with the GitHub icon glued to the first character", () => {
    const html = renderMarkdown("[https://github.com/nextstepai/nextstep/pull/4099](https://github.com/nextstepai/nextstep/pull/4099)");

    expect(html).toContain('<a href="https://github.com/nextstepai/nextstep/pull/4099"');
    ...
    expect(html).toContain("ttps://github.com/nextstepai/nextstep/pull/4099");
    expect(html).not.toContain("lucide-external-link");
  });

  it("prefixes GitHub autolinks with the GitHub icon", () => {
    const html = renderMarkdown("See https://github.com/nextstepai/nextstep/issues/1778");

    expect(html).toContain('<a href="https://github.com/nextstepai/nextstep/issues/1778"');
    expect(html).toContain('class="lucide lucide-github mr-1 inline h-3.5 w-3.5 align-[-0.125em]"');
  });
  ...
  it("renders a copy button alongside fenced code blocks", () => {
    ...
    expect(html).toContain("Nextstep-markdown-codeblock");
    expect(html).toContain("Nextstep-markdown-codeblock-copy");
  ...
  it("does not render a copy button on inline code", () => {
    ...
    expect(html).not.toContain("Nextstep-markdown-codeblock-copy");
  });
