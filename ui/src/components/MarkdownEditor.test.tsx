...
import { buildProjectMentionHref, buildSkillMentionHref } from "@nextstepai/shared";
import {
...
describe("MarkdownEditor", () => {
  ...
  it("keeps mention queries active across spaces", () => {
    expect(findMentionMatch("Ping @Nextstep App", "Ping @Nextstep App".length)).toEqual({
      trigger: "mention",
      marker: "@",
      query: "Nextstep App",
      atPos: 5,
      endPos: "Ping @Nextstep App".length,
    });
  });
  ...
  async function openMentionMenuFor(
    ...
    const option = Array.from(document.body.querySelectorAll('button[type="button"]'))
      .find((node) => node.textContent?.includes("Nextstep App")) as HTMLButtonElement | undefined;
  ...
  it("accepts mention selection from a touch tap", async () => {
    ...
    expect(handleChange).toHaveBeenCalledWith(
      `[@Nextstep App](${buildProjectMentionHref("project-123", "#336699")}) `,
    );
