...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className="Nextstep-markdown-issue-ref"
      title={title}
...
function createWikiLinkNode(href: string, wikiLink: ParsedWikiLink): MarkdownAstNode {
  return {
    ...
    data: {
      hProperties: {
        "data-Nextstep-wiki-link": "true",
        "data-Nextstep-wiki-target": wikiLink.target,
      },
    },
...
function CodeBlock({
  ...
  return (
    <div className="Nextstep-markdown-codeblock">
      <pre
        {...preProps}
        ref={preRef}
      ...
      <button
        ...
        title={label}
        className="Nextstep-markdown-codeblock-copy"
      ...
      >
        ...
        <span className="Nextstep-markdown-codeblock-copy-label">{label}</span>
      </button>
    </div>
  );
}
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`Nextstep-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="Nextstep-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("Nextstep-mermaid-status", error && "Nextstep-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="Nextstep-mermaid-source">
...
export function MarkdownBody({
  ...
  const components: Components = {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div className="Nextstep-markdown-table-scroll" role="region" aria-label="Scrollable table" tabIndex={0}>
    ...
    a: ({ node: _node, href, style: linkStyle, children: linkChildren, ...anchorProps }) => {
      ...
      const isWikiLink = dataProps["data-Nextstep-wiki-link"] === "true";
      ...
      if (parsed) {
        ...
        return (
          <a
            href={targetHref}
            className={cn(
              "Nextstep-mention-chip",
              `Nextstep-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "Nextstep-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "Nextstep-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
