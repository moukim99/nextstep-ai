...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Nextstep API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "NextstepApiUrl",
                  String(config.NextstepApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "NextstepApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://Nextstep.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/Nextstep-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "Nextstep"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Nextstep persists a device key during onboarding so pairing approvals
