...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@nextstepai/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@nextstepai/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@nextstepai/adapter-utils").TranscriptEntry[];
