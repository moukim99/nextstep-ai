 ...
 *
 * Plugin bundles are built with `external: ["@nextstepai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__NextstepPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__NextstepPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__NextstepPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__NextstepPluginBridge__?.sdkUi ?? {};
        function missing(name) {
          return function MissingNextstepSdkUiComponent() {
            throw new Error('Nextstep plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@nextstepai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@nextstepai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@nextstepai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@nextstepai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@nextstepai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__NextstepPluginBridge__) {
 ...
 * self-register via `window.NextstepPlugins.registerReactComponent()`.
 ...
 * of `@nextstepai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
