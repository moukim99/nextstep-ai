...
/**
 * Filter .Nextstep.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterNextstepYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://Nextstep.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm nextstepai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Nextstep](https://Nextstep.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Nextstep](https://Nextstep.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .Nextstep.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .Nextstep.yaml
    const yamlPath = exportData.NextstepExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterNextstepYaml(exportData.files[yamlPath], checkedFiles);
    }
