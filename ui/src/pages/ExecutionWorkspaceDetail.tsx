...
import type { ExecutionWorkspace, Issue, Project, ProjectWorkspace, RoutineListItem } from "@nextstepai/shared";
...
function LegacyWorkspaceTabRedirect({ workspaceId }: { workspaceId: string }) {
  useEffect(() => {
    try {
      localStorage.removeItem(`Nextstep:execution-workspace-tab:${workspaceId}`);
    } catch {}
  }, [workspaceId]);
...
function ExecutionWorkspaceIssuesList({
  ...
  return (
    <IssuesList
      ...
      viewStateKey="Nextstep:execution-workspace-issues-view"
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        ...
        body: "Nextstep created a run using this execution workspace.",
        tone: "success",
      });
    },
    onSettled: () => {
            ...
            <Card className="rounded-none">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <Field label="Provision command" hint="Runs when Nextstep prepares this execution workspace">
                    <Textarea
