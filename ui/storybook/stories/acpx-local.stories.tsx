...
import type { AdapterConfigSchema, CreateConfigValues } from "@nextstepai/adapter-utils";
import { parseAcpxStdoutLine } from "@nextstepai/adapter-acpx-local/ui";
import type {
  Agent,
  AgentSkillSnapshot,
...
type SchemaWindow = typeof window & {
  __NextstepStorybookAdapterSchemas?: Record<string, unknown>;
};
...
const acpxLocalConfigSchema: AdapterConfigSchema = {
  fields: [
    ...
    },
    {
      ...
      hint: "Fallback if the ACP agent asks for input outside an interactive session. Nextstep still auto-approves permissions by default.",
    },
    {
      key: "cwd",
      label: "Working directory",
      type: "text",
      hint: "Absolute fallback directory. Nextstep execution workspaces can override this at runtime.",
    },
    {
      key: "stateDir",
      label: "State directory",
      type: "text",
      hint: "Optional ACPX session state directory. Defaults to Nextstep-managed company/agent scoped storage.",
    },
    {
      key: "fastMode",
...
function installAcpxSchemaMock(): void {
  ...
  win.__NextstepStorybookAdapterSchemas = {
    ...(win.__NextstepStorybookAdapterSchemas ?? {}),
...
const acpxSkillsCompanyLibrary: CompanySkillListItem[] = [
  {
    ...
    name: "Nextstep",
    description:
      "Coordination skill: heartbeats, checkout, comments, and routine API patterns for Nextstep agents.",
    sourceType: "local_path",
    sourceLocator: "skills/Nextstep",
    sourceRef: null,
    ...
    editable: false,
    editableReason: "Required by Nextstep",
    sourceLabel: "Nextstep",
    sourceBadge: "Nextstep",
    sourcePath: "skills/Nextstep",
  },
  {
    ...
    name: "Design guide",
    description:
      "Nextstep UI design system reference: tokens, typography, status colors, and reusable component patterns.",
    sourceType: "local_path",
...
function buildAcpxClaudeSnapshot(): AgentSkillSnapshot {
  return {
    ...
    supported: true,
    mode: "ephemeral",
    desiredSkills: ["Nextstep", "design-guide"],
    warnings: [],
    entries: [
      {
        key: "Nextstep",
        ...
        managed: true,
        required: true,
        requiredReason: "Nextstep coordination skill is mandatory for control-plane agents.",
        state: "configured",
  ...
  return {
    ...
    entries: [
      ...
      },
      {
        ...
        targetPath: null,
        detail:
          "Desired state is stored in Nextstep only; custom ACP commands need an explicit skill integration contract before runtime sync is available.",
      },
      {
        key: "mobile-app-qa",
        runtimeName: "mobile-app-qa",
...
function AcpxClaudeSkillsStory() {
  ...
  return (
    <StoryFrame
      ...
      subtitle="Runtime-synced state. Selected skills are mounted into the next ACPX Claude session via the Nextstep skills directory."
    >
...
function AcpxCustomSkillsStory() {
  ...
  return (
    <StoryFrame
      ...
      subtitle="Unsupported runtime sync. Desired skills are tracked in Nextstep only until a custom ACP command declares a skill integration contract."
    >
