...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "Nextstep");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://Nextstep:Nextstep@127.0.0.1:${preferredPort}/Nextstep`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "Nextstep");
    return {
      connectionString: `postgres://Nextstep:Nextstep@127.0.0.1:${port}/Nextstep`,
  ...
  const adminConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "Nextstep");

  return {
    connectionString: `postgres://Nextstep:Nextstep@127.0.0.1:${selectedPort}/Nextstep`,
