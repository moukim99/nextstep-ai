...
const DEFAULT_NEXTSTEP_EMBEDDED_POSTGRES_PORT = 54329;

function getReservedTestPorts(): Set<number> {
  const configuredPorts = [
    DEFAULT_NEXTSTEP_EMBEDDED_POSTGRES_PORT,
    Number.parseInt(process.env.NEXTSTEP_EMBEDDED_POSTGRES_PORT ?? "", 10),
    ...String(process.env.NEXTSTEP_TEST_POSTGRES_RESERVED_PORTS ?? "")
      .split(",")
...
async function getAvailablePort(): Promise<number> {
  ...
  throw new Error(
    `Failed to allocate embedded Postgres test port outside reserved Nextstep ports: ${[
      ...reservedPorts,
    ].join(", ")}`,
  );
}
...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  const { dataDir, instance } = await createEmbeddedPostgresTestInstance(
    "Nextstep-embedded-postgres-probe-",
  );
...
export async function startEmbeddedPostgresTestDatabase(
  ...
  try {
    ...
    const connectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${port}/Nextstep`;
