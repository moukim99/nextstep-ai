...
describe("Nextstep MCP tools", () => {
  ...
  it("adds auth headers and run id to mutating requests", async () => {
    ...
    const tool = getTool("NextstepUpdateIssue");
    await tool.execute({
      issueId: "PAP-1135",
      status: "done",
    });
    ...
    expect((init.headers as Record<string, string>)["X-Nextstep-Run-Id"]).toBe(
  ...
  it("uses default company id for company-scoped list tools", async () => {
    ...
    const tool = getTool("NextstepListIssues");
  ...
  it("uses default agent id for checkout requests", async () => {
    ...
    const tool = getTool("NextstepCheckoutIssue");
    await tool.execute({
      issueId: "PAP-1135",
    });
  ...
  it("allows create issue requests to omit status so the API applies assignee defaults", async () => {
    ...
    const tool = getTool("NextstepCreateIssue");
    await tool.execute({
  ...
  it("defaults issue document format to markdown", async () => {
    ...
    const tool = getTool("NextstepUpsertIssueDocument");
    await tool.execute({
      issueId: "PAP-1135",
      key: "plan",
      body: "# Updated",
  ...
  it("controls issue workspace services through the current execution workspace", async () => {
    ...
    const tool = getTool("NextstepControlIssueWorkspaceServices");
    await tool.execute({
      issueId: "PAP-1135",
      action: "restart",
  ...
  it("waits for an issue workspace runtime service URL", async () => {
    ...
    const tool = getTool("NextstepWaitForIssueWorkspaceService");
    const response = await tool.execute({
      issueId: "PAP-1135",
      serviceName: "web",
      timeoutSeconds: 1,
  ...
  it("creates suggest_tasks interactions with the expected issue-scoped payload", async () => {
    ...
    const tool = getTool("NextstepSuggestTasks");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates request_confirmation interactions with plan target payloads", async () => {
    ...
    const tool = getTool("NextstepRequestConfirmation");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates approvals with the expected company-scoped payload", async () => {
    ...
    const tool = getTool("NextstepCreateApproval");
    await tool.execute({
      type: "hire_agent",
  ...
  it("rejects invalid generic request paths", async () => {
    ...
    const tool = getTool("NextstepApiRequest");
    const response = await tool.execute({
      method: "GET",
      path: "issues",
    });
