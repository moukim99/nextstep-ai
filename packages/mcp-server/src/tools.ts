...
async function getIssueWorkspaceRuntime(client: NextstepApiClient, issueId: string) {
...
export function createToolDefinitions(client: NextstepApiClient): ToolDefinition[] {
  return [
    makeTool(
      "NextstepMe",
      "Get the current authenticated Nextstep actor details",
      z.object({}),
    ...
    makeTool(
      "NextstepApiRequest",
      "Make a JSON request to an existing Nextstep /api endpoint for unsupported operations",
      apiRequestSchema,
