...
describe("issue references", () => {
  ...
  it("parses relative and absolute issue hrefs", () => {
    ...
    expect(parseIssueReferenceHref("https://Nextstep.ing/PAP/issues/pap-789#comment-1")).toEqual({
      identifier: "PAP-789",
    });
    expect(parseIssueReferenceHref("https://Nextstep.ing/projects/PAP-789")).toBeNull();
  });
