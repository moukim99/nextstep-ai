...
const NEXTSTEP_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];
const MATERIALIZED_SKILL_SENTINEL = ".Nextstep-materialized-skill.json";
const MATERIALIZED_SKILL_LOCK_OWNER = "owner.json";
const MATERIALIZED_SKILL_LOCK_STALE_MS = 30_000;

export const DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE = [
  "You are agent {{agent.id}} ({{agent.name}}). Continue your Nextstep work.",
  "",
  "Execution contract:",
...
export interface NextstepSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
export interface MaterializedNextstepSkillCopyResult {
  copiedFiles: number;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: NextstepSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "NEXTSTEP_required",
      originLabel: "Required by Nextstep",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Nextstep",
    readOnly: false,
  };
}
...
type NextstepWakeExecutionPrincipal = {
  type: "agent" | "user" | null;
  agentId: string | null;
  userId: string | null;
};

type NextstepWakeExecutionStage = {
  wakeRole: "reviewer" | "approver" | "executor" | null;
  stageId: string | null;
  stageType: string | null;
  currentParticipant: NextstepWakeExecutionPrincipal | null;
  returnAssignee: NextstepWakeExecutionPrincipal | null;
  reviewRequest: {
    instructions: string;
  } | null;
...
type NextstepWakeContinuationSummary = {
  key: string | null;
  title: string | null;
  body: string;
  bodyTruncated: boolean;
...
type NextstepWakeLivenessContinuation = {
  attempt: number | null;
...
type NextstepWakeChildIssueSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type NextstepWakeBlockerSummary = {
  id: string | null;
  identifier: string | null;
  title: string | null;
  status: string | null;
...
type NextstepWakeTreeHoldSummary = {
  holdId: string | null;
...
type NextstepWakePayload = {
  reason: string | null;
  issue: NextstepWakeIssue | null;
  checkedOutByHarness: boolean;
  dependencyBlockedInteraction: boolean;
  treeHoldInteraction: boolean;
  activeTreeHold: NextstepWakeTreeHoldSummary | null;
  unresolvedBlockerIssueIds: string[];
  unresolvedBlockerSummaries: NextstepWakeBlockerSummary[];
  executionStage: NextstepWakeExecutionStage | null;
  continuationSummary: NextstepWakeContinuationSummary | null;
  livenessContinuation: NextstepWakeLivenessContinuation | null;
  interactionKind: string | null;
  interactionStatus: string | null;
  childIssueSummaries: NextstepWakeChildIssueSummary[];
  childIssueSummaryTruncated: boolean;
  commentIds: string[];
  latestCommentId: string | null;
  comments: NextstepWakeComment[];
...
function normalizeNextstepWakeIssue(value: unknown): NextstepWakeIssue | null {
...
function normalizeNextstepWakeComment(value: unknown): NextstepWakeComment | null {
...
function normalizeNextstepWakeContinuationSummary(value: unknown): NextstepWakeContinuationSummary | null {
...
function normalizeNextstepWakeLivenessContinuation(value: unknown): NextstepWakeLivenessContinuation | null {
...
function normalizeNextstepWakeChildIssueSummary(value: unknown): NextstepWakeChildIssueSummary | null {
...
function normalizeNextstepWakeBlockerSummary(value: unknown): NextstepWakeBlockerSummary | null {
...
function normalizeNextstepWakeTreeHoldSummary(value: unknown): NextstepWakeTreeHoldSummary | null {
...
function normalizeNextstepWakeExecutionPrincipal(value: unknown): NextstepWakeExecutionPrincipal | null {
...
function normalizeNextstepWakeExecutionStage(value: unknown): NextstepWakeExecutionStage | null {
  ...
  const allowedActions = Array.isArray(stage.allowedActions)
    ...
    : [];
  const currentParticipant = normalizeNextstepWakeExecutionPrincipal(stage.currentParticipant);
  const returnAssignee = normalizeNextstepWakeExecutionPrincipal(stage.returnAssignee);
...
export function normalizeNextstepWakePayload(value: unknown): NextstepWakePayload | null {
  const payload = parseObject(value);
  const comments = Array.isArray(payload.comments)
    ? payload.comments
        .map((entry) => normalizeNextstepWakeComment(entry))
        .filter((entry): entry is NextstepWakeComment => Boolean(entry))
    : [];
  ...
  const commentIds = Array.isArray(payload.commentIds)
    ? payload.commentIds
        ...
        .map((entry) => entry.trim())
    : [];
  const executionStage = normalizeNextstepWakeExecutionStage(payload.executionStage);
  const continuationSummary = normalizeNextstepWakeContinuationSummary(payload.continuationSummary);
  const livenessContinuation = normalizeNextstepWakeLivenessContinuation(payload.livenessContinuation);
  const childIssueSummaries = Array.isArray(payload.childIssueSummaries)
    ? payload.childIssueSummaries
        .map((entry) => normalizeNextstepWakeChildIssueSummary(entry))
        .filter((entry): entry is NextstepWakeChildIssueSummary => Boolean(entry))
    : [];
  ...
  const unresolvedBlockerSummaries = Array.isArray(payload.unresolvedBlockerSummaries)
    ? payload.unresolvedBlockerSummaries
        .map((entry) => normalizeNextstepWakeBlockerSummary(entry))
        .filter((entry): entry is NextstepWakeBlockerSummary => Boolean(entry))
    : [];

  const activeTreeHold = normalizeNextstepWakeTreeHoldSummary(payload.activeTreeHold);
  if (comments.length === 0 && commentIds.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && !activeTreeHold && !executionStage && !continuationSummary && !livenessContinuation && !normalizeNextstepWakeIssue(payload.issue)) {
    return null;
  }

  return {
    reason: asString(payload.reason, "").trim() || null,
    issue: normalizeNextstepWakeIssue(payload.issue),
...
export function stringifyNextstepWakePayload(value: unknown): string | null {
  const normalized = normalizeNextstepWakePayload(value);
  if (!normalized) return null;
  return JSON.stringify(normalized);
}

export function readNextstepIssueWorkModeFromContext(value: unknown): string | null {
  const context = parseObject(value);
  const issue = parseObject(context.NextstepIssue);
  const direct = asString(issue.workMode, "").trim();
  if (direct) return direct;
  const wake = normalizeNextstepWakePayload(context.NextstepWake);
  return wake?.issue?.workMode ?? null;
}

export function renderNextstepWakePrompt(
  value: unknown,
  ...
  const principalLabel = (principal: NextstepWakeExecutionPrincipal | null) => {
  ...
  const lines = resumedSession
      ? [
        "## Nextstep Resume Delta",
        "",
        "You are resuming an existing Nextstep session.",
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "NEXTSTEP_RESOLVED_COMMAND"] = redactCommandTextForLogs(resolvedCommand);
  }

  return redactEnvForLogs(merged);
}

export function buildNextstepEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    NEXTSTEP_AGENT_ID: agent.id,
    NEXTSTEP_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.NEXTSTEP_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.NEXTSTEP_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl =
    process.env.NEXTSTEP_RUNTIME_API_URL ??
    process.env.NEXTSTEP_API_URL ??
    `http://${runtimeHost}:${runtimePort}`;
  vars.NEXTSTEP_API_URL = apiUrl;
  return vars;
}

export function applyNextstepWorkspaceEnv(
  env: Record<string, string>,
  input: {
  ...
  const mappings = [
    ["NEXTSTEP_WORKSPACE_CWD", input.workspaceCwd],
    ["NEXTSTEP_WORKSPACE_SOURCE", input.workspaceSource],
    ["NEXTSTEP_WORKSPACE_STRATEGY", input.workspaceStrategy],
    ["NEXTSTEP_WORKSPACE_ID", input.workspaceId],
    ["NEXTSTEP_WORKSPACE_REPO_URL", input.workspaceRepoUrl],
    ["NEXTSTEP_WORKSPACE_REPO_REF", input.workspaceRepoRef],
    ["NEXTSTEP_WORKSPACE_BRANCH", input.workspaceBranch],
    ["NEXTSTEP_WORKSPACE_WORKTREE_PATH", input.workspaceWorktreePath],
...
export function shapeNextstepWorkspaceEnvForExecution(input: {
  workspaceCwd?: string | null;
  workspaceWorktreePath?: string | null;
  ...
  if (executionCwd === null) {
    ...
    console.warn(
      "[Nextstep] shapeNextstepWorkspaceEnvForExecution called with executionCwd=null on a remote target; " +
...
export function refreshNextstepWorkspaceEnvForExecution(input: {
  env: Record<string, string>;
  ...
  const shapedWorkspaceEnv = shapeNextstepWorkspaceEnvForExecution({
  ...
  delete input.env.NEXTSTEP_WORKSPACE_CWD;
  delete input.env.NEXTSTEP_WORKSPACE_WORKTREE_PATH;
  delete input.env.NEXTSTEP_WORKSPACES_JSON;

  applyNextstepWorkspaceEnv(input.env, {
  ...
  if (shapedWorkspaceEnv.workspaceHints.length > 0) {
    input.env.NEXTSTEP_WORKSPACES_JSON = JSON.stringify(shapedWorkspaceEnv.workspaceHints);
  }
...
export function sanitizeInheritedNextstepEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...baseEnv };
  for (const key of Object.keys(env)) {
    if (!key.startsWith("NEXTSTEP_")) continue;
    if (key === "NEXTSTEP_RUNTIME_API_URL") continue;
    if (key === "NEXTSTEP_LISTEN_HOST") continue;
    if (key === "NEXTSTEP_LISTEN_PORT") continue;
    delete env[key];
  }
  return env;
}
...
export async function resolveNextstepSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...NEXTSTEP_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listNextstepSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<NextstepSkillEntry[]> {
  const root = await resolveNextstepSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return Promise.all(dirs.map(async (entry) => {
      ...
      return {
        key: `nextstepai/nextstep/${entry.name}`,
        runtimeName: entry.name,
        source: skillDir,
        required,
        requiredReason: required
          ? "Bundled Nextstep skills are always available for local adapters."
          : null,
      };
    }));
  } catch {
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Nextstep skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Nextstep cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredNextstepRuntimeSkills(value: unknown): NextstepSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: NextstepSkillEntry[] = [];
...
export async function readNextstepRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<NextstepSkillEntry[]> {
  const configuredEntries = normalizeConfiguredNextstepRuntimeSkills(config.NextstepRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listNextstepSkillEntries(moduleDir, additionalCandidates);
}

export async function readNextstepSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
...
export function readNextstepSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.NextstepSkillSync;
...
function canonicalizeDesiredNextstepSkillReference(
  reference: string,
...
export function resolveNextstepDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readNextstepSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredNextstepSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeNextstepSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.NextstepSkillSync;
  const current =
  ...
  next.NextstepSkillSync = current;
  return next;
}

export async function ensureNextstepSkillSymlink(
  source: string,
  target: string,
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out waiting for Nextstep skill materialization lock at ${lockDir}`);
      }
...
export async function materializeNextstepSkillCopy(
  source: string,
  target: string,
): Promise<MaterializedNextstepSkillCopyResult> {
  ...
  if (!rootStat.isDirectory()) {
    throw new Error("Nextstep skills must be directories.");
  }

  const result: MaterializedNextstepSkillCopyResult = {
    copiedFiles: 0,
    skippedSymlinks: [],
  };
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    const rawMerged: NodeJS.ProcessEnv = {
      ...sanitizeInheritedNextstepEnv(process.env),
      ...opts.env,
    };

    // Strip Claude Code nesting-guard env vars so spawned `claude` processes
    // don't refuse to start with "cannot be launched inside another session".
    // These vars leak in when the Nextstep server itself is started from
    // within a Claude Code session (e.g. `npx nextstepai run` in a terminal
