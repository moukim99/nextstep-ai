...
describe("sandbox callback bridge", () => {
    ...
    return {
      execute: async (input: {
        ...
        if (
          ...
          env.NEXTSTEP_TEST_STDIN = input.stdin;
          args[1] = `printf '%s' \"$NEXTSTEP_TEST_STDIN\" | (${args[1]})`;
        }
        try {
  ...
  it("round-trips localhost bridge requests over the sandbox queue without forwarding the bridge token", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-runtime-"));
    ...
    const queueDir = path.posix.join(prepared.runtimeRootDir, "Nextstep-bridge");
    ...
    const okResponse = await fetch(`${bridge.baseUrl}/api/agents/me?view=compact`, {
      headers: {
        ...
        "x-Nextstep-run-id": "run-bridge-1",
    ...
    expect(seenRequests[0]?.headers["x-Nextstep-run-id"]).toBeUndefined();

  });

  it("denies non-allowlisted requests by default", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-default-policy-"));
    cleanupDirs.push(rootDir);
  ...
  it("drains already-queued requests on stop", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-drain-"));
    cleanupDirs.push(rootDir);
  ...
  it("writes fast 503 responses for queued requests that miss the drain deadline", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-drain-timeout-"));
    cleanupDirs.push(rootDir);
  ...
  it("handles SSH queue polling failures without emitting an unhandled rejection", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-ssh-failure-"));
    cleanupDirs.push(rootDir);
    ...
    try {
      const worker = await startSandboxCallbackBridgeWorker({
        client: {
          ...
          listJsonFiles: async () => {
            throw new Error(
              "list /remote/.Nextstep-runtime/gemini/Nextstep-bridge/queue/requests failed with exit code 255: kex_exchange_identification: read: Connection reset by peer",
            );
          },
          readTextFile: async () => {
  ...
  it("serializes remote response writes so stop does not recreate a late orphaned response", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-response-lock-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(
      readdir(directories.responsesDir).then((entries) =>
        entries.filter((entry) => entry.endsWith(".tmp") || entry.includes(".Nextstep-write.lock")),
      ),
    ).resolves.toEqual([]);
  });

  it("rejects non-JSON request bodies and full queues at the bridge server", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-server-guards-"));
    cleanupDirs.push(rootDir);
  ...
  it("returns a 502 when the host response times out", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-timeout-"));
    cleanupDirs.push(rootDir);
  ...
  it("returns a 502 for malformed host response files", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-malformed-response-"));
    cleanupDirs.push(rootDir);
  ...
  it("reuses an already-uploaded bridge entrypoint when the remote file hash matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-sync-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(readFile(path.posix.join(remoteAssetDir, "Nextstep-bridge-server.mjs"), "utf8")).resolves.toBe(expandedSource);
    await expect(
      readdir(remoteAssetDir).then((entries) =>
        entries.filter(
          (entry) =>
            entry.endsWith(".Nextstep-upload.b64") ||
            entry.endsWith(".partial") ||
            entry === ".Nextstep-bridge-upload.lock",
        ),
      ),
    ).resolves.toEqual([]);
  });
  ...
  it("rejects a corrupted bridge entrypoint upload without committing a torn remote file", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-bridge-sync-corrupt-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(readFile(path.posix.join(remoteAssetDir, "Nextstep-bridge-server.mjs"), "utf8")).rejects.toThrow();
    await expect(
      readdir(remoteAssetDir).then((entries) =>
        entries.filter(
          (entry) =>
