...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const runtimeRootDir = path.posix.join(workspaceRemoteDir, ".Nextstep-runtime", input.adapterKey);
  const baselineSnapshot = await captureDirectorySnapshot(input.workspaceLocalDir, {
    exclude: [...new Set([".Nextstep-runtime", ...(input.preserveAbsentOnRestore ?? []), ...(input.workspaceExclude ?? [])])],
  });

  await withTempDir("Nextstep-sandbox-sync-", async (tempDir) => {
    ...
    const preservedNames = new Set([".Nextstep-runtime", ...(input.preserveAbsentOnRestore ?? [])]);
  ...
  return {
    ...
    restoreWorkspace: async () => {
      await withTempDir("Nextstep-sandbox-restore-", async (tempDir) => {
