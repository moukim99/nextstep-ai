...
export interface AdapterExecutionTargetNextstepBridgeHandle {
  env: Record<string, string>;
  stop(): Promise<void>;
}
...
function resolveDefaultNextstepApiUrl(): string {
  const runtimeHost = resolveHostForUrl(
    process.env.NEXTSTEP_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  // 3100 matches the default Nextstep dev server port when the runtime does not provide one.
...
function isBridgeDebugEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.NEXTSTEP_BRIDGE_DEBUG?.trim().toLowerCase();
...
export function adapterExecutionTargetUsesNextstepBridge(
  target: AdapterExecutionTarget | null | undefined,
): boolean {
  return target?.kind === "remote";
}
  ...
  if (detectCommand) {
    ...
    if (!recheck.timedOut && recheck.exitCode === 0) {
      if (input.onLog) {
        ...
        await input.onLog(
          "stderr",
          `[Nextstep] Install command ${reason} (${installCommand}) but ${detectCommand} is on PATH; continuing.\n`,
        );
      }
      return;
    }
 ...
 * the directory state inside the environment, not on the Nextstep host.
 *
...
export function runtimeAssetDir(
  ...
  return prepared.assetDirs[key] ?? path.posix.join(fallbackRemoteCwd, ".Nextstep-runtime", key);
}
...
export async function startAdapterExecutionTargetNextstepBridge(input: {
  runId: string;
  target: AdapterExecutionTarget | null | undefined;
...
}): Promise<AdapterExecutionTargetNextstepBridgeHandle | null> {
  if (!adapterExecutionTargetUsesNextstepBridge(input.target)) {
    return null;
  }
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side Nextstep API token.");
  }

  const runtimeRootDir =
    input.runtimeRootDir?.trim().length
      ? input.runtimeRootDir.trim()
      : path.posix.join(target.remoteCwd, ".Nextstep-runtime", input.adapterKey);
  const bridgeRuntimeDir = path.posix.join(runtimeRootDir, "Nextstep-bridge");
  ...
  const hostApiUrl =
    ...
    process.env.NEXTSTEP_RUNTIME_API_URL?.trim() ||
    process.env.NEXTSTEP_API_URL?.trim() ||
    resolveDefaultNextstepApiUrl();
  const shellCommand = adapterExecutionTargetShellCommand(target);
  const runner = adapterExecutionTargetCommandRunner(target);

  await onLog(
    "stdout",
    `[Nextstep] Starting sandbox callback bridge for ${input.adapterKey} in ${bridgeRuntimeDir}.\n`,
  );
  ...
  try {
    ...
    // NEXTSTEP_BRIDGE_DEBUG opts into verbose stdout logs of every bridge
    ...
    worker = await startSandboxCallbackBridgeWorker({
      ...
      handleRequest: async (request) => {
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[Nextstep] Bridge proxy ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        ...
        headers.set("x-Nextstep-run-id", input.runId);
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[Nextstep] Bridge proxy response ${response.status} for ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        return {
  ...
  return {
    env: {
      NEXTSTEP_API_URL: server.baseUrl,
      NEXTSTEP_API_KEY: bridgeToken,
