...
async function createSshAuthArgs(
  ...
  if (config.strictHostKeyChecking) {
    if (config.knownHosts) {
      const knownHosts = await withTempFile("Nextstep-ssh-known-hosts-", config.knownHosts, 0o600);
  ...
  if (config.privateKey) {
    const privateKey = await withTempFile("Nextstep-ssh-key-", config.privateKey, 0o600);
...
async function importGitWorkspaceToSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-ssh-bundle-"));
  ...
  const tempRef = `refs/Nextstep/ssh-sync/import/${randomUUID()}`;
  ...
  try {
    ...
    const remoteSetupScript = [
      "set -e",
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".Nextstep-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".Nextstep-runtime", "import-XXXXXX.bundle"))})`,
      ...
      `git -C ${shellQuote(input.remoteDir)} clean -fdx -e .Nextstep-runtime >/dev/null`,
...
async function exportGitWorkspaceFromSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-ssh-bundle-"));
  const bundlePath = path.join(bundleDir, "workspace.bundle");
  const importedRef = input.importedRef ?? `refs/Nextstep/ssh-sync/imported/${randomUUID()}`;

  try {
    const exportScript = [
      "set -e",
      `git -C ${shellQuote(input.remoteDir)} update-ref refs/Nextstep/ssh-sync/export HEAD`,
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".Nextstep-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".Nextstep-runtime", "export-XXXXXX.bundle"))})`,
      'cleanup() { rm -f "$tmp_bundle"; git -C ' + shellQuote(input.remoteDir) + ' update-ref -d refs/Nextstep/ssh-sync/export >/dev/null 2>&1 || true; }',
      'trap cleanup EXIT',
      `git -C ${shellQuote(input.remoteDir)} bundle create "$tmp_bundle" refs/Nextstep/ssh-sync/export >/dev/null`,
      'cat "$tmp_bundle"',
    ].join("\n");
    ...
    await runLocalGit(input.localDir, ["fetch", "--force", bundlePath, `refs/Nextstep/ssh-sync/export:${importedRef}`], {
      timeout: 60_000,
      maxBuffer: 1024 * 1024,
    });
...
async function integrateImportedGitHead(input: {
  ...
  const mergeCommit = await runLocalGit(
    ...
    [
      ...
      input.importedHead,
      "-m",
      `Nextstep SSH sync merge ${input.importedHead.slice(0, 12)}`,
    ],
    {
      timeout: 60_000,
      maxBuffer: 64 * 1024,
...
export async function syncDirectoryFromSsh(input: {
  ...
  const auth = await createSshAuthArgs(input.spec);
  const stagingDir = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-ssh-sync-back-"));
...
export async function prepareWorkspaceForSshExecution(input: {
  ...
  await clearRemoteDirectory({
    spec: input.spec,
    remoteDir,
    preserveEntries: [".Nextstep-runtime"],
  });
  await syncDirectoryToSsh({
    spec: input.spec,
    localDir: input.localDir,
    remoteDir,
    exclude: [".Nextstep-runtime"],
  });
...
export async function restoreWorkspaceFromSshExecution(input: {
  ...
  if (input.baselineSnapshot) {
    ...
    const importedRef = input.restoreGitHistory
      ? `refs/Nextstep/ssh-sync/imported/${randomUUID()}`
      : null;
    try {
