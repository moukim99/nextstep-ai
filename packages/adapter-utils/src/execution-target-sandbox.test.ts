...
describe("sandbox adapter execution targets", () => {
  ...
  it("treats SSH targets as bridge-only", () => {
    ...
    expect(adapterExecutionTargetUsesNextstepBridge(target)).toBe(true);
    expect(adapterExecutionTargetSessionIdentity(target)).toEqual({
      transport: "ssh",
      host: "ssh.example.test",
      port: 22,
  ...
  it("starts a localhost Nextstep bridge for sandbox targets in bridge mode", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-execution-target-bridge-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".Nextstep-runtime", "codex");
    ...
    const apiServer = createServer((req, res) => {
      requests.push({
        ...
        runId: typeof req.headers["x-Nextstep-run-id"] === "string" ? req.headers["x-Nextstep-run-id"] : null,
      });
    ...
    const bridge = await startAdapterExecutionTargetNextstepBridge({
      runId: "run-bridge",
      target,
      runtimeRootDir,
      adapterKey: "codex",
    ...
    try {
      ...
      expect(bridge?.env.NEXTSTEP_API_URL).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(bridge?.env.NEXTSTEP_API_KEY).not.toBe("real-run-jwt");
      expect(bridge?.env.NEXTSTEP_API_BRIDGE_MODE).toBe("queue_v1");

      const response = await fetch(`${bridge!.env.NEXTSTEP_API_URL}/api/agents/me`, {
        headers: {
          authorization: `Bearer ${bridge!.env.NEXTSTEP_API_KEY}`,
  ...
  it("fails oversized host responses with a 502 before returning them to the sandbox client", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "Nextstep-execution-target-bridge-limit-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".Nextstep-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
