...
describe("buildInvocationEnvForLogs", () => {
  it("redacts inline secrets from resolved command metadata", () => {
    ...
    expect(loggedEnv.NEXTSTEP_RESOLVED_COMMAND).toBe(
...
describe("materializeNextstepSkillCopy", () => {
  it("refuses to materialize into an ancestor of the source", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-skill-copy-"));
    try {
      const source = path.join(root, "parent", "skill");
      await fs.mkdir(source, { recursive: true });
      await fs.writeFile(path.join(source, "SKILL.md"), "# skill\n", "utf8");

      await expect(materializeNextstepSkillCopy(source, path.join(root, "parent"))).rejects.toThrow(
        /ancestor/,
      );
  ...
  it("does not delete and recopy an unchanged materialized skill target", async () => {
    ...
    try {
      ...
      const first = await materializeNextstepSkillCopy(source, target);
      expect(first.copiedFiles).toBe(1);
      await fs.writeFile(path.join(target, "local-marker.txt"), "keep\n", "utf8");

      const second = await materializeNextstepSkillCopy(source, target);
  ...
  it("breaks stale materialization locks left by dead processes", async () => {
    ...
    try {
      ...
      await expect(materializeNextstepSkillCopy(source, target)).resolves.toMatchObject({ copiedFiles: 1 });
...
describe("renderNextstepWakePrompt", () => {
  it("keeps the default local-agent prompt action-oriented", () => {
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("Start actionable work in this heartbeat");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("do not stop at a plan");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("clear final disposition");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("evidence, not valid liveness paths by themselves");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("keep `in_progress` only when a live continuation path exists");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("Prefer the smallest verification that proves the change");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("Use child issues");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("instead of polling agents, sessions, or processes");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("Create child issues directly when you know what needs to be done");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("POST /api/issues/{issueId}/interactions");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("kind suggest_tasks, ask_user_questions, or request_confirmation");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("confirmation:{issueId}:plan:{revisionId}");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain("Wait for acceptance before creating implementation subtasks");
    expect(DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE).toContain(
      "Respect budget, pause/cancel, approval gates, and company boundaries",
    );
  });

  it("adds the execution contract to scoped wake prompts", () => {
    const prompt = renderNextstepWakePrompt({
    ...
    expect(prompt).toContain("## Nextstep Wake Payload");
  ...
  it("renders planning-mode directives for assignment and comment wakes", () => {
    const assignmentPrompt = renderNextstepWakePrompt({
      reason: "issue_assigned",
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
    ...
    const commentPrompt = renderNextstepWakePrompt({
  ...
  it("includes continuation and child issue summaries in structured wake context", () => {
    ...
    expect(JSON.parse(stringifyNextstepWakePayload(payload) ?? "{}")).toMatchObject({
      continuationSummary: {
    ...
    const prompt = renderNextstepWakePrompt(payload);
...
describe("applyNextstepWorkspaceEnv", () => {
  it("adds shared workspace env vars including AGENT_HOME", () => {
    const env = applyNextstepWorkspaceEnv(
      {},
      {
        ...
        workspaceRepoUrl: "https://github.com/nextstepai/nextstep.git",
    ...
    expect(env).toEqual({
      NEXTSTEP_WORKSPACE_CWD: "/tmp/workspace",
      NEXTSTEP_WORKSPACE_SOURCE: "project_primary",
      NEXTSTEP_WORKSPACE_STRATEGY: "git_worktree",
      NEXTSTEP_WORKSPACE_ID: "workspace-1",
      NEXTSTEP_WORKSPACE_REPO_URL: "https://github.com/nextstepai/nextstep.git",
      NEXTSTEP_WORKSPACE_REPO_REF: "main",
      NEXTSTEP_WORKSPACE_BRANCH: "feature/test",
      NEXTSTEP_WORKSPACE_WORKTREE_PATH: "/tmp/worktree",
...
describe("shapeNextstepWorkspaceEnvForExecution", () => {
  it("rewrites workspace env paths for remote execution", () => {
    const shaped = shapeNextstepWorkspaceEnvForExecution({
      workspaceCwd: "/tmp/workspace",
      workspaceWorktreePath: "/tmp/worktree",
      workspaceHints: [
        {
          workspaceId: "workspace-1",
          cwd: "/tmp/workspace",
          repoUrl: "https://github.com/nextstepai/nextstep.git",
        },
        {
...
describe("refreshNextstepWorkspaceEnvForExecution", () => {
  it("rewrites Nextstep workspace env to the prepared remote runtime cwd", () => {
    const env: Record<string, string> = {
      NEXTSTEP_WORKSPACE_CWD: "/remote/workspace",
      NEXTSTEP_WORKSPACE_WORKTREE_PATH: "/host/worktree",
      NEXTSTEP_WORKSPACES_JSON: JSON.stringify([
    ...
    const shaped = refreshNextstepWorkspaceEnvForExecution({
      env,
      envConfig: {
        QA_PROJECT_WORKSPACE_CWD: "/host/workspace",
      },
      ...
      executionTargetIsRemote: true,
      executionCwd: "/remote/workspace/.Nextstep-runtime/runs/run-1/workspace",
    });

    expect(shaped).toEqual({
      workspaceCwd: "/remote/workspace/.Nextstep-runtime/runs/run-1/workspace",
      workspaceWorktreePath: null,
      workspaceHints: [
        {
          workspaceId: "workspace-1",
          cwd: "/remote/workspace/.Nextstep-runtime/runs/run-1/workspace",
        },
        {
          workspaceId: "workspace-2",
        },
    ...
    expect(env.NEXTSTEP_WORKSPACE_CWD).toBe("/remote/workspace/.Nextstep-runtime/runs/run-1/workspace");
    expect(env.NEXTSTEP_WORKSPACE_WORKTREE_PATH).toBeUndefined();
    expect(env.QA_PROJECT_WORKSPACE_CWD).toBe("/remote/workspace/.Nextstep-runtime/runs/run-1/workspace");
    expect(JSON.parse(env.NEXTSTEP_WORKSPACES_JSON ?? "[]")).toEqual([
      {
        workspaceId: "workspace-1",
        cwd: "/remote/workspace/.Nextstep-runtime/runs/run-1/workspace",
      },
