# `@nextstepai/plugin-sdk`

Official TypeScript SDK for Nextstep plugin authors.

- **Worker SDK:** `@nextstepai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@nextstepai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@nextstepai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@nextstepai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@nextstepai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@nextstepai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@nextstepai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, `useHostNavigation`, slot prop types |
| `@nextstepai/plugin-sdk/ui/hooks` | Hooks only |
| `@nextstepai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@nextstepai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@nextstepai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@nextstepai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@nextstepai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@nextstepai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @nextstepai/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Nextstep app. They can call ordinary Nextstep HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Nextstep instance at runtime.
...
- The host ships a small shared React component kit through `@nextstepai/plugin-sdk/ui`. Use it for native Nextstep controls; custom React and CSS are still supported.
...
```ts
import { definePlugin, runWorker } from "@nextstepai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@nextstepai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Nextstep information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@nextstepai/plugin-sdk`.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:Nextstep.missions:feature",
...
```tsx
import { usePluginData, usePluginAction } from "@nextstepai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@nextstepai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@nextstepai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@nextstepai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@nextstepai/plugin-sdk/ui";
...
Routes Nextstep-internal plugin links through the host router without a full document reload. Use `linkProps()` for anchors so the browser still gets a real `href` for copy-link, modifier-click, middle-click, and open-in-new-tab behavior.

```tsx
import { useHostNavigation } from "@nextstepai/plugin-sdk/ui";
...
Avoid raw same-origin `href`s or `window.location.assign()` for Nextstep-internal navigation from plugin UI. Those bypass the host router and can reload the whole app. External links should keep normal anchors with `target="_blank"` and `rel="noopener noreferrer"` as appropriate.
...
```tsx
import { usePluginStream } from "@nextstepai/plugin-sdk/ui";
...
The host provides selected shared UI components through `@nextstepai/plugin-sdk/ui`.
...
used by Nextstep issue comments and documents:

```tsx
import { MarkdownBlock, MarkdownEditor } from "@nextstepai/plugin-sdk/ui";
...
```tsx
<MarkdownBlock
  content={"See [[wiki/entities/Nextstep|Nextstep]]."}
  enableWikiLinks
...
```tsx
import { FileTree, type FileTreeNode } from "@nextstepai/plugin-sdk/ui";
...
or configure work against Nextstep entities. Both are controlled components and
load their options from the host for the provided company.

```tsx
import { AssigneePicker, ProjectPicker } from "@nextstepai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@nextstepai/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@nextstepai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@nextstepai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@nextstepai/plugin-sdk/bundlers";
...
```bash
Nextstep-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@nextstepai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__NEXTSTEP__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__NEXTSTEP__/events` streams `reload` SSE events on UI build changes
