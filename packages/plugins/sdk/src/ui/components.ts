 ...
 *
 * These components are exported from `@nextstepai/plugin-sdk/ui` and are
 ...
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components In `@nextstepai/plugin-sdk/ui`
 */
...
function createSdkUiComponent<TProps>(name: string): React.ComponentType<TProps> {
  return function NextstepSdkUiComponent(props: TProps) {
...
/**
 * Renders Nextstep's shared Markdown editor.
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components
 */
...
/**
 * Renders Nextstep's native issue list component for company-scoped plugin
...
/**
 * Renders Nextstep's native managed routines list for plugin settings pages.
 */
