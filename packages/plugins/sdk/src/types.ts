/**
 * Core types for the Nextstep plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@nextstepai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @nextstepai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Nextstep object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Nextstep UI. */
  title?: string;
...
export interface PluginLocalFoldersClient {
  ...
  declarations(): import("@nextstepai/shared").PluginLocalFolderDeclaration[];
...
/**
 * `ctx.events` — subscribe to and emit Nextstep domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Nextstep domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Nextstep secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Nextstep secret provider (e.g. `"MY_API_KEY"`).
   *
...
/**
 * `ctx.routines` — resolve and reconcile plugin-managed Nextstep routines.
 *
 ...
 * ```
 *
 * @see usePluginStream in `@nextstepai/plugin-sdk/ui`
 */
