...
function requireCapability(manifest: NextstepPluginManifestV1, allowed: Set<PluginCapability>, capability: PluginCapability) {
 ...
 * can validate plugin behavior without spinning up the Nextstep server runtime.
 */
export function createTestHarness(options: TestHarnessOptions): TestHarness {
  ...
  function isManagedAgent(agent: Agent, agentKey: string) {
    const marker = agent.metadata?.NextstepManagedResource;
    return Boolean(
      marker
