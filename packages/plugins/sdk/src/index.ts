/**
 * `@nextstepai/plugin-sdk` — Nextstep plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@nextstepai/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@nextstepai/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @nextstepai/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@nextstepai/plugin-sdk) for all plugin authoring needs.
export type {
 ...
 * @example
 * ```ts
 * import { z } from "@nextstepai/plugin-sdk";
 *
