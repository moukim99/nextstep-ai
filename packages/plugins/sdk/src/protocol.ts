 ...
 * channel.
 *
 * The Nextstep plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@nextstepai/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all Nextstep messages.
 */
...
/**
 * Nextstep plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Nextstep instance. */
    instanceId: string;
    /** Semver version of the running Nextstep host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
