...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".Nextstep-sdk");
...
/**
 * Generate a complete Nextstep plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Nextstep plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "Nextstep-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["Nextstep", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@nextstepai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@nextstepai/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@nextstepai/plugin-sdk/ui";
...
} from "@nextstepai/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@nextstepai/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@nextstepai/plugin-sdk\` and \`@nextstepai/shared\` from a local Nextstep checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.Nextstep-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-Nextstep-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <Nextstep-sdk-path>]");
    process.exit(1);
  }
