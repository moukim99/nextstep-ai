...
Example Nextstep plugin that demonstrates:
...
```bash
pnpm --filter @nextstepai/plugin-file-browser-example build
pnpm nextstepai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm nextstepai plugin uninstall Nextstep-file-browser-example --force
```
...
- Optional: use `Nextstep-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
