...
import type { Goal, Issue } from "@nextstepai/shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.companySummary,
    {
      ...
      description: "Summarizes current company counts from the Nextstep APIs.",
...
const plugin: NextstepPlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
