...
type NextstepSprite = (typeof NEXTSTEP_SPRITES)[number];
...
function spriteSize(sprite: NextstepSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = NEXTSTEP_SPRITES[(baseCol + baseRow) % NEXTSTEP_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = NEXTSTEP_SPRITES[Math.floor(Math.random() * NEXTSTEP_SPRITES.length)]!;
