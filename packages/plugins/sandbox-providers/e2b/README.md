# `@nextstepai/plugin-e2b`

Published E2B sandbox provider plugin for Nextstep.

This package lives in the Nextstep monorepo, but it is intentionally excluded from the root `pnpm` workspace and shaped to publish and install like a standalone npm package. That means operators can install it from the Plugins page by package name, and the host will fetch its transitive dependencies at install time without adding lockfile churn to the Nextstep repo.

## Install

From a Nextstep instance, install:
...
- When you save an environment, Nextstep stores pasted API keys as company secrets.
...
These commands assume the repo root has already been installed once so the local `@nextstepai/plugin-sdk` workspace package is available to the compiler during development.

## Package layout

- `src/manifest.ts` declares the sandbox-provider driver metadata
- `src/plugin.ts` implements the environment lifecycle hooks
- `NextstepPlugin.manifest` and `NextstepPlugin.worker` point the host at the built plugin entrypoints in `dist/`
