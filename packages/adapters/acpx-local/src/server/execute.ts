...
import type { AdapterExecutionContext, AdapterExecutionResult } from "@nextstepai/adapter-utils";
import { readAdapterExecutionTarget, adapterExecutionTargetSessionIdentity } from "@nextstepai/adapter-utils/execution-target";
import {
  DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE,
  applyNextstepWorkspaceEnv,
  asNumber,
  asString,
  ...
  ensurePathInEnv,
  joinPromptSections,
  materializeNextstepSkillCopy,
  parseObject,
  readNextstepRuntimeSkillEntries,
  readNextstepIssueWorkModeFromContext,
  renderNextstepWakePrompt,
  renderTemplate,
  resolveNextstepDesiredSkillNames,
  rewriteWorkspaceCwdEnvVarsForExecution,
  shapeNextstepWorkspaceEnvForExecution,
  stringifyNextstepWakePayload,
  type NextstepSkillEntry,
} from "@nextstepai/adapter-utils/server-utils";
import { shellQuote } from "@nextstepai/adapter-utils/ssh";
import {
  createAcpRuntime,
  createAgentRegistry,
  createRuntimeStore,
...
const NEXTSTEP_MANAGED_CODEX_SKILLS_MANIFEST = ".Nextstep-managed-skills.json";
...
function defaultNextstepInstanceDir(): string {
  const home = process.env.NEXTSTEP_HOME?.trim() || path.join(os.homedir(), ".Nextstep");
  const instanceId = process.env.NEXTSTEP_INSTANCE_ID?.trim() || "default";
  return path.join(home, "instances", instanceId);
}

function defaultStateDir(companyId: string, agentId: string): string {
  return path.join(defaultNextstepInstanceDir(), "companies", companyId, "acpx-local", "agents", agentId);
}

function resolveManagedCodexHomeDir(companyId: string): string {
  return path.join(defaultNextstepInstanceDir(), "companies", companyId, "codex-home");
}
...
async function prepareManagedCodexHome(input: {
  ...
  await onLog(
    "stdout",
    `[Nextstep] Using Nextstep-managed ACPX Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
...
async function buildSkillSetKey(input: {
  skills: NextstepSkillEntry[];
  label: string;
}): Promise<string> {
  const hash = createHash("sha256");
  hash.update(`Nextstep-acpx-${input.label}-skills:v1\n`);
...
): Promise<{ allSkills: NextstepSkillEntry[]; selectedSkills: NextstepSkillEntry[]; desiredSkillNames: string[] }> {
  const allSkills = await readNextstepRuntimeSkillEntries(config, __moduleDir);
  const desiredSkillNames = resolveNextstepDesiredSkillNames(config, allSkills);
...
async function prepareClaudeSkillRuntime(input: {
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      const result = await materializeNextstepSkillCopy(entry.source, target);
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[Nextstep] Materialized ACPX Claude skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[Nextstep] Failed to materialize ACPX Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
  ...
  const promptInstructions = selectedSkills.length > 0
    ? [
        "Nextstep has materialized selected runtime skills for this ACPX Claude session.",
  ...
  return {
    ...
    commandNotes: selectedSkills.length > 0
      ? [`Materialized ${selectedSkills.length} Nextstep skill(s) for ACPX Claude at ${skillsHome}.`]
      : [],
  };
}

async function readManagedCodexSkillsManifest(skillsHome: string): Promise<Set<string>> {
  const manifestPath = path.join(skillsHome, NEXTSTEP_MANAGED_CODEX_SKILLS_MANIFEST);
  try {
...
async function writeManagedCodexSkillsManifest(skillsHome: string, skillNames: Iterable<string>): Promise<void> {
  ...
  await fs.writeFile(
    path.join(skillsHome, NEXTSTEP_MANAGED_CODEX_SKILLS_MANIFEST),
...
async function reconcileManagedCodexSkills(input: {
  skillsHome: string;
  allSkills: NextstepSkillEntry[];
  selectedSkills: NextstepSkillEntry[];
  onLog: AdapterExecutionContext["onLog"];
}): Promise<void> {
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[Nextstep] Revoked ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const entry of input.allSkills) {
    ...
    if (await removeSkillTarget(target)) {
      await input.onLog("stdout", `[Nextstep] Revoked legacy ACPX Codex skill "${entry.runtimeName}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[Nextstep] Revoked unavailable ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
}
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[Nextstep] Materialized ACPX Codex skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[Nextstep] Failed to inject ACPX Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
async function buildRuntime(input: {
  ...
  const workspaceContext = parseObject(context.NextstepWorkspace);
  ...
  const hasExplicitApiKey =
    typeof envConfig.NEXTSTEP_API_KEY === "string" && envConfig.NEXTSTEP_API_KEY.trim().length > 0;
  const env: Record<string, string> = { ...buildNextstepEnv(agent), NEXTSTEP_RUN_ID: runId };
  const wakeTaskId =
  ...
  const linkedIssueIds = Array.isArray(context.issueIds)
    ...
    : [];
  const wakePayloadJson = stringifyNextstepWakePayload(context.NextstepWake);
  const issueWorkMode = readNextstepIssueWorkModeFromContext(context);
  if (wakeTaskId) env.NEXTSTEP_TASK_ID = wakeTaskId;
  if (issueWorkMode) env.NEXTSTEP_ISSUE_WORK_MODE = issueWorkMode;
  if (wakeReason) env.NEXTSTEP_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.NEXTSTEP_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.NEXTSTEP_APPROVAL_ID = approvalId;
  if (approvalStatus) env.NEXTSTEP_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.NEXTSTEP_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (wakePayloadJson) env.NEXTSTEP_WAKE_PAYLOAD_JSON = wakePayloadJson;
  applyNextstepWorkspaceEnv(env, {
  ...
  if (!hasExplicitApiKey && authToken) env.NEXTSTEP_API_KEY = authToken;
  ...
  if (acpxAgent === "claude") {
    ...
    const desired = resolveNextstepDesiredSkillNames(config, await readNextstepRuntimeSkillEntries(config, __moduleDir));
    skillsIdentity = { mode: "custom_unsupported", desiredSkillNames: desired };
    if (desired.length > 0) {
      skillCommandNotes.push("Selected Nextstep skills are tracked only; ACPX custom commands do not expose a runtime skill contract yet.");
    }
  }
  ...
  const sessionKey = `Nextstep:${agent.companyId}:${agent.id}:${taskKey}:${fingerprint}`;
...
async function applySessionConfigOptions(input: {
  ...
  if (!input.runtime.setConfigOption) {
    ...
    await input.onLog("stderr", `[Nextstep] ${message}\n`);
  ...
  for (const option of options) {
    ...
    await input.onLog(
      "stdout",
      `[Nextstep] Applied ACPX ${input.prepared.acpxAgent} config ${option.key}=${option.value}\n`,
    );
  }
}
...
async function buildPrompt(ctx: AdapterExecutionContext, resumedSession: boolean): Promise<{
  ...
  const promptTemplate = asString(config.promptTemplate, DEFAULT_NEXTSTEP_AGENT_PROMPT_TEMPLATE);
  ...
  if (instructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[Nextstep] Warning: could not read agent instructions file "${instructionsFilePath}": ${reason}\n`,
      );
  ...
  const renderedBootstrapPrompt =
    !resumedSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderNextstepWakePrompt(context.NextstepWake, { resumedSession });
  const shouldUseResumeDeltaPrompt = resumedSession && wakePrompt.length > 0;
  const promptInstructionsPrefix = shouldUseResumeDeltaPrompt ? "" : instructionsPrefix;
  const renderedPrompt = shouldUseResumeDeltaPrompt ? "" : renderTemplate(promptTemplate, templateData);
  const sessionHandoffNote = asString(context.NextstepSessionHandoffMarkdown, "").trim();
  const taskContextNote = asString(context.NextstepTaskMarkdown, "").trim();
...
export function createAcpxLocalExecutor(deps: ExecuteDeps = {}) {
  ...
  return async function executeAcpxLocal(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    if (!canResume && asString(previousParams.runtimeSessionName, "")) {
      await ctx.onLog(
        "stdout",
        `[Nextstep] ACPX session "${asString(previousParams.runtimeSessionName, "")}" does not match the current agent/cwd/mode/runtime identity; starting fresh in "${prepared.cwd}".\n`,
      );
    }
    ...
    try {
      if (!handle) {
        try {
          ...
          await ctx.onLog(
            "stdout",
            `[Nextstep] ACPX resume session "${resumeSessionId}" is unavailable; retrying with a fresh session.\n`,
          );
          handle = await runtime.ensureSession({
            sessionKey: prepared.sessionKey,
            agent: prepared.acpxAgent,
    ...
    try {
      ...
      await runtime.close({
        handle: sessionHandle,
        reason: "Nextstep config cleanup",
    ...
    if (ctx.onMeta) {
      await ctx.onMeta({
        ...
        cwd: prepared.cwd,
        commandNotes: [
          `ACPX runtime embedded in Nextstep with ${prepared.mode} session mode.`,
    ...
    try {
      ...
      if (terminal.status === "failed" || terminal.status === "cancelled" || timedOut) {
        ...
        if (warmHandleMatches(existing, runtime, sessionHandle) && existing) {
          await closeWarmHandle({
            ...
            entry: existing,
            reason: timedOut ? "Nextstep timeout cleanup" : `Nextstep turn ${terminal.status}`,
        ...
        if (existing && !warmHandleMatches(existing, runtime, sessionHandle)) {
          await runtime.close({
            handle: sessionHandle,
            reason: "Nextstep duplicate warm handle cleanup",
            discardPersistentState: false,
          }).catch(() => {});
        } else {
        ...
        if (warmHandleMatches(existing, runtime, sessionHandle) && existing) {
          await closeWarmHandle({
            ...
            key: prepared.sessionKey,
            entry: existing,
            reason: "Nextstep completed turn cleanup",
          });
        } else {
          await runtime.close({
            handle: sessionHandle,
      ...
      await runtime.close({
        handle: sessionHandle,
        reason: timedOut ? "Nextstep timeout cleanup" : "Nextstep error cleanup",
