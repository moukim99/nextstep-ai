...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-acpx-skills-"));
  tempRoots.push(root);
  return root;
}
...
describe("acpx_local runtime skill isolation", () => {
  it.skipIf(process.platform === "win32")("materializes ACPX Claude skills without symlinked descendants", async () => {
    ...
    const { meta } = await runExecutor({
      agent: "claude",
      stateDir,
      NextstepRuntimeSkills: [skill],
      NextstepSkillSync: { desiredSkills: [skill.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("revokes removed ACPX Codex skills and skips symlinked descendants", async () => {
    ...
    const baseConfig = {
      ...
      NextstepRuntimeSkills: [keep, remove],
    };

    await runExecutor({
      ...baseConfig,
      NextstepSkillSync: { desiredSkills: [keep.key, remove.key] },
    });
    expect(await pathExists(path.join(codexHome, "skills", remove.runtimeName, "SKILL.md"))).toBe(true);

    await runExecutor({
      ...baseConfig,
      NextstepSkillSync: { desiredSkills: [keep.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("removes legacy ACPX Codex skill symlinks when a skill is no longer desired", async () => {
    ...
    await runExecutor({
      ...
      stateDir: path.join(root, "state"),
      env: { CODEX_HOME: codexHome },
      NextstepRuntimeSkills: [legacy],
      NextstepSkillSync: { desiredSkills: [] },
    });
  ...
  it.skipIf(process.platform === "win32")("replaces stale managed Codex auth files with source symlinks", async () => {
    ...
    const NextstepInstanceId = "test-instance";
    ...
    try {
      ...
      process.env.NEXTSTEP_INSTANCE_ID = NextstepInstanceId;
      await runExecutor({
        agent: "codex",
        stateDir: path.join(root, "state"),
  ...
  it("keeps fresh credential wrapper scripts across ACPX agent changes", async () => {
    ...
    await runExecutor({
      ...baseConfig,
      agent: "custom-a",
      env: { NEXTSTEP_API_KEY: "old-key" },
    });
    await runExecutor({
      ...baseConfig,
      agent: "custom-b",
      env: { NEXTSTEP_API_KEY: "new-key" },
    });
    ...
    expect(wrapper).not.toContain("NEXTSTEP_API_KEY");
    expect(wrapper).not.toContain("new-key");
    expect(wrapper).not.toContain("old-key");
    expect(env).toContain("NEXTSTEP_API_KEY='new-key'");
  ...
  it("shapes ACPX wrapper workspace env for remote execution identities", async () => {
    ...
    expect(env).toContain("NEXTSTEP_WORKSPACE_CWD='/remote/workspace'");
    expect(env).not.toContain("NEXTSTEP_WORKSPACE_WORKTREE_PATH=");
  });
  ...
  it("keeps distinct wrapper env files for concurrent runs with different credentials", async () => {
    ...
    await runExecutor({
      ...baseConfig,
      env: { NEXTSTEP_API_KEY: "first-key" },
    });
    await runExecutor({
      ...baseConfig,
      env: { NEXTSTEP_API_KEY: "second-key" },
    });
    ...
    expect(envFiles.filter((contents) => contents.includes("NEXTSTEP_API_KEY='first-key'"))).toHaveLength(1);
    expect(envFiles.filter((contents) => contents.includes("NEXTSTEP_API_KEY='second-key'"))).toHaveLength(1);
  });

  it("passes Nextstep env through the ACP agent wrapper instead of process.env", async () => {
      ...
      createRuntime: () => ({
        ...
        startTurn: () => ({
          events: (async function* () {
            ...
            observedApiKeyDuringStream = process.env.NEXTSTEP_API_KEY;
            yield { type: "done", stopReason: "end_turn" };
          })(),
          result: Promise.resolve({ status: "completed", stopReason: "end_turn" }),
          cancel: async () => {},
    ...
    const previousApiKey = process.env.NEXTSTEP_API_KEY;
    try {
      delete process.env.NEXTSTEP_API_KEY;
      const result = await execute({
        runId: "run-1",
        agent: {
          id: "agent-1",
      ...
      if (previousApiKey === undefined) delete process.env.NEXTSTEP_API_KEY;
      else process.env.NEXTSTEP_API_KEY = previousApiKey;
    }
  });
});
