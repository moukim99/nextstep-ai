...
import { ensureNextstepSkillSymlink, type NextstepSkillEntry } from "@nextstepai/adapter-utils/server-utils";

const DEFAULT_NEXTSTEP_INSTANCE_ID = "default";

type SkillEntry = NextstepSkillEntry;
...
async function buildClaudePromptBundleKey(input: {
  ...
  hash.update("Nextstep-claude-prompt-bundle:v1\n");
...
export async function prepareClaudePromptBundle(input: {
  ...
  for (const entry of skills) {
    ...
    try {
      await ensureNextstepSkillSymlink(entry.source, target);
    } catch (err) {
      await onLog(
        "stderr",
        `[Nextstep] Failed to materialize Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
