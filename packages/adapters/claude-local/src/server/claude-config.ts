...
export function resolveManagedClaudeConfigSeedDir(
  ...
  return companyId
    ? path.resolve(NextstepHome, "instances", instanceId, "companies", companyId, "claude-config-seed")
    : path.resolve(NextstepHome, "instances", instanceId, "claude-config-seed");
}
...
export async function prepareClaudeConfigSeed(
  ...
  if (copiedFiles.length > 0) {
    await onLog(
      "stdout",
      `[Nextstep] Prepared Claude config seed "${targetDir}" from "${sourceDir}" (${copiedFiles.map((file) => file.name).join(", ")}).\n`,
    );
  } else {
    await onLog(
      "stdout",
      `[Nextstep] No local Claude config seed files were found in "${sourceDir}". Remote Claude auth may still require login.\n`,
    );
  }
