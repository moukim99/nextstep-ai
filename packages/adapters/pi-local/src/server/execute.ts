...
const NEXTSTEP_SESSIONS_DIR = path.join(os.homedir(), ".pi", "Nextsteps");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[Nextstep] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[Nextstep] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[Nextstep] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildPiSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-pi-skills-"));
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(NEXTSTEP_SESSIONS_DIR, { recursive: true });
  return NEXTSTEP_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(NEXTSTEP_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const piSkillEntries = await readNextstepRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveNextstepDesiredSkillNames(config, piSkillEntries);
  if (!executionTargetIsRemote) {
  ...
  // invoke skill binaries (e.g. `Nextstep-get-issue`) by name. Without this,
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[Nextstep] Syncing workspace and Pi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (runtimeSessionId && !canResumeSession) {
    const staleSessionCwdNote =
      savedSessionCwd !== null && !sessionHeaderCwdMatches
        ? ` Pi stored cwd "${savedSessionCwd}" in the session header, so Nextstep will start a fresh session for "${effectiveExecutionCwd}".`
        : "";
    await onLog(
      "stdout",
      executionTargetIsRemote
        ? `[Nextstep] Pi session "${runtimeSessionId}" does not match the current remote execution state and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote} Starting a fresh remote session.\n`
        : `[Nextstep] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote}\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[Nextstep] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const renderedBootstrapPrompt =
    !canResumeSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderNextstepWakePrompt(context.NextstepWake, { resumedSession: canResumeSession });
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[Nextstep] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
      );
