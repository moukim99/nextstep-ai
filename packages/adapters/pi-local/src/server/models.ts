...
import { asString, runChildProcess } from "@nextstepai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.NEXTSTEP_PI_COMMAND === "string" &&
    process.env.NEXTSTEP_PI_COMMAND.trim().length > 0
      ? process.env.NEXTSTEP_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["NEXTSTEP_", "npm_", "NPM_"] as const;
