...
Use when:
- You want Nextstep to run Pi (the AI coding agent) locally as the agent runtime
...
- Nextstep requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/Nextsteps/ and resumed with --session.
