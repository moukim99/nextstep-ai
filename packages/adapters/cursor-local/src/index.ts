...
Use when:
- You want Nextstep to run Cursor Agent CLI locally as the agent runtime
...
- Nextstep auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$Nextstep" and related skills on local runs.
- Nextstep auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
