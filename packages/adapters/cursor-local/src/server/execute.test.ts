...
vi.mock("@nextstepai/adapter-utils/execution-target", async () => {
  ...
  return {
    ...actual,
    startAdapterExecutionTargetNextstepBridge: async () => null,
  };
});
...
describe("cursor execute", () => {
  it("installs the default agent command on a fresh sandbox lease before execution", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-cursor-fresh-lease-"));
    ...
    try {
      ...
      expect(prompt).toContain("Follow the Nextstep heartbeat.");
    } finally {
  ...
  it("reruns sandbox command resolution after managed runtime setup and keeps the original sandbox home", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-cursor-fresh-lease-managed-"));
