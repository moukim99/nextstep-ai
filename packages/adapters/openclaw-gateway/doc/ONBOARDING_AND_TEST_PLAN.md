...
## Scope
This plan is now **gateway-only**. Nextstep supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Nextstep skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Nextstep task.
...
```bash
pnpm --filter @nextstepai/server test:run -- openclaw-gateway
pnpm --filter @nextstepai/server typecheck
pnpm --filter @nextstepai/ui typecheck
pnpm --filter nextstepai typecheck
```
