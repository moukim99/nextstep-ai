...
const DEFAULT_CLIENT_VERSION = "Nextstep";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "Nextstep";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`Nextstep:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`Nextstep:issue:${input.issueId}`, input.agentId);
  }
...
function resolveNextstepApiUrlOverride(value: unknown): string | null {
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/Nextstep-claimed-api-key.json";

function resolveClaimedApiKeyPath(value: unknown): string {
  return nonEmpty(value) ?? DEFAULT_CLAIMED_API_KEY_PATH;
}

function buildNextstepEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const NextstepApiUrlOverride = resolveNextstepApiUrlOverride(ctx.config.NextstepApiUrl);
  const NextstepEnv: Record<string, string> = {
    ...buildNextstepEnv(ctx.agent),
    NEXTSTEP_RUN_ID: ctx.runId,
  };

  if (NextstepApiUrlOverride) {
    NextstepEnv.NEXTSTEP_API_URL = NextstepApiUrlOverride;
  }
  if (wakePayload.taskId) NextstepEnv.NEXTSTEP_TASK_ID = wakePayload.taskId;
  const issueWorkMode = readNextstepIssueWorkModeFromContext(ctx.context);
  if (issueWorkMode) NextstepEnv.NEXTSTEP_ISSUE_WORK_MODE = issueWorkMode;
  if (wakePayload.wakeReason) NextstepEnv.NEXTSTEP_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) NextstepEnv.NEXTSTEP_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) NextstepEnv.NEXTSTEP_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) NextstepEnv.NEXTSTEP_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    NextstepEnv.NEXTSTEP_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  payload: WakePayload,
  NextstepEnv: Record<string, string>,
  structuredWakePrompt: string,
): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/Nextstep-claimed-api-key.json";
  const orderedKeys = [
    "NEXTSTEP_RUN_ID",
    "NEXTSTEP_AGENT_ID",
    "NEXTSTEP_COMPANY_ID",
    "NEXTSTEP_API_URL",
    "NEXTSTEP_TASK_ID",
    "NEXTSTEP_WAKE_REASON",
    "NEXTSTEP_WAKE_COMMENT_ID",
    "NEXTSTEP_APPROVAL_ID",
    "NEXTSTEP_APPROVAL_STATUS",
    "NEXTSTEP_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = NextstepEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = NextstepEnv.NEXTSTEP_API_URL ?? "<set NEXTSTEP_API_URL>";

  const lines = [
    "Nextstep wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `NEXTSTEP_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load NEXTSTEP_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $NEXTSTEP_API_KEY on every API call.",
    "- Use X-Nextstep-Run-Id: $NEXTSTEP_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: NEXTSTEP_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$NEXTSTEP_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$NEXTSTEP_COMPANY_ID/issues?assigneeAgentId=$NEXTSTEP_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardNextstepPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  NextstepEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateNextstep = parseObject(payloadTemplate.Nextstep);
  const workspace = asRecord(ctx.context.NextstepWorkspace);
  const workspaces = Array.isArray(ctx.context.NextstepWorkspaces)
    ? ctx.context.NextstepWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.NextstepRuntimeServiceIntents)
    ? ctx.context.NextstepRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardNextstep: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: NextstepEnv.NEXTSTEP_API_URL ?? null,
  };
  const structuredWake = parseObject(ctx.context.NextstepWake);
  if (Object.keys(structuredWake).length > 0) {
    standardNextstep.wake = structuredWake;
  }

  if (workspace) {
    standardNextstep.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardNextstep.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardNextstep.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "Nextstep-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const NextstepEnv = buildNextstepEnvForWake(ctx, wakePayload);
  const structuredWakePrompt = renderNextstepWakePrompt(ctx.context.NextstepWake);
  const structuredWakeJson = stringifyNextstepWakePayload(ctx.context.NextstepWake);
  ...
  const NextstepPayload = buildStandardNextstepPayload(ctx, wakePayload, NextstepEnv, payloadTemplate);
  ...
  agentParams.Nextstep = NextstepPayload;
