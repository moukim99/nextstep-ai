...
import { asString, parseObject } from "@nextstepai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Nextstep server host.",
        });
      }
    } catch (err) {
      checks.push({
