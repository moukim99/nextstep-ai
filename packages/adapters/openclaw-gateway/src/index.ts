...
Use when:
- You want Nextstep to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Nextstep server.
...
- NextstepApiUrl (string, optional): absolute Nextstep base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/Nextstep-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default Nextstep)

Standard outbound payload additions:
- Nextstep (object): standardized Nextstep context added to every gateway agent request
- Nextstep.workspace (object, optional): resolved execution workspace for this run
- Nextstep.workspaces (array, optional): additional workspace hints Nextstep exposed to the run
- Nextstep.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
