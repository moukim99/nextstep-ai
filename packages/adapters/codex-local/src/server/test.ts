...
async function prepareCodexHelloProbe(input: {
  ...
  if (input.targetIsRemote && !input.probeApiKey) {
    ...
    preparedRuntimeWorkspaceLocalDir = await fs.mkdtemp(
      path.join(os.tmpdir(), `Nextstep-codex-envtest-${input.runId}-`),
    );
    preparedRuntime = await prepareAdapterExecutionTargetRuntime({
      ...
      // `.Nextstep-runtime/runs/<runId>/workspace` to whatever it gets, so
  ...
  if (input.probeApiKey) {
    const probeHome = input.targetIsRemote
      ? `/tmp/Nextstep-codex-probe-${input.runId}`
      : path.join(os.tmpdir(), `Nextstep-codex-probe-${input.runId}`);
    return {
      command: "sh",
      args: [
        "-c",
        'set -e; mkdir -p "$CODEX_HOME"; umask 077; printf "%s" "$_NEXTSTEP_CODEX_AUTH_JSON" > "$CODEX_HOME/auth.json"; unset _NEXTSTEP_CODEX_AUTH_JSON; trap \'rm -rf "$CODEX_HOME"\' EXIT INT TERM; "$0" "$@"',
        input.command,
        ...input.args,
      ],
      env: {
        ...
        _NEXTSTEP_CODEX_AUTH_JSON: JSON.stringify({ OPENAI_API_KEY: input.probeApiKey }),
      },
      cleanup,
    };
  }
        ...
        if (probe.timedOut) {
          ...
          checks.push({
            ...
            hint: probeApiKey
              ...
              : "Codex CLI does not read OPENAI_API_KEY from the environment; set OPENAI_API_KEY in this adapter's config (so Nextstep writes it to `$CODEX_HOME/auth.json`) or run `codex login` on the host first.",
          });
        } else {
          checks.push({
