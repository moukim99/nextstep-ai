...
const {
  ...
  return {
    ...
    prepareAdapterExecutionTargetRuntime: vi.fn(async () => ({
      target: null,
      workspaceRemoteDir: "/remote/workspace/.Nextstep-runtime/runs/test/workspace",
      runtimeRootDir: "/remote/workspace/.Nextstep-runtime/runs/test/workspace/.Nextstep-runtime/codex",
      assetDirs: {
        home: "/remote/workspace/.Nextstep-runtime/runs/test/workspace/.Nextstep-runtime/codex/home",
      },
      restoreWorkspace,
    })),
    prepareManagedCodexHome: vi.fn(async () => "/tmp/Nextstep-managed-codex-home"),
    restoreWorkspace,
  };
});
...
describe("codex remote environment diagnostics", () => {
  ...
  it("stages managed CODEX_HOME in an isolated runtime dir and keeps the probe cwd on the original remote workspace", async () => {
    ...
    expect(runtimeInput?.workspaceLocalDir).toContain(`${os.tmpdir()}/Nextstep-codex-envtest-`);
    ...
    expect(probeCall?.[4]).toMatchObject({
      ...
      env: expect.objectContaining({
        CODEX_HOME: "/remote/workspace/.Nextstep-runtime/runs/test/workspace/.Nextstep-runtime/codex/home",
      }),
    });
