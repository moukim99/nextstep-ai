...
const DEFAULT_NEXTSTEP_INSTANCE_ID = "default";
...
function isWorktreeMode(env: NodeJS.ProcessEnv): boolean {
  return TRUTHY_ENV_RE.test(env.NEXTSTEP_IN_WORKTREE ?? "");
}
...
export function resolveManagedCodexHomeDir(
  ...
  const NextstepHome = nonEmpty(env.NEXTSTEP_HOME) ?? path.resolve(os.homedir(), ".Nextstep");
  const instanceId = nonEmpty(env.NEXTSTEP_INSTANCE_ID) ?? DEFAULT_NEXTSTEP_INSTANCE_ID;
  return companyId
    ? path.resolve(NextstepHome, "instances", instanceId, "companies", companyId, "codex-home")
    : path.resolve(NextstepHome, "instances", instanceId, "codex-home");
}
...
export async function prepareManagedCodexHome(
  ...
  if (seedFromShared) {
    ...
    await onLog(
      "stdout",
      `[Nextstep] Using ${isWorktreeMode(env) ? "worktree-isolated" : "Nextstep-managed"} Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
    );
  }
  ...
  if (apiKey) {
    ...
    await onLog(
      "stdout",
      `[Nextstep] Wrote API-key auth.json into Codex home "${targetHome}" from configured OPENAI_API_KEY.\n`,
    );
  }
