...
async function buildOpenCodeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildPersistentSkillSnapshot({
    ...
    externalDetail: "Installed outside Nextstep management in the shared skills home.",
    warnings: [
...
export async function syncOpenCodeSkills(
  ...
  const availableEntries = await readNextstepRuntimeSkillEntries(ctx.config, __moduleDir);
  ...
  for (const available of availableEntries) {
    ...
    await ensureNextstepSkillSymlink(available.source, target);
  }
