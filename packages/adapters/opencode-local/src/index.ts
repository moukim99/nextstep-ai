...
Use when:
- You want Nextstep to run OpenCode locally as the agent runtime
...
- dangerouslySkipPermissions (boolean, optional): inject a runtime OpenCode config that allows \`external_directory\` access without interactive prompts; defaults to true for unattended Nextstep runs
...
- Nextstep requires an explicit \`model\` value for \`opencode_local\` agents.
...
- When \`dangerouslySkipPermissions\` is enabled, Nextstep injects a temporary \
