...
For `claude_local` and `codex_local`, Nextstep assumes the CLI is already installed and authenticated on the host machine.
...
Nextstep stores resumable session state per `(agent, taskKey, adapterType)`.
