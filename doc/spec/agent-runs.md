...
This spec defines how Nextstep actually runs agents while staying runtime-agnostic.
...
1. Nextstep is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Nextstep manages lifecycle, and on exit Nextstep parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `Nextstep.skill` (shared Nextstep skill text block)
...
2. Preferred transport is env vars (`NEXTSTEP_*`) injected at runtime.
