...
This document covers the GitHub and npm setup required for the current Nextstep release model:
...
Do this for every public package that Nextstep publishes.
...
3. add a trusted publisher for the GitHub repository `nextstepai/nextstep`
...
2. open that package on npm and add the `nextstepai/nextstep` trusted publisher for `.github/workflows/release.yml`
...
2. revoke any personal automation token that used to publish Nextstep
