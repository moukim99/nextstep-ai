...
Low-level reference for how Nextstep packages are prepared and published to npm.
...
Nextstep no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `nextstepai`, imports code from workspace packages such as:
...
### Manual first publish for `@nextstepai/ui`
...
npm view @nextstepai/ui version

# make sure the dist payload is fresh
pnpm --filter @nextstepai/ui build
...
- If `npm view @nextstepai/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@nextstepai/ui`, the `nextstepai` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Nextstep uses calendar versions:
...
- `nextstepai@2026.318.1-canary.2`

This keeps the default install path unchanged while allowing explicit installs with:

```bash
npx nextstepai@canary onboard
```
...
- every published internal `@nextstepai/*` dependency referenced by that manifest exists on npm
...
Nextstep does not auto-publish every non-private workspace package anymore.
...
Example for `@nextstepai/adapter-acpx-local` from the repo root:

```bash
# safe preview
pnpm run release:bootstrap-package -- @nextstepai/adapter-acpx-local

# one-time first publish from an authenticated maintainer machine
pnpm run release:bootstrap-package -- @nextstepai/adapter-acpx-local --publish --otp 123456
```
...
1. open `https://www.npmjs.com/package/@nextstepai/adapter-acpx-local`
2. go to `Settings` → `Trusted publishing`
3. add repository `nextstepai/nextstep`
