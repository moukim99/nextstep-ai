...
This guide describes the current, implemented way to create a Nextstep plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Nextstep app.
...
- The host provides a small shared React component kit through
  `@nextstepai/plugin-sdk/ui`; use it for common Nextstep controls before
...
```bash
pnpm --filter @nextstepai/create-Nextstep-plugin build
node packages/plugins/create-Nextstep-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Nextstep repo:

```bash
pnpm --filter @nextstepai/create-Nextstep-plugin build
node packages/plugins/create-Nextstep-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@nextstepai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@nextstepai/plugin-sdk` from the local Nextstep checkout into a `.Nextstep-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Nextstep from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
## Managed Nextstep resources

Plugins that provide durable Nextstep business objects should declare them in
...
- Managed agents: declare top-level `agents[]` and require
  ...
  normal Nextstep agents with plugin ownership metadata, not background plugin
  workers.
...
- Managed routines: declare top-level `routines[]` and require
  ...
  jobs that should create visible Nextstep issues. Prefer managed routines over
  ...
  agents: [
    {
      ...
      instructions: {
        content: "Follow the Nextstep heartbeat and produce concise research briefs.",
      },
    },
  ],
  projects: [
...
```ts
import { definePlugin } from "@nextstepai/plugin-sdk";
...
- Use managed routines for recurring or externally triggered work that should
  ...
  triggers, and each run has the normal Nextstep issue/audit trail.
...
- Keep defaults conservative. Managed declarations are suggestions owned by the
  plugin, but the resulting resources are normal Nextstep records that the
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@nextstepai/plugin-sdk/ui`
...
Use shared components from `@nextstepai/plugin-sdk/ui` when the plugin needs a
Nextstep-native control. The host owns the implementation, so plugins inherit
...
Use `FileTree` from `@nextstepai/plugin-sdk/ui` whenever the plugin only needs
