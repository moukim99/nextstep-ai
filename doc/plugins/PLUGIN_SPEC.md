# Nextstep Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Nextstep's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Nextstep app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Nextstep HTTP APIs directly.
...
- The current runtime ships a small host-provided plugin UI component kit through `@nextstepai/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
Nextstep plugin design is based on the following assumptions:

1. Nextstep is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Nextstep business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Nextstep core.
...
2. Let plugins add major capabilities without editing Nextstep core.
...
4. Depend on project-local plugin folders such as `.nextstep/plugins`.
...
The single Nextstep deployment an operator installs and controls.

### 5.2 Company

A first-class Nextstep business object inside the instance.
...
A trusted in-process extension loaded directly by Nextstep core.
...
An installable instance-wide extension package loaded through the Nextstep plugin runtime.
...
Nextstep has two extension classes.
...
Nextstep already has a concrete workspace model:
...
Plugins live under the Nextstep instance directory.

Suggested layout:

- `~/.nextstep/instances/default/plugins/package.json`
- `~/.nextstep/instances/default/plugins/node_modules/`
- `~/.nextstep/instances/default/plugins/.cache/`
- `~/.nextstep/instances/default/data/plugins/<plugin-id>/`
...
Nextstep should add CLI commands:

- `pnpm nextstepai plugin list`
- `pnpm nextstepai plugin install <package[@version]>`
- `pnpm nextstepai plugin uninstall <plugin-id>`
- `pnpm nextstepai plugin upgrade <plugin-id> [version]`
- `pnpm nextstepai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@nextstep/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface NextstepPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Nextstep agents can use during runs.
...
- Nextstep server starts one worker process per installed plugin
...
Receives one typed Nextstep domain event.
...
export function definePlugin(definition: PluginDefinition): NextstepPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: NextstepPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@nextstepai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@nextstepai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@nextstepai/plugin-sdk/ui`)
...
Nextstep-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
Nextstep navigation because those can force a full document reload.
...
- Plugin bundles must not import from host internals. They may only import from `@nextstepai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@nextstepai/plugin-sdk/ui`
...
The `@nextstepai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Nextstep secret provider system rather than a plain text input
...
1. Core Nextstep data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Nextstep objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Nextstep product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Nextstep later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Nextstep secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm nextstepai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Nextstep server. This is a normative requirement, not optional.
...
### 27.1 `@nextstepai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@nextstepai/plugin-test-harness";
...
For developing a plugin against a running Nextstep instance:

- The operator installs the plugin from a local path: `pnpm nextstepai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-Nextstep-plugin`) that scaffolds:

- `package.json` with correct `NextstepPlugin` keys
...
- `@nextstep/plugin-workspace-files`
- `@nextstep/plugin-terminal`
- `@nextstep/plugin-git`
- `@nextstep/plugin-linear`
- `@nextstep/plugin-github-issues`
- `@nextstep/plugin-grafana`
- `@nextstep/plugin-runtime-processes`
- `@nextstep/plugin-stripe`
...
- `@nextstepai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@nextstepai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@nextstepai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@nextstepai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@nextstepai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@nextstepai/plugin-sdk/ui`
...
- `@nextstepai/plugin-test-harness`
- `create-Nextstep-plugin` starter template
...
Nextstep should not implement a generic in-process hook bag modeled directly after local coding tools.

Nextstep should implement:
...
That is the complete target design for the Nextstep plugin system.
