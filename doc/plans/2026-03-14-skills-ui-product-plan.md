...
This document defines the product and UI plan for skill management in Nextstep.
...
- Nextstep company import/export can include skills as package content
...
3. The UI must always tell the truth about what Nextstep knows:
   - desired state in Nextstep
...
Nextstep should treat skills at two scopes:
...
- created locally inside Nextstep later
...
7. “Understand what Nextstep knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Nextstep-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Nextstep did not install.

These should be clearly marked:

- external
- not managed by Nextstep
...
- `desired_only`: Nextstep wants it, adapter does not show it yet
- `external`: adapter has it but Nextstep does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Nextstep only tracks desired state
...
- Nextstep can still track desired skills
...
- Nextstep can see adapter skills
...
- `.Nextstep.yaml` may add Nextstep-specific fidelity, but should not replace the base shortname association model
...
- `.Nextstep.yaml` may add Nextstep-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Nextstep should have a company-scoped skill model or managed package model representing:
...
Nextstep should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Nextstep-specific.
...
That gives Nextstep one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
