...
Nextstep's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Nextstep's API, UI, and data model | Yes |
...
modules/
  observability/
    Nextstep.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@nextstepai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@nextstepai/core";
...
```
1. Scan modules/ directory for Nextstep.module.json manifests
...
```jsonc
// nextstep.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@nextstepai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@nextstepai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:Nextstep/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.Nextstep.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm nextstepai store list                    # browse available modules and templates
pnpm nextstepai store install <module-id>     # install a module
pnpm nextstepai store import <template-id>    # import a company template
pnpm nextstepai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Nextstep instance | `server:started` |
...
4. **Module config** — `nextstep.config.json` with per-module config, env var interpolation
...
10. **`@nextstepai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
