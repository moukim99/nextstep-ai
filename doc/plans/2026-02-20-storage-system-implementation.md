...
Add a single storage subsystem for Nextstep that supports:
...
- API serves attachment bytes through authenticated Nextstep endpoints.
...
- Default local path is under instance root: `~/.nextstep/instances/<instanceId>/data/storage`.
...
- `nextstepai onboard` writes a valid `storage` config block by default.
- `nextstepai configure --section storage` can switch between local and s3 modes.
...
- `nextstepai doctor` reports actionable storage status.
