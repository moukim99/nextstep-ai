...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Nextstep.
...
- current Nextstep plugin and adapter surfaces in this repo
...
## What Nextstep already has

Nextstep already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Nextstep

These ideas from `pi-mono` fit Nextstep with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Nextstep should continue exposing run and transcript events to plugins, for example:
...
This matches Nextstep's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Nextstep already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Nextstep already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Nextstep, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Nextstep core:

### 1. Arbitrary mutation hooks on control-plane decisions

Nextstep should not let general plugins rewrite:
...
Nextstep should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Nextstep is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Nextstep's board-and-issues model.

## Recommended Nextstep direction
...
If the question is "what works for Nextstep too?":
