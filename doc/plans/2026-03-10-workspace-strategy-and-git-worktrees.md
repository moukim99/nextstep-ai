...
`PAP-447` asks how Nextstep should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Nextstep instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Nextstep:

- some operators use Nextstep to manage Nextstep and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Nextstep
- Claude and Codex expose different built-in affordances, so Nextstep should not overfit to one tool

## Core Product Decision

Nextstep should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Nextstep
...
- Nextstep should not assume that every adapter can see or use a host filesystem path directly
...
If Nextstep makes worktrees agent-first, it will blur:
...
- Nextstep core resolves and records execution workspace state
...
That gives Nextstep room to support:
...
- Nextstep handles workspace runtime services under the hood using project/workspace policy
...
Once Nextstep is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Nextstep should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Nextstep cannot infer
...
- when issue moves to done, should Nextstep auto-commit?
- should it auto-push?
...
Nextstep already has the right foundation for a project-first model.
...
For server-spawned local adapters, Nextstep already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Nextstep should mirror directly

Implication:

- for `codex_local`, Nextstep should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Nextstep should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Nextstep should separate:
...
Nextstep should be able to express intentions such as:
...
These are not specific to Nextstep. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".nextstep/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Nextstep-specific `devServer` flag.
...
Nextstep should distinguish between:
...
Nextstep should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Nextstep should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Nextstep
...
For runtime services specifically, Nextstep should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Nextstep-only dev-server flags
...
3. Inject resolved execution workspace details into `context.NextstepWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Nextstep instance can be reused through env/config injection when appropriate
...
- Nextstep can decide whether to reuse or start a fresh service deterministically
...
3. Test against Nextstep-on-Nextstep workflows first.
...
7. Workspace runtime services are modeled generically, not as Nextstep-specific dev-server toggles.
