...
Turn the `agent-os` research into a low-risk Nextstep execution plan that preserves Nextstep's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Nextstep should not absorb `agent-os` as a product model or orchestration layer.

Nextstep should evaluate `agent-os` in three narrow areas:
...
This work is successful when Nextstep has:
...
- a proposed runtime capability model that fits current Nextstep adapters
...
- replace Nextstep heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Nextstep core

## Existing Nextstep integration points
...
- prove that Nextstep can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Nextstep's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Nextstep-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Nextstep workloads
...
- identify where Nextstep should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Nextstep
...
- runtime experimentation could blur the boundary between Nextstep as control plane and Nextstep as execution platform
