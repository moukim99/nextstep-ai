...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/Nextstep-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `NEXTSTEP_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `NEXTSTEP_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Nextstep may create host-local worktrees, processes, and paths
- `cloud`
  - Nextstep should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `Nextstep | github | vercel | s3 | custom`
...
- use `NEXTSTEP_EXECUTION_TOPOLOGY` as a defaulting guardrail
