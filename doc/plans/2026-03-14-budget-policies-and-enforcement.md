...
Nextstep already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Nextstep should implement next.

## Product Goals

Nextstep should let operators:
...
For the next implementation pass, Nextstep should enforce these defaults:
...
Nextstep should separate subscription-included usage from billed spend:
...
Nextstep should have two threshold classes:
...
Nextstep must know that a project is budget-paused and enforce it.
...
Nextstep must also block execution before new work starts.
