...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Nextstep plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Nextstep.
...
- package: `@nextstepai/plugin-kitchen-sink-example`
- plugin id: `Nextstep.kitchen-sink-example` or `Nextstep-kitchen-sink-example`

Recommendation: use `Nextstep-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Nextstep event such as `issue.created`
...
Recommendation: default to a Nextstep-local endpoint or a stable public echo endpoint to avoid flaky docs.
