...
Yes, there is a likely root cause in Nextstep's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Nextstep can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Nextstep already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Nextstep may think it cancelled the run because the parent process is gone
...
This work is successful when Nextstep can:
...
- require every skill author to add bespoke browser teardown logic before Nextstep can clean up correctly
...
- make the leak measurable from Nextstep's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Nextstep
...
Treat this as a Nextstep executor ownership bug, not an `agent-browser` bug.
