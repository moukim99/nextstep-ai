...
Nextstep should not replace its workspace runtime service model with VS Code tasks.
...
- Nextstep owns long-running workspace services and their desired state
- Nextstep shows operators exactly which named thing they are starting or stopping
- Nextstep distinguishes long-running services from one-shot jobs
...
This is directionally correct for Nextstep because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Nextstep does not yet leverage existing workspace metadata.

## Recommendation

Keep Nextstep runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Nextstep.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Nextstep service or job semantics should become runnable in Nextstep.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Nextstep runtime services.
...
That creates a bad fit if Nextstep tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Nextstep's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Nextstep should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Nextstep can resolve
...
- selected variable substitution when Nextstep can resolve it safely from workspace context
- mapping task metadata into Nextstep readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Nextstep cannot execute directly
...
Nextstep needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Nextstep metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Nextstep
...
For a workspace with `cwd`, Nextstep should look for `.vscode/tasks.json`.
...
Nextstep should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Nextstep metadata marks it as a service, or
...
Nextstep-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Nextstep would import and why.
...
Goal:
run supported imported service commands through the current Nextstep supervisor.
...
Success condition:
imported service commands behave like native Nextstep services once adopted.
...
- removal of Nextstep's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Nextstep to VS Code and making the model worse for non-VS-Code repos
...
Nextstep should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Nextstep workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
