...
Nextstep needs to support two very different but equally valid ways of working:
...
Today, Nextstep already has the beginnings of this model:
...
- making Nextstep feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Nextstep users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Nextstep deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Nextstep into a full code review product
...
This object must be recorded explicitly so that Nextstep can:
...
Nextstep should treat PRs as a type of work product linked back to:
...
Nextstep already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Nextstep must be a first-class deployment mode

Nextstep cannot assume that it is running on the same machine as the code.

In cloud deployments, Nextstep may:
...
- `Runtime service`: a process or service Nextstep owns or tracks for a workspace
...
It also lets cloud-hosted Nextstep deployments point at codebases and remotes without pretending the Nextstep host has direct filesystem access.
...
Without an explicit `execution workspace` record, Nextstep has nowhere stable to attach:
...
Nextstep needs a single place to show:
...
- Nextstep-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Nextstep owning the execution host
...
## 1. Nextstep host and execution host must be decoupled
...
without the Nextstep host itself running local git or local preview processes.
...
Nextstep should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Nextstep.
...
Nextstep should keep workspace automation centered on a small set of hooks:
...
Nextstep should provide the equivalent visibility in the project `Code` surface:
...
Nextstep should treat execution workspaces as the natural home for:
...
Nextstep should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Nextstep should continue treating cleanup policy as part of the core workspace model:
...
Nextstep should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Nextstep host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Nextstep should borrow Worktrunk's good execution patterns while keeping the broader Nextstep model:
...
That keeps Nextstep focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Nextstep deployments remain first-class

That is a better fit for Nextstep than either extreme:
