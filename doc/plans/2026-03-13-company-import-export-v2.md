...
This document defines the next-stage plan for Nextstep company import/export.

The core shift is:

- move from a Nextstep-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Nextstep.
...
Nextstep already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Nextstep
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `Nextstep.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.Nextstep.yaml` sidecar for high-fidelity Nextstep-specific details
...
11. adapter-aware skill sync surfaces so Nextstep can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Nextstep at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Nextstep sidecar.
...
Nextstep must not redefine `SKILL.md`.
...
- Nextstep-specific fidelity lives in `.Nextstep.yaml`
- Nextstep may resolve and install `SKILL.md` packages, but it must not require a Nextstep-only skill format
...
### 5.4 Base Package Vs Nextstep Extension
...
- base package:
  ...
  - no Nextstep-only runtime fields by default
- Nextstep extension:
  - `.Nextstep.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Nextstep tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`Nextstep.manifest.json` is not part of the future package direction.
...
Nextstep import/export should support these entity kinds:
...
- Nextstep can treat “this set of agents came from team package X” as the imported-team model
...
Nextstep should support source references in package metadata with:
...
Nextstep should define a new adapter surface area around skills:
...
Planned Nextstep behavior:

- if an adapter supports read, Nextstep should show current skills in the UI
- if an adapter supports write, Nextstep should let the user enable/disable imported skills
- if an adapter supports sync, Nextstep should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.Nextstep.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.Nextstep.yaml` alongside the base package
...
## 10. Storage Model Inside Nextstep
...
Nextstep should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `nextstepai company export <company-id> --out <path>`
- `nextstepai company import <path-or-url> --dry-run`
- `nextstepai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Nextstep storage, or only referenced at import time?
...
3. Should Nextstep support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Nextstep aligned with:
