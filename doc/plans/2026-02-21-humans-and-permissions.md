...
- local mode that still feels instant (`npx nextstepai run` and go)
...
2. CLI command `pnpm nextstepai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm nextstepai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm nextstepai onboard`).
...
- expose key instance settings in API + CLI (`nextstepai configure` / `nextstepai onboard`)
...
7. `pnpm nextstepai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
