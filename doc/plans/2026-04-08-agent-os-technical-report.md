# Agent OS Technical Report for Nextstep
...
`agent-os` is not a competitor to Nextstep's core product. It is an execution substrate: an embedded, VM-like runtime for agents, tools, filesystems, and session orchestration. Nextstep is a control plane: company scoping, task hierarchy, approvals, budgets, activity logs, workspaces, and governance.

The strongest takeaway is not "copy agent-os wholesale." The strongest takeaway is that Nextstep could selectively use its runtime ideas to improve local agent execution safety, reproducibility, and portability while keeping all company/task/governance logic in Nextstep.

My recommendation is:

1. Do not merge agent-os concepts into the Nextstep core product model.
...
4. Do not import its workflow/cron/queue abstractions into Nextstep core until they are reconciled with Nextstep's issue/comment/governance model.
...
That gives `agent-os` a much stronger isolation story than Nextstep's current "launch a host CLI in a workspace" local adapter approach.
...
This is directly relevant to Nextstep because our adapter layer still normalizes each CLI agent in a fairly bespoke way.

## Nextstep anchor points

The most relevant current Nextstep surfaces for any future `agent-os` integration are:
...
## What Nextstep can learn from it

### 1. A safer local execution substrate

Nextstep's local adapters currently run host CLIs in managed workspaces and rely on adapter-specific behavior plus process-level controls. That is pragmatic, but weakly isolated.
...
This fits Nextstep because it improves execution safety without changing the control-plane model.

### 2. Snapshotted execution roots instead of only mutable workspaces

Nextstep already has strong execution-workspace concepts, but they are repo/worktree-centric. `agent-os` adds a stronger "start from known lower layers, write into a disposable upper layer" model.
...
Nextstep has governance at the company/task level:
...
It has less structure at the runtime capability level. `agent-os` offers a clear vocabulary that Nextstep could adopt even without adopting the runtime itself:
...
Nextstep's plugin system and adapters already have the beginnings of a controlled extension surface. `agent-os` reinforces the value of exposing capabilities as typed tools rather than raw shell access.

Concrete Nextstep uses:
...
This aligns well with Nextstep's governance story.

### 5. Better adapter normalization around sessions and capabilities

Nextstep's adapter contract already supports execution results, session params, environment tests, skill syncing, quota windows, and optional `detectModel()`. But much of the per-agent behavior is still adapter-specific.
...
Nextstep does not need ACP everywhere, but it would benefit from a more formal internal session capability model inspired by this.
...
Nextstep can adopt that philosophy directly:
...
## What does not fit Nextstep well

### 1. Its built-in orchestration primitives overlap the wrong layer

`agent-os` includes cron/session/workflow style primitives inside the runtime package. Nextstep already has higher-level orchestration concepts:
...
If Nextstep copied `agent-os` cron/workflow/queue ideas directly into core, we would likely duplicate orchestration across two layers. That would blur ownership and make debugging harder.

Nextstep should keep orchestration authoritative at the control-plane layer.
...
Those are Nextstep's differentiators. They should not be displaced by runtime abstractions.
...
The permission model is good, but it is low-level. Nextstep would still need to answer:
...
- real integration work for Codex/Cursor/Gemini-style Nextstep adapters

So the main near-term value is not universal replacement. It is selective use where compatibility is strong.

## Concrete recommendations for Nextstep
...
- keep Nextstep heartbeat/task/workspace/budget logic unchanged
...
- Nextstep's Codex adapter is already important and carries repo-specific behavior
...
- Nextstep still records logs, summaries, cost metadata, and issue comments normally
...
Even without using `agent-os`, Nextstep should consider standardizing adapter/runtime permissions around a vocabulary like:
...
Nextstep should evaluate whether some execution workspaces can be backed by:
...
Nextstep plugins and adapters should continue moving toward explicit typed tools over ad hoc shell access. `agent-os` confirms that this is the right direction.
...
### Recommendation E: do not import runtime-level workflows into Nextstep core

Nextstep should not copy `agent-os` cron/workflow/queue concepts into core orchestration yet.
...
- moving Nextstep orchestration into agent-os workflows
...
`agent-os` is useful to Nextstep as an execution technology reference, not as a product model.

Nextstep should treat it the same way it treats sandboxes or agent CLIs:
