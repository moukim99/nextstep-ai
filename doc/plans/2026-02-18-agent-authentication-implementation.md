...
- New config/env settings:
  - `NEXTSTEP_AGENT_JWT_SECRET`
  - `NEXTSTEP_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `NEXTSTEP_AGENT_JWT_ISSUER` (default: `Nextstep`)
  - `NEXTSTEP_AGENT_JWT_AUDIENCE` (default: `Nextstep-api`)
...
1. In:
   ...
   inject `NEXTSTEP_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `NEXTSTEP_API_KEY`, do not overwrite it.
- Continue injecting:
  - `NEXTSTEP_AGENT_ID`
  - `NEXTSTEP_COMPANY_ID`
  - `NEXTSTEP_API_URL`
...
- Local adapters authenticate without manual `NEXTSTEP_API_KEY` config.
