...
This document defines the rollout plan for adapter-wide skill support in Nextstep.
...
- Nextstep stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Nextstep currently has these adapters:
...
1. Desired skills live in Nextstep for every adapter.
...
These adapters have a stable local skills directory that Nextstep can read and manage.
...
These adapters do not have a meaningful Nextstep-owned persistent install state.
...
- desired skills stored in Nextstep
...
- runtime already injects Nextstep skills into `~/.cursor/skills`
...
- runtime already injects Nextstep skills into `~/.gemini/skills`
...
- runtime already injects Nextstep skills into `~/.pi/agent/skills`
...
- Nextstep can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Nextstep skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Nextstep may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Nextstep skills that are clearly marked as Nextstep-managed
...
- Nextstep stores desired skills only
...
- avoid destructive wording unless Nextstep can prove a skill is Nextstep-managed
...
That gets Nextstep from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
