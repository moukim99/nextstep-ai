...
This document defines the V1 design for a Nextstep adapter that integrates with
...
4. synthesized stdout events for Nextstep UI/CLI
...
3. **Agent callback auth to Nextstep** uses a bootstrap exchange flow (no long-lived Nextstep key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Nextstep endpoints, not full SKILL.md prompt injection.
...
- `NextstepPublicUrl` (optional override; else `NEXTSTEP_PUBLIC_URL` env)
...
## Nextstep Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `NEXTSTEP_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.NextstepPublicUrl`
2. `process.env.NEXTSTEP_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Nextstep credentials in prompt text.

Flow:

1. Before launch/follow-up, Nextstep mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Nextstep validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Nextstep API calls.
...
1. Prompt includes a compact instruction to fetch skills from Nextstep.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/Nextstep`
   - `GET /api/skills/Nextstep-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <NextstepPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Nextstep run context.
...
5. `NextstepPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Nextstep skill endpoints |
| Nextstep auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Nextstep context.
3. Add richer PR metadata surfacing in Nextstep UI.
