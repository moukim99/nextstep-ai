...
Agents need API keys to authenticate with Nextstep. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Nextstep
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Nextstep generates a token and passes it directly to the agent
...
`NEXTSTEP_API_URL`, `NEXTSTEP_AGENT_ID`, `NEXTSTEP_COMPANY_ID`. We need to
add a `NEXTSTEP_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Nextstep CLI
...
```
Nextstep auth login
# Opens browser -> user confirms -> token stored at ~/.nextstep/credentials
```
...
managed by the Nextstep server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Nextstep.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Nextstep SKILL.md (or a link to it)
   - What information Nextstep needs from the agent (e.g. webhook URL, adapter
...
5. Nextstep stores the pending registration.
...
7. On approval, Nextstep provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Nextstep issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.Nextstep.ing/skills/Nextstep/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Nextstep should send you heartbeats",
...
1. Generate a Nextstep invite link for the company.
...
4. A Nextstep company member approves the new agent.
5. Nextstep begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `NEXTSTEP_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `nextstepai auth login` for developer-managed remote agents.                                      |
