...
Today release smoke testing for published Nextstep packages is manual and shell-driven:
...
- Docker
- `npx nextstepai@canary`
- `npx nextstepai@latest`
...
- runs `nextstepai@${nextstepai_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@Nextstep.local`
- password: `Nextstep-smoke-password`
...
- the suite passes locally against both:
  - `nextstepai_VERSION=canary`
  - `nextstepai_VERSION=latest`
...
1. A published `nextstepai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `nextstepai@latest` Docker install can be smoke-tested by Playwright in CI.
