# Nextstep V1 Implementation Spec
...
Nextstep V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.nextstep/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.nextstep/instances/default/data/storage` (`local_disk`)
...
- `runtime_config` jsonb not null default `{}`; may include Nextstep runtime policy such as `modelProfiles.cheap.adapterConfig` for an optional low-cost model lane that does not change the primary adapter config
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before Nextstep runtime-owned keys are injected
...
- when Nextstep cannot safely infer the next action, it surfaces the problem through visible blocked/recovery work instead of silently completing or reassigning work
...
- if unset, auto-use embedded PostgreSQL under `~/.nextstep/instances/default/db`
...
- `.Nextstep.yaml` sidecar for Nextstep-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.Nextstep.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and Nextstep routine fidelity in `.Nextstep.yaml`
- Nextstep imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.Nextstep.yaml`
