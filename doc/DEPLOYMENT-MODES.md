...
Nextstep supports two runtime modes:
...
Nextstep now treats **bind** as a separate concern from auth:
...
pnpm nextstepai onboard --yes --bind lan
pnpm nextstepai run --bind tailnet
```
...
When running `authenticated` mode, if the only instance admin is `local-board`, Nextstep emits a startup warning with a one-time high-entropy claim URL.
