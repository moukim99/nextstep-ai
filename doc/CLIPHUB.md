...
ClipHub is the public registry where people share, discover, and download Nextstep company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Nextstep what a package registry is to a programming language. Nextstep already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Nextstep — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Nextstep instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Nextstep spec. It contains:
...
**Install (fresh start):**
```
Nextstep install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Nextstep instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Nextstep, export your company as a template, then publish:

```
Nextstep export --template my-company
Nextstep publish cliphub my-company
```
...
ClipHub is a **separate service** from Nextstep itself. Nextstep is self-hosted; ClipHub is a hosted registry that Nextstep instances talk to.
...
| **Nextstep CLI** | `nextstepai install`, `nextstepai publish`, `nextstepai cliphub sync` — built into Nextstep |
| **Nextstep UI** | "Browse ClipHub" panel in the Nextstep web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Nextstep) |
| Backend | TypeScript + Hono (consistent with Nextstep) |
| Database | PostgreSQL |
...
4. Run `nextstepai install cliphub:acme/lean-saas-shop`
5. Nextstep creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Nextstep until it works well
2. Export: `nextstepai export --template my-agency`
3. Publish: `nextstepai publish cliphub my-agency`
...
3. Install just that agent: `nextstepai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Nextstep

ClipHub is **not required** to use Nextstep. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Nextstep what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `nextstepai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Nextstep web UI
- [ ] `nextstepai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Nextstep's own philosophy)
