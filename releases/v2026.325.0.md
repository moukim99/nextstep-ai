...
- **Company import/export** — Full company portability with a file-browser UX for importing and exporting agent companies. Includes rich frontmatter preview, nested file picker, merge-history support, GitHub shorthand refs, and CLI `company import`/`company export` commands. Imported companies open automatically after import, and heartbeat timers are disabled for imported agents by default. ([#840](https://github.com/nextstepai/nextstep/pull/840), [#1631](https://github.com/nextstepai/nextstep/pull/1631), [#1632](https://github.com/nextstepai/nextstep/pull/1632), [#1655](https://github.com/nextstepai/nextstep/pull/1655))
- **Company skills library** — New company-scoped skills system with a skills UI, agent skill sync across all local adapters (Claude, Codex, Pi, Gemini), pinned GitHub skills with update checks, and built-in skill support. ([#1346](https://github.com/nextstepai/nextstep/pull/1346))
- **Routines and recurring tasks** — Full routines engine with triggers, routine runs, coalescing, and recurring task portability. Includes API documentation and routine export support. ([#1351](https://github.com/nextstepai/nextstep/pull/1351), [#1622](https://github.com/nextstepai/nextstep/pull/1622), @aronprins)
...
- **Improved CLI API connection errors** — Better error messages when the CLI cannot reach the Nextstep API.
...
- **Docker image CI** — Added Docker image build and deploy workflow. ([#542](https://github.com/nextstepai/nextstep/pull/542), @albttx)
- **Project filter on issues** — Issues list can now be filtered by project. ([#552](https://github.com/nextstepai/nextstep/pull/552), @mvanhorn)
- **Inline comment image attachments** — Uploaded images are now embedded inline in comments. ([#551](https://github.com/nextstepai/nextstep/pull/551), @mvanhorn)
- **AGENTS.md fallback** — Claude-local adapter gracefully falls back when AGENTS.md is missing. ([#550](https://github.com/nextstepai/nextstep/pull/550), @mvanhorn)
- **Company-creator skill** — New skill for scaffolding agent company packages from scratch.
- **Reports page rename** — Reports section renamed for clarity. ([#1380](https://github.com/nextstepai/nextstep/pull/1380), @DanielSousa)
- **Eval framework bootstrap** — Promptfoo-based evaluation framework with YAML test cases for systematic agent behavior testing. ([#832](https://github.com/nextstepai/nextstep/pull/832), @mvanhorn)
- **Board CLI authentication** — Browser-based auth flow for the CLI so board users can authenticate without manually copying API keys. ([#1635](https://github.com/nextstepai/nextstep/pull/1635))

## Fixes

- **Embedded Postgres initdb in Docker slim** — Fixed initdb failure in slim containers by adding proper initdbFlags types. ([#737](https://github.com/nextstepai/nextstep/pull/737), @alaa-alghazouli)
- **OpenClaw gateway crash** — Fixed unhandled rejection when challengePromise fails. ([#743](https://github.com/nextstepai/nextstep/pull/743), @Sigmabrogz)
...
- **Login form 1Password detection** — Fixed login form not being detected by password managers; Enter key now submits correctly. ([#1014](https://github.com/nextstepai/nextstep/pull/1014))
...
- **Pi local adapter** — Fixed Pi adapter missing from `isLocal` check. ([#1382](https://github.com/nextstepai/nextstep/pull/1382), @lucas-stellet)
- **CLI auth migration idempotency** — Made migration 0044 idempotent to avoid failures on re-run.
- **Dev restart tracking** — `.Nextstep` and test-only paths are now ignored in dev restart detection.
...
- **Worktree merge-history migrations** — Fixed migration handling in worktree contexts. ([#1385](https://github.com/nextstepai/nextstep/pull/1385))
