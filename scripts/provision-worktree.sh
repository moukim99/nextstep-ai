...
base_cwd="${NEXTSTEP_WORKSPACE_BASE_CWD:?NEXTSTEP_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${NEXTSTEP_WORKSPACE_CWD:?NEXTSTEP_WORKSPACE_CWD is required}"
NEXTSTEP_home="${NEXTSTEP_HOME:-$HOME/.Nextstep}"
NEXTSTEP_instance_id="${NEXTSTEP_INSTANCE_ID:-default}"
NEXTSTEP_dir="$worktree_cwd/.Nextstep"
worktree_config_path="$NEXTSTEP_dir/config.json"
worktree_env_path="$NEXTSTEP_dir/.env"
worktree_name="${NEXTSTEP_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${NEXTSTEP_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.nextstep/config.json" || -L "$base_cwd/.nextstep/config.json" ) ]]; then
  source_config_path="$base_cwd/.nextstep/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$NEXTSTEP_home/instances/$NEXTSTEP_instance_id/config.json"
fi
...
run_isolated_worktree_init() {
  ...
  if command -v pnpm >/dev/null 2>&1 && pnpm nextstepai --help >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      pnpm nextstepai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi

  if command -v nextstepai >/dev/null 2>&1; then
    (
      cd "$worktree_cwd"
      nextstepai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    )
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  NEXTSTEP_DIR="$NEXTSTEP_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  NEXTSTEP_WORKTREES_DIR="${NEXTSTEP_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const NextstepDir = process.env.NEXTSTEP_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.NEXTSTEP_WORKTREES_DIR) ?? "~/.Nextstep-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(NextstepDir, "config.json");
  const envPath = path.resolve(NextstepDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "Nextstep",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.NEXTSTEP_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.NEXTSTEP_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.NEXTSTEP_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "NEXTSTEP_HOME=" + JSON.stringify(worktreeHome),
    "NEXTSTEP_INSTANCE_ID=" + JSON.stringify(instanceId),
    "NEXTSTEP_CONFIG=" + JSON.stringify(configPath),
    "NEXTSTEP_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "NEXTSTEP_IN_WORKTREE=true",
    "NEXTSTEP_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.NEXTSTEP_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("NEXTSTEP_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]]; then
  echo "Reusing existing isolated Nextstep worktree config at $worktree_config_path" >&2
else
  if nextstepai_command_available; then
    run_isolated_worktree_init
  else
    echo "nextstepai CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
    write_fallback_worktree_config
  fi
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".Nextstep-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
