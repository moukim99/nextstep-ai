...
COMPANY_ID="${COMPANY_ID:-${NEXTSTEP_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-Nextstep-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-Nextstep-openclaw-smoke}"
...
Provide one of:
  NEXTSTEP_AUTH_HEADER=\"Bearer <board-token>\"
  NEXTSTEP_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Nextstep OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.Nextstep.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
