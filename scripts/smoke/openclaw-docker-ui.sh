...
NEXTSTEP_HOST_PORT="${NEXTSTEP_HOST_PORT:-3100}"
NEXTSTEP_HOST_FROM_CONTAINER="${NEXTSTEP_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.Nextstep-openclaw.override.yml"
...
detect_NEXTSTEP_base_url() {
  ...
  for candidate in "$NEXTSTEP_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${NEXTSTEP_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${NEXTSTEP_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
NEXTSTEP_base_url="$(detect_NEXTSTEP_base_url || true)"
...
Nextstep URL for OpenClaw container:
EOF
  if [[ -n "$NEXTSTEP_base_url" ]]; then
    cat <<EOF
  $NEXTSTEP_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${NEXTSTEP_HOST_PORT}
  (Do not use http://127.0.0.1:${NEXTSTEP_HOST_PORT} inside the container.)
  If Nextstep rejects the host, run on host machine:
    pnpm nextstepai allowed-hostname host.docker.internal
  Then restart Nextstep and re-run this script.
EOF
  fi
  cat <<EOF
