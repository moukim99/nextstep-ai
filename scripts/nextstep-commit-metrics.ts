...
const DEFAULT_QUERY = "\"Co-Authored-By: Nextstep <noreply@Nextstep.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/Nextstep-commit-metrics-cache.json");
...
const NEXTSTEP_EMAIL = "noreply@Nextstep.ing";
const NEXTSTEP_NAME = "Nextstep";
...
function printHelp() {
  console.log(`Usage: tsx scripts/Nextstep-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === NEXTSTEP_EMAIL) || (displayName && displayName.toLowerCase() === NEXTSTEP_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Nextstep commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "Nextstep-commit-metrics",
