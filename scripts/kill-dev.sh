#!/usr/bin/env bash
#
# Kill all local Nextstep dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all Nextstep dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/Nextstep(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.nextstep/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.nextstep/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.nextstep/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/Nextstep*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.nextstep/instances/*/db/postmaster.pid
    "$sibling_root"/.nextstep/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 && ${#browser_pids[@]} -eq 0 ]]; then
  echo "No Nextstep dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Nextstep dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Nextstep node processes..."
