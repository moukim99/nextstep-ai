...
async function ensureWorkspaceLinksCurrent(workspaceDir: string) {
  ...
  console.log(`[Nextstep] detected stale workspace package links for ${workspaceDir}; relinking dependencies...`);
  for (const mismatch of mismatches) {
    console.log(
      `[Nextstep]   ${mismatch.packageName}: ${mismatch.actualPath ?? "missing"} -> ${mismatch.expectedPath}`,
    );
  }
  ...
  for (const mismatch of mismatches) {
    ...
    await fs.symlink(mismatch.expectedPath, linkPath, isWin ? "junction" : "dir");
  }
