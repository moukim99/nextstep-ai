...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[Nextstep] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`nextstepai worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".Nextstep", "dev-server-status.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-nextstep-dev-server-status-token";
...
const ignoredRelativePaths = new Set([
  ".nextstep/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[Nextstep] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[Nextstep] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[Nextstep] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  NEXTSTEP_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.NEXTSTEP_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.NEXTSTEP_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.NEXTSTEP_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.NEXTSTEP_DEV_SERVER_STATUS_TOKEN;
  env.NEXTSTEP_MIGRATION_PROMPT ??= "never";
  env.NEXTSTEP_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[Nextstep] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.NEXTSTEP_BIND = effectiveBind;
  if (bindHost) {
    env.NEXTSTEP_BIND_HOST = bindHost;
  } else {
    delete env.NEXTSTEP_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.NEXTSTEP_DEPLOYMENT_MODE;
    delete env.NEXTSTEP_DEPLOYMENT_EXPOSURE;
    delete env.NEXTSTEP_AUTH_BASE_URL_MODE;
    console.log("[Nextstep] dev mode: local_trusted (bind=loopback)");
  } else {
    env.NEXTSTEP_DEPLOYMENT_MODE = "authenticated";
    env.NEXTSTEP_DEPLOYMENT_EXPOSURE = "private";
    env.NEXTSTEP_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[Nextstep] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.NEXTSTEP_BIND;
  delete env.NEXTSTEP_BIND_HOST;
  delete env.NEXTSTEP_DEPLOYMENT_MODE;
  delete env.NEXTSTEP_DEPLOYMENT_EXPOSURE;
  delete env.NEXTSTEP_AUTH_BASE_URL_MODE;
  console.log("[Nextstep] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[Nextstep] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@nextstepai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[Nextstep] Command failed with code ${status.code}: pnpm --filter @nextstepai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[Nextstep] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.NEXTSTEP_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[Nextstep] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[Nextstep] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@nextstepai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[Nextstep] plugin sdk build failed");
...
async function startServerChild() {
  ...
  const childEnv = { ...env };
  if (isFirstStart) {
    childEnv.NEXTSTEP_OPEN_ON_LISTEN = "true";
    isFirstStart = false;
  }

  child = spawn(
    pnpmBin,
    ["--filter", "@nextstepai/server", serverScript, ...forwardedArgs],
