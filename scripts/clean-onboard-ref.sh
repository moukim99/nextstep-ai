...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Nextstep data dir to use
  NEXTSTEP_HOST=127.0.0.1    Host passed to the onboarded server
  NEXTSTEP_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/Nextstep-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/Nextstep-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export NEXTSTEP_HOST="${NEXTSTEP_HOST:-127.0.0.1}"
export NEXTSTEP_PORT="${NEXTSTEP_PORT:-3100}"
export NEXTSTEP_OPEN_ON_LISTEN="${NEXTSTEP_OPEN_ON_LISTEN:-false}"
...
echo "NEXTSTEP_HOST: $NEXTSTEP_HOST"
echo "NEXTSTEP_PORT: $NEXTSTEP_PORT"
...
env \
  ...
  HOST="$NEXTSTEP_HOST" \
  PORT="$NEXTSTEP_PORT" \
  NEXTSTEP_OPEN_ON_LISTEN="$NEXTSTEP_OPEN_ON_LISTEN" \
  pnpm nextstepai onboard --yes --data-dir "$PC_DATA"
