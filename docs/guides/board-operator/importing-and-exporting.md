...
Nextstep companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .Nextstep.yaml     # Adapter config, env inputs, routines
```
...
- **.Nextstep.yaml** holds Nextstep-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
nextstepai company export <company-id> --out ./my-export
```
...
nextstepai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
nextstepai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
nextstepai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.Nextstep.yaml`
...
```sh
# From a local folder
nextstepai company import ./my-export

# From a GitHub URL
nextstepai company import https://github.com/org/repo

# From a GitHub subfolder
nextstepai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
nextstepai company import org/repo
nextstepai company import org/repo/companies/acme
```
...
If `--target` is not specified, Nextstep infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
nextstepai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
nextstepai company import org/company-templates/engineering-team \
  --target new \
...
```sh
nextstepai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
nextstepai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
nextstepai company import ./package \
  --target new \
  --yes \
  --json
```
...
Nextstep supports several GitHub URL formats:
