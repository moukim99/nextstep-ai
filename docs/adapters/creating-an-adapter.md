...
Build a custom adapter to connect Nextstep to any agent runtime.
...
|---|---|---|
| Source | Inside `Nextstep-fork` | Separate npm package |
| Distribution | Ships with Nextstep | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Nextstep's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@nextstepai/adapter-utils/server-utils`
2. Build environment with `buildNextstepEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@nextstepai/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildNextstepEnv(agent)` | `@nextstepai/adapter-utils/server-utils` | Inject `NEXTSTEP_*` env vars |
| `renderTemplate(tpl, data)` | `@nextstepai/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@nextstepai/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@nextstepai/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Nextstep's source:
...
`format-event.ts` — pretty-prints stdout for `nextstepai run --watch` using `picocolors`.
...
With these flags set, the Nextstep UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Nextstep source changes required.
...
Make Nextstep skills discoverable to your agent runtime without writing to the agent's working directory:
