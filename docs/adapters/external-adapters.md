...
summary: Build, package, and distribute adapters as plugins without modifying Nextstep source
---

Nextstep supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Nextstep's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `Nextstep-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Nextstep | Published to npm or linked via `file:` |
| Updates | Requires Nextstep release | Independent versioning |
...
```json
{
  ...
  "dependencies": {
    "@nextstepai/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `Nextstep.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@nextstepai/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with Nextstep vars injected
  const env = buildNextstepEnv(agent);
...
#### Available Helpers from `@nextstepai/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildNextstepEnv(agent)` | Inject `NEXTSTEP_*` environment variables |
...
```sh
# Via the Nextstep UI
# Settings → Adapters → Install from npm → "my-Nextstep-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-Nextstep-adapter"}'
```
...
Local adapters are symlinked into Nextstep's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.nextstep/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-Nextstep-adapter",
...
```ts
import type { AdapterSessionCodec } from "@nextstepai/adapter-utils";
...
Other Nextstep users can then install your adapter by package name from the UI or API.
