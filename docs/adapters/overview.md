...
summary: What adapters are and how they connect agents to Nextstep
---

Adapters are the bridge between Nextstep's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, Nextstep:
...
|---------|---------|----------|-------------|
| Droid Local | `@henkey/droid-Nextstep-adapter` | `droid_local` | Runs Factory Droid locally |

## External Adapters

You can build and distribute adapters as standalone packages — no changes to Nextstep's source code required. External adapters are loaded at startup via the plugin system.
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `nextstepai run --watch`
```
...
External adapters can ship a self-contained UI parser that tells the Nextstep web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
