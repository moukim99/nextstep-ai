...
- Running a Python script that calls the Nextstep API
...
1. Nextstep spawns the configured command as a child process
2. Standard Nextstep environment variables are injected (`NEXTSTEP_AGENT_ID`, `NEXTSTEP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Nextstep API and perform work.
