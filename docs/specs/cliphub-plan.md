# ClipHub: Marketplace for Nextstep Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Nextstep configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Nextstep-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Nextstep company (agents, projects, governance) |
...
A complete Nextstep company configuration:
...
Single-agent configurations designed to plug into a Nextstep org:
...
Portable skill files that any Nextstep agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Nextstep's skill loading system
...
interface Listing {
  ...
  NextstepVersionMin: string;     // Minimum Nextstep version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Nextstep company
...
| `POST` | `/api/listings/:id/install` | Install to Nextstep company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Nextstep company (or create new)
  → ClipHub API calls Nextstep API to:
  ...
  → Redirect to Nextstep dashboard with new team running
```
...
### 5.3 Creator: Export from Nextstep → Publish

```
Running Nextstep company → "Export as Blueprint" (CLI or UI)
  → Nextstep exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Nextstep brand blue/purple)
...
| Install flow | One-click deploy to Nextstep company |
...
{
  "targetCompanyId": "uuid",         // Existing Nextstep company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `Nextstep-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Nextstep
- **Backend**: Node.js API (or extend Nextstep server)
- **Database**: Postgres (can share Nextstep's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Nextstep auth (or OAuth2)

### 9.2 Integration with Nextstep

ClipHub can be:
- **Option A**: A separate app that calls Nextstep's API to install blueprints
- **Option B**: A built-in section of the Nextstep UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Nextstep UI and API.
...
- [ ] Install flow: blueprint → Nextstep company
...
- [ ] "Export from Nextstep" CLI command
