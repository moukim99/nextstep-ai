...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Nextstep.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.Nextstep.yaml`
...
- Nextstep must not require extra top-level fields for skill validity
- Nextstep-specific extensions must live under `metadata.Nextstep` or `metadata.sources`
...
For Nextstep, the preferred fidelity extension is:
...
- issue/task Nextstep-only metadata
...
- Nextstep tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Nextstep shape:
...
Additional rules for Nextstep exporters:
...
- omit empty and default-valued Nextstep fields when possible
...
- Nextstep exporters should emit `.Nextstep.yaml` as a sidecar by default
...
Nextstep can map this spec to its runtime model like this:
...
- Nextstep extension:
  - `.Nextstep.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Nextstep-specific fidelity

Inline Nextstep-only metadata that must live inside a shared markdown file should use:

- `metadata.Nextstep`

That keeps the base format broader than Nextstep.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Nextstep.

## 20. Cutover

Nextstep should cut over to this markdown-first package model as the primary portability format.

`Nextstep.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Nextstep, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
