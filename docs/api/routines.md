...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Nextstep-Signature` + `X-Nextstep-Timestamp` header pair matching the trigger's signing mode.
