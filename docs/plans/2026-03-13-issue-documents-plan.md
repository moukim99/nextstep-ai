...
Add first-class **documents** to Nextstep as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Nextstep workflow/docs.
...
Update the Nextstep agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/Nextstep/SKILL.md`
