...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Nextstep saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
```bash
pnpm nextstepai feedback report
```
...
```bash
# Installed CLI
nextstepai feedback report

# Point to a different server or company
pnpm nextstepai feedback report --api-base http://127.0.0.1:3000 --company-id <company-id>

# Include raw payload dumps in the report
pnpm nextstepai feedback report --payloads
```
...
```bash
pnpm nextstepai feedback export
```
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # Nextstep feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the Nextstep envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.

```bash
# Custom server and output directory
pnpm nextstepai feedback export --api-base http://127.0.0.1:3000 --company-id <company-id> --out ./my-export
```
...
- Default endpoint: when no feedback export backend URL is configured, Nextstep falls back to `https://telemetry.Nextstep.ing`
