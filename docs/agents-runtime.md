...
Audience: Operators setting up and running agents in Nextstep

## 1. What this system does

Agents in Nextstep do not run continuously.  
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-Nextstep-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Nextstep assumes the CLI is already installed and authenticated on the host machine.
...
Nextstep stores session IDs for resumable adapters.
...
Nextstep pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Nextstep surfaces this as a warning in environment tests, not a hard error.
