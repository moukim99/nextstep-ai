...
summary: Deploy Nextstep to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy Nextstep to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
- The Nextstep repo cloned locally
...
export NEXTSTEP_DOMAIN=Nextstep.example.com   # your domain
...
aws ecr create-repository \
  --repository-name Nextstep-server \
...
# Build
docker build -t Nextstep-server .

# Tag and push
docker tag Nextstep-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/Nextstep-server:latest
...
ALB_SG=$(aws ec2 create-security-group \
  --group-name Nextstep-alb \
  --description "Nextstep ALB" \
  --vpc-id $VPC_ID \
...
ECS_SG=$(aws ec2 create-security-group \
  --group-name Nextstep-ecs \
  --description "Nextstep ECS tasks" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
RDS_SG=$(aws ec2 create-security-group \
  --group-name Nextstep-rds \
  --description "Nextstep RDS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
EFS_SG=$(aws ec2 create-security-group \
  --group-name Nextstep-efs \
  --description "Nextstep EFS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
aws rds create-db-subnet-group \
  --db-subnet-group-name Nextstep-db-subnet \
  --db-subnet-group-description "Nextstep RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier Nextstep-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username Nextstep \
...
DATABASE_URL="postgresql://Nextstep:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/Nextstep"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=Nextstep-data \
...
aws secretsmanager create-secret \
  --name Nextstep/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name Nextstep/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name Nextstep/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name Nextstep/openai-api-key \
  --secret-string "YOUR_OPENAI_KEY"

aws secretsmanager create-secret \
  --name Nextstep/github-token \
...
# Task execution role
aws iam create-role \
  --role-name Nextstep-ecs-execution \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:Nextstep/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name Nextstep-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name Nextstep

aws logs create-log-group --log-group-name /ecs/Nextstep
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    -e "s|<EFS_ID>|$EFS_ID|g" \
    -e "s|<DOMAIN>|$NEXTSTEP_DOMAIN|g" \
    docker/ecs-task-definition.json > /tmp/Nextstep-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/Nextstep-task-def.json
```
...
- Create a CNAME or ALIAS record for `$NEXTSTEP_DOMAIN` -> `$ALB_DNS`
...
aws ecs create-service \
  --cluster Nextstep \
  --service-name Nextstep-server \
  --task-definition Nextstep-server \
  --desired-count 1 \
  --launch-type FARGATE \
...
aws ecs describe-services \
  --cluster Nextstep \
  --services Nextstep-server \
  --query 'services[0].{desired:desiredCount,running:runningCount,status:status}'

# Check task health
aws ecs list-tasks --cluster Nextstep --service-name Nextstep-server
TASK_ARN=$(aws ecs list-tasks --cluster Nextstep --service-name Nextstep-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster Nextstep --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/Nextstep --since 10m --follow

# Hit the health endpoint
curl -sf https://$NEXTSTEP_DOMAIN/api/health
```
...
aws ecs update-service \
  --cluster Nextstep \
  --service Nextstep-server \
  --task-definition Nextstep-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier Nextstep-db
aws rds start-db-instance --db-instance-identifier Nextstep-db
```
...
aws ecs update-service --cluster Nextstep --service Nextstep-server --desired-count 0
aws ecs delete-service --cluster Nextstep --service Nextstep-server --force
aws ecs delete-cluster --cluster Nextstep
...
aws rds delete-db-instance \
  --db-instance-identifier Nextstep-db \
  --final-db-snapshot-identifier Nextstep-db-final
aws rds wait db-instance-deleted --db-instance-identifier Nextstep-db
aws rds delete-db-subnet-group --db-subnet-group-name Nextstep-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id Nextstep/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name Nextstep-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name Nextstep-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name Nextstep-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name Nextstep-ecs-execution
aws iam delete-role --role-name Nextstep-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/Nextstep
```
