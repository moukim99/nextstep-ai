...
Nextstep supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
```sh
pnpm nextstepai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Nextstep emits a one-time claim URL at startup:
...
```sh
NEXTSTEP_DEPLOYMENT_MODE=authenticated NEXTSTEP_BIND=lan pnpm nextstepai run
```
