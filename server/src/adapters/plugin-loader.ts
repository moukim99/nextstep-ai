...
function extractUiParserSource(
  ...
  const contractVersion = pkg.Nextstep?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no Nextstep.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
