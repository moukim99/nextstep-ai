...
import { getAdapterSessionManagement } from "@nextstepai/adapter-utils";
import {
...
} from "@nextstepai/adapter-acpx-local";
import {
...
} from "@nextstepai/adapter-claude-local";
import {
...
} from "@nextstepai/adapter-codex-local/server";
import {
  agentConfigurationDoc as codexAgentConfigurationDoc,
  models as codexModels,
  modelProfiles as codexModelProfiles,
} from "@nextstepai/adapter-codex-local";
import {
...
} from "@nextstepai/adapter-cursor-local";
import {
...
} from "@nextstepai/adapter-gemini-local";
import {
...
} from "@nextstepai/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@nextstepai/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@nextstepai/adapter-openclaw-gateway";
...
} from "@nextstepai/adapter-pi-local";
import {
...
const hermesLocalAdapter: ServerAdapterModule = {
  ...
  execute: async (ctx) => {
    ...
    const explicitApiKey =
      typeof existingEnv.NEXTSTEP_API_KEY === "string" && existingEnv.NEXTSTEP_API_KEY.trim().length > 0;
    ...
    const authGuardPrompt = [
      "Nextstep API safety rule:",
      "Use Authorization: Bearer $NEXTSTEP_API_KEY on every Nextstep API request.",
      "Use X-Nextstep-Run-Id: $NEXTSTEP_RUN_ID on every Nextstep API request that writes or mutates data, including comments and issue updates.",
      "Never use a board, browser, or local-board session for Nextstep API writes.",
    ].join("\n");
    ...
    const patchedConfig: Record<string, unknown> = {
      ...
      env: {
        ...existingEnv,
        ...(!explicitApiKey ? { NEXTSTEP_API_KEY: normalizedCtx.authToken } : {}),
        NEXTSTEP_RUN_ID: normalizedCtx.runId,
      },
    };
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[Nextstep] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[Nextstep] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[Nextstep] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[Nextstep] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
