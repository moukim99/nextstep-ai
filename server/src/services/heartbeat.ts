...
import { trackAgentFirstHeartbeat } from "@nextstepai/shared/telemetry";
...
import { extractSkillMentionIds } from "@nextstepai/shared";
...
const DEFERRED_WAKE_CONTEXT_KEY = "_NextstepWakeContext";
const WAKE_COMMENT_IDS_KEY = "wakeCommentIds";
const NEXTSTEP_WAKE_PAYLOAD_KEY = "NextstepWake";
const NEXTSTEP_HARNESS_CHECKOUT_KEY = "NextstepHarnessCheckedOut";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const REPO_ONLY_CWD_SENTINEL = "/__NEXTSTEP_repo_only__";
...
export function applyRunScopedMentionedSkillKeys(
  ...
  const existingPreference = readNextstepSkillSyncPreference(config);
  return writeNextstepSkillSyncPreference(config, [
...
export function compactRunLogChunk(chunk: string, maxChars = MAX_PERSISTED_LOG_CHUNK_CHARS) {
  ...
  const marker = `\n[Nextstep truncated run log chunk: omitted ${omittedChars} chars]\n`;
...
function enrichWakeContextSnapshot(input: {
  ...
  if (wakeCommentIds.length > 0) {
    ...
    delete contextSnapshot[NEXTSTEP_WAKE_PAYLOAD_KEY];
...
export function mergeCoalescedContextSnapshot(
  ...
  if (mergedCommentIds.length > 0) {
    ...
    delete merged[NEXTSTEP_WAKE_PAYLOAD_KEY];
  }
...
async function buildNextstepWakePayload(input: {
  db: Db;
  companyId: string;
  contextSnapshot: Record<string, unknown>;
  continuationSummary?:
  ...
  return {
    ...
    checkedOutByHarness: input.contextSnapshot[NEXTSTEP_HARNESS_CHECKOUT_KEY] === true,
...
export function buildNextstepTaskMarkdown(input: {
  issue: {
    id: string;
    identifier: string | null;
    title: string;
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  function monitorRecoveryComment(input: {
    ...
    return [
      `Nextstep cleared the scheduled external-service monitor for ${label} because ${reason}.`,
      "",
          ...
          .where(
            and(
              ...
              sql`(
                ...
                or ${agentWakeupRequests.payload} -> '_NextstepWakeContext' ->> 'issueId' = ${issue.id}
                or ${agentWakeupRequests.payload} -> '_NextstepWakeContext' ->> 'taskId' = ${issue.id}
              )`,
            ),
          )
          .limit(1)
  ...
  async function cancelQueuedRunForBlockedDependencies(
    ...
    const now = new Date();
    const reason =
      "Cancelled because issue dependencies are still blocked; Nextstep will wake the assignee when blockers resolve";
  ...
  async function executeRun(runId: string) {
    ...
    if (
      ...
      try {
        ...
        context[NEXTSTEP_HARNESS_CHECKOUT_KEY] = true;
      } catch (error) {
        if (!isCheckoutConflictError(error)) throw error;
        context[NEXTSTEP_HARNESS_CHECKOUT_KEY] = false;
      }
    ...
    if (continuationSummary) {
      context.NextstepContinuationSummary = {
        key: continuationSummary.key,
        title: continuationSummary.title,
      ...
      delete context.NextstepContinuationSummary;
    }
    const NextstepWakePayload = await buildNextstepWakePayload({
      db,
      companyId: agent.companyId,
      contextSnapshot: context,
      continuationSummary,
    ...
    if (NextstepWakePayload) {
      context[NEXTSTEP_WAKE_PAYLOAD_KEY] = NextstepWakePayload;
    } else {
      delete context[NEXTSTEP_WAKE_PAYLOAD_KEY];
    }
    const taskMarkdown = buildNextstepTaskMarkdown({
      issue: issueRef
        ? {
            id: issueRef.id,
            identifier: issueRef.identifier,
    ...
    if (issueRef) {
      ...
      delete context.NextstepIssue;
    }
    if (wakeCommentContext) {
      context.NextstepWakeComment = wakeCommentContext;
    } else {
      delete context.NextstepWakeComment;
    }
    if (taskMarkdown) {
      context.NextstepTaskMarkdown = taskMarkdown;
    } else {
      delete context.NextstepTaskMarkdown;
    }
    ...
    let runtimeConfig = {
      ...
      NextstepRuntimeSkills: runtimeSkillEntries,
    };
    ...
    context.NextstepWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.NextstepRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.NextstepRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.NextstepSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.NextstepSessionRotationReason = sessionCompaction.reason;
      context.NextstepPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.NextstepSessionHandoffMarkdown;
      delete context.NextstepSessionRotationReason;
      delete context.NextstepPreviousSessionId;
    }
    ...
    try {
      ...
      if (runScopedMentionedSkillKeys.length > 0) {
        await onLog(
          "stdout",
          `[Nextstep] Enabled run-scoped skills from issue mentions: ${runScopedMentionedSkillKeys.join(", ")}\n`,
        );
      }
      ...
      if (runtimeServices.length > 0) {
        context.NextstepRuntimeServices = runtimeServices;
        context.NextstepRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[Nextstep] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected NEXTSTEP_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.NextstepRuntimeServices = combinedRuntimeServices;
        context.NextstepRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[Nextstep] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
      ...
      if (finalizedRun) {
        ...
        if (issueId && outcome === "succeeded") {
          try {
            ...
            await onLog(
              "stderr",
              `[Nextstep] Failed to post run summary comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
  ...
  function buildImmediateExecutionPathRecoveryComment(input: {
    ...
    if (input.status === "todo") {
      return (
        "Nextstep automatically retried dispatch for this assigned `todo` issue during terminal run recovery, " +
    ...
    return (
      "Nextstep automatically retried continuation for this assigned `in_progress` issue during terminal run " +
