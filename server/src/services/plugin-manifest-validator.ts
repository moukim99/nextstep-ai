 ...
 */
import { pluginManifestV1Schema } from "@nextstepai/shared";
import type { NextstepPluginManifestV1 } from "@nextstepai/shared";
import { PLUGIN_API_VERSION } from "@nextstepai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): NextstepPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as NextstepPluginManifestV1,
        };
      }
