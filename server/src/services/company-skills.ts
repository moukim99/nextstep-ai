...
import { companies, companySkills } from "@nextstepai/db";
import { readNextstepSkillSyncPreference } from "@nextstepai/adapter-utils/server-utils";
import type { NextstepSkillEntry } from "@nextstepai/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@nextstepai/shared";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("nextstepai/nextstep/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveNextstepInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "NEXTSTEP_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Nextstep skills are read-only.",
      sourceLabel: "Nextstep bundled",
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Nextstep workspace",
        sourceBadge: "Nextstep",
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "NEXTSTEP_bundled";
      out.push({
        key: skill.key,
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "NEXTSTEP_bundled"
        && incomingKind === "github"
        && incomingOwner === "nextstepai"
        && incomingRepo === "Nextstep"
      ) {
        out.push(existing);
        continue;
      }
