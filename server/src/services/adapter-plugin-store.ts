 ...
 * ~/.nextstep/adapter-plugins.json. This is the source of truth for which
...
import { resolveNextstepHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-Nextstep-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const NextstepDir = resolveNextstepHomeDir();
  return {
    adapterPluginsDir: path.join(NextstepDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(NextstepDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(NextstepDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "Nextstep-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Nextstep external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
