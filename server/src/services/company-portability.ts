...
import {
  readNextstepSkillSyncPreference,
  writeNextstepSkillSyncPreference,
} from "@nextstepai/adapter-utils/server-utils";
import { requireOpenCodeModelId } from "@nextstepai/adapter-opencode-local/server";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".Nextstep.yaml" || normalized === ".Nextstep.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const Nextstep = isPlainRecord(metadata?.Nextstep) ? metadata?.Nextstep as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.NextstepSkillKey)
    ?? asString(Nextstep?.skillKey)
    ?? asString(Nextstep?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "NEXTSTEP_bundled") {
    return `nextstepai/nextstep/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "Nextstep" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Nextstep will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .Nextstep.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[NextstepExtensionPath];
  if (selectedFiles.has(NextstepExtensionPath) && typeof extensionEntry === "string") {
    filtered[NextstepExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findNextstepExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".Nextstep.yaml"] === "string") return ".Nextstep.yaml";
  if (typeof files[".Nextstep.yml"] === "string") return ".Nextstep.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.Nextstep.yaml") || entry.endsWith("/.Nextstep.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "NEXTSTEP_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "nextstepai/nextstep",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/nextstepai/nextstep/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "NEXTSTEP_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.NextstepSkillKey = skill.key;
  metadata.Nextstep = {
    ...(isPlainRecord(metadata.Nextstep) ? metadata.Nextstep : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const NextstepExtensionPath = findNextstepExtensionPath(normalizedFiles);
  const NextstepExtension = NextstepExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, NextstepExtensionPath) ?? "")
    : {};
  const NextstepCompany = isPlainRecord(NextstepExtension.company) ? NextstepExtension.company : {};
  const NextstepSidebar = normalizePortableSidebarOrder(NextstepExtension.sidebar);
  const NextstepAgents = isPlainRecord(NextstepExtension.agents) ? NextstepExtension.agents : {};
  const NextstepProjects = isPlainRecord(NextstepExtension.projects) ? NextstepExtension.projects : {};
  const NextstepTasks = isPlainRecord(NextstepExtension.tasks) ? NextstepExtension.tasks : {};
  const NextstepRoutines = isPlainRecord(NextstepExtension.routines) ? NextstepExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(NextstepCompany.brandColor),
      logoPath: asString(NextstepCompany.logoPath) ?? asString(NextstepCompany.logo),
      attachmentMaxBytes:
        typeof NextstepCompany.attachmentMaxBytes === "number" && Number.isFinite(NextstepCompany.attachmentMaxBytes)
          ? Math.max(1, Math.floor(NextstepCompany.attachmentMaxBytes))
          : null,
      requireBoardApprovalForNewAgents:
        typeof NextstepCompany.requireBoardApprovalForNewAgents === "boolean"
          ? NextstepCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof NextstepCompany.feedbackDataSharingEnabled === "boolean"
          ? NextstepCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof NextstepCompany.feedbackDataSharingConsentAt === "string"
          ? NextstepCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(NextstepCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(NextstepCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: NextstepSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(NextstepAgents[slug]) ? NextstepAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(NextstepProjects[slug]) ? NextstepProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(NextstepTasks[slug]) ? NextstepTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(NextstepRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(NextstepRoutines[slug]) ? NextstepRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  const strictSecretsMode = process.env.NEXTSTEP_SECRETS_STRICT_MODE === "true";
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeNextstepSkillSyncPreference(
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".Nextstep.yaml" ||
          relative === ".Nextstep.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const NextstepAgentsOut: Record<string, Record<string, unknown>> = {};
    const NextstepProjectsOut: Record<string, Record<string, unknown>> = {};
    const NextstepTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const NextstepRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readNextstepSkillSyncPreference(
        ...
        NextstepAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      NextstepProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      NextstepTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      NextstepRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const NextstepExtensionPath = ".Nextstep.yaml";
    const NextstepAgents = Object.fromEntries(
      Object.entries(NextstepAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const NextstepProjects = Object.fromEntries(
      Object.entries(NextstepProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const NextstepTasks = Object.fromEntries(
      Object.entries(NextstepTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const NextstepRoutines = Object.fromEntries(
      Object.entries(NextstepRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[NextstepExtensionPath] = buildYamlFile(
      {
        schema: "Nextstep/v1",
        ...
        agents: Object.keys(NextstepAgents).length > 0 ? NextstepAgents : undefined,
        projects: Object.keys(NextstepProjects).length > 0 ? NextstepProjects : undefined,
        tasks: Object.keys(NextstepTasks).length > 0 ? NextstepTasks : undefined,
        routines: Object.keys(NextstepRoutines).length > 0 ? NextstepRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, NextstepExtensionPath);
