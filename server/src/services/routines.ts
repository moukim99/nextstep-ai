...
import { trackRoutineRun } from "@nextstepai/shared/telemetry";
...
function resolveRoutineVariableValues(
  ...
  for (const variable of variables) {
    ...
    // Nextstep manages from execution context, so callers cannot override them.
  ...
  return {
    ...
    createTrigger: async (
      ...
      if (input.kind === "webhook") {
        ...
        secretMaterial = {
          webhookUrl: `${process.env.NEXTSTEP_API_URL}/api/routine-triggers/public/${publicId}/fire`,
    ...
    rotateTriggerSecret: async (
      ...
      return {
        trigger: trigger as RoutineTrigger,
        secretMaterial: {
          webhookUrl: `${process.env.NEXTSTEP_API_URL}/api/routine-triggers/public/${existing.publicId}/fire`,
    ...
    firePublicTrigger: async (publicId: string, input: {
      ...
      if (trigger.signingMode === "none") {
        ...
        // generic X-Nextstep-Signature header so operators can use github_hmac
