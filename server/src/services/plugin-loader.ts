 ...
 *    (`~/.nextstep/plugins/`) and `node_modules` for packages matching
 *    the `Nextstep-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Nextstep plugins.
 * Packages matching this pattern are considered Nextstep plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "Nextstep-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: NextstepPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.nextstep/plugins/ local directory
  | "npm"               // npm packages matching Nextstep-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: NextstepPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the Nextstep-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "Nextstep-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: NextstepPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Nextstep plugins installed as npm packages in the current
   * Node.js environment matching the "Nextstep-plugin-*" naming convention.
   *
   ...
   * the "NextstepPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Nextstep plugin.
   * Throws if the package is a Nextstep plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<NextstepPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: NextstepPluginManifestV1;
    newManifest: NextstepPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Nextstep plugin naming convention.
 * Accepts both the "Nextstep-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @nextstepai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "NextstepPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const NextstepPlugin = pkgJson["NextstepPlugin"];
  if (
    NextstepPlugin !== null &&
    typeof NextstepPlugin === "object" &&
    !Array.isArray(NextstepPlugin)
  ) {
    const manifestRelPath = (NextstepPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: NextstepPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumNextstepVersion;
}
 ...
 *   packageName: "Nextstep-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: NextstepPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as NextstepPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Nextstep plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<NextstepPluginManifestV1> {
    let raw: unknown;
  ...
  async function loadManifestFromPackageRoot(
    packageRoot: string,
  ): Promise<NextstepPluginManifestV1 | null> {
  ...
  async function refreshPluginManifestFromPackage(
    ...
    if (!manifest) {
      throw new Error(`Plugin package ${plugin.packageName} no longer exposes a Nextstep manifest`);
    }
   ...
   * if the package is not a Nextstep plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasNextstepPlugin = "NextstepPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasNextstepPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<NextstepPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: NextstepPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      const workerOptions: WorkerStartOptions = {
        ...
        autoRestart: true,
        env: {
          NEXTSTEP_DEPLOYMENT_MODE: instanceInfo.deploymentMode ?? "",
          NEXTSTEP_DEPLOYMENT_EXPOSURE: instanceInfo.deploymentExposure ?? "",
        },
      };

      // Repo-local plugin installs can resolve workspace TS sources at runtime
      // (for example @nextstepai/shared exports). Run those workers through
