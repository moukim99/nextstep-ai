    ...
    if (!stat.isDirectory()) {
      ...
      if (access === "readWrite") {
        try {
          ...
          const probePath = path.join(realPath, `.Nextstep-local-folder-probe-${process.pid}-${Date.now()}`);
...
export async function writePluginLocalFolderTextAtomic(
  ...
  const tempPath = path.join(
    ...
    `.Nextstep-${path.basename(resolved.absolutePath)}-${process.pid}-${randomUUID()}.tmp`,
  );
...
export function defaultLocalFolderBasePath(pluginKey: string, companyId: string) {
  return path.join(os.homedir(), ".Nextstep", "plugin-data", companyId, pluginKey);
}
