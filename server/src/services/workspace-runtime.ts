...
import type { AdapterRuntimeServiceReport } from "@nextstepai/adapter-utils";
import type { Db } from "@nextstepai/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@nextstepai/db";
import {
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".Nextstep", "dist", "node_modules"]);
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.NEXTSTEP_WORKSPACE_CWD = input.worktreePath;
  env.NEXTSTEP_WORKSPACE_PATH = input.worktreePath;
  env.NEXTSTEP_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.NEXTSTEP_WORKSPACE_BRANCH = input.branchName;
  env.NEXTSTEP_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.NEXTSTEP_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.NEXTSTEP_WORKSPACE_SOURCE = input.base.source;
  env.NEXTSTEP_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.NEXTSTEP_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.NEXTSTEP_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.NEXTSTEP_PROJECT_ID = input.base.projectId ?? "";
  env.NEXTSTEP_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.NEXTSTEP_AGENT_ID = input.agent.id ?? "";
  env.NEXTSTEP_AGENT_NAME = input.agent.name;
  env.NEXTSTEP_COMPANY_ID = input.agent.companyId;
  env.NEXTSTEP_ISSUE_ID = input.issue?.id ?? "";
  env.NEXTSTEP_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.NEXTSTEP_ISSUE_TITLE = input.issue?.title ?? "";
  env.NEXTSTEP_ISSUE_WORK_MODE = input.issue?.workMode ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.NEXTSTEP_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.NEXTSTEP_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.NEXTSTEP_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.NEXTSTEP_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.NEXTSTEP_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.NEXTSTEP_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.NEXTSTEP_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.NEXTSTEP_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.NEXTSTEP_PROJECT_ID = input.workspace.projectId ?? "";
  env.NEXTSTEP_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.NEXTSTEP_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".Nextstep", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Nextstep", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
