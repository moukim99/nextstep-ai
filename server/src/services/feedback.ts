...
import { readNextstepSkillSyncPreference } from "@nextstepai/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@nextstepai/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@nextstepai/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@nextstepai/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveNextstepInstanceRoot } from "../home-paths.js";
...
const FEEDBACK_SCHEMA_VERSION = "Nextstep-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "Nextstep-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "Nextstep-feedback-v1";
const FEEDBACK_DESTINATION = "NEXTSTEP_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "NEXTSTEP_run" &&
    file.source !== "NEXTSTEP_run_events" &&
    file.source !== "NEXTSTEP_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.NEXTSTEP_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readNextstepSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let NextstepRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      NextstepRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "Nextstep/run.json",
        contentType: "application/json",
        source: "NEXTSTEP_run",
        contents: `${JSON.stringify(NextstepRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.NextstepRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "Nextstep/run-events.json",
        contentType: "application/json",
        source: "NEXTSTEP_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "Nextstep/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "NEXTSTEP_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.NextstepRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
