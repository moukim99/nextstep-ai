...
import { agentApiKeys, agents, authUsers, companies, companyMemberships, instanceUserRoles } from "@nextstepai/db";
...
async function resolveCloudTenantActor(db: Db, req: Request): Promise<Express.Request["actor"] | null> {
  const expectedToken = process.env.NEXTSTEP_CLOUD_TENANT_SERVER_TOKEN?.trim();
  if (!expectedToken) return null;

  const token = req.header("x-Nextstep-cloud-tenant-token")?.trim();
  if (!token || !constantTimeStringEqual(token, expectedToken)) return null;

  const userId = requiredCloudHeader(req, "x-Nextstep-cloud-user-id");
  const userEmail = requiredCloudHeader(req, "x-Nextstep-cloud-user-email").toLowerCase();
  const stackId = requiredCloudHeader(req, "x-Nextstep-cloud-stack-id");
  const stackRole = stackMembershipRole(req.header("x-Nextstep-cloud-stack-role"));
  const userName = req.header("x-Nextstep-cloud-user-name")?.trim() || userEmail;
  const NextstepCompanyId = req.header("x-Nextstep-cloud-Nextstep-company-id")?.trim();
  const companyId = cloudTenantCompanyId(stackId);
  const companyName = NextstepCompanyId || `${stackId} Nextstep`;
  const now = new Date();
  ...
  await db
    ...
    .values({
      id: companyId,
      name: companyName,
      description: `Provisioned by Nextstep Cloud for stack ${stackId}.`,
      status: "active",
...
function cloudTenantCompanyId(stackId: string): string {
  const bytes = createHash("sha256").update(`Nextstep-cloud-tenant-company:${stackId}`).digest();
