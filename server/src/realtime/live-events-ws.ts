...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@nextstepai/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  NextstepUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).NextstepUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.NextstepUpgradeContext = context;
