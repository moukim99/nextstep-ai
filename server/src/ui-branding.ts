const FAVICON_BLOCK_START = "<!-- NEXTSTEP_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- NEXTSTEP_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- NEXTSTEP_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- NEXTSTEP_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.NEXTSTEP_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.NEXTSTEP_WORKTREE_NAME) ?? nonEmpty(env.NEXTSTEP_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.NEXTSTEP_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="Nextstep-worktree-enabled" content="true" />',
    `<meta name="Nextstep-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="Nextstep-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="Nextstep-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
