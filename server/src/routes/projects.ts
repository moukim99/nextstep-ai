...
import { trackProjectCreated } from "@nextstepai/shared/telemetry";
...
export function projectRoutes(db: Db) {
  ...
  async function handleProjectWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Project workspace needs a local path before Nextstep can run workspace commands" });
      return;
    }
