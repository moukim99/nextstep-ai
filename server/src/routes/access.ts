...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole, PermissionKey } from "@nextstepai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "Nextstep" &&
    normalized !== "Nextstep-create-agent" &&
    normalized !== "Nextstep-create-plugin" &&
    normalized !== "Nextstep-converting-plans-to-tasks" &&
...
/** Resolve the Nextstep repo skills directory (built-in / managed skills). */
function resolveNextstepSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const NextstepSkillsDir = resolveNextstepSkillsDir();

  // Build set of Nextstep-managed skill names
  const NextstepSkillNames = new Set<string>();
  if (NextstepSkillsDir) {
    try {
      for (const entry of fs.readdirSync(NextstepSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) NextstepSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isNextstepManaged: NextstepSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.NextstepApiUrl)) {
    const legacyNextstepApiUrl = nonEmptyTrimmedString(input.NextstepApiUrl);
    if (legacyNextstepApiUrl) merged.NextstepApiUrl = legacyNextstepApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    NextstepApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.NextstepApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawNextstepApiUrl =
    typeof defaults.NextstepApiUrl === "string"
      ? defaults.NextstepApiUrl.trim()
      : "";
  if (rawNextstepApiUrl) {
    try {
      const parsedNextstepApiUrl = new URL(rawNextstepApiUrl);
      if (
        parsedNextstepApiUrl.protocol !== "http:" &&
        parsedNextstepApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_NEXTSTEP_api_url_protocol",
          level: "warn",
          message: `NextstepApiUrl must use http:// or https:// (got ${parsedNextstepApiUrl.protocol}).`
        });
      } else {
        normalized.NextstepApiUrl = parsedNextstepApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_NEXTSTEP_api_url_configured",
          level: "info",
          message: `NextstepApiUrl set to ${parsedNextstepApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_NEXTSTEP_api_url_invalid",
        level: "warn",
        message: `Invalid NextstepApiUrl: ${rawNextstepApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Nextstep host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Nextstep is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm nextstepai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/Nextstep`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/Nextstep-claimed-api-key.json and load NEXTSTEP_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: NextstepApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Nextstep hostname is reachable and allowed via `pnpm nextstepai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Nextstep API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/Nextstep/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Nextstep OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          NextstepApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Nextstep generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "NextstepApiUrl": "https://Nextstep-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Nextstep before key claim is allowed.
    ...
    - ~/.openclaw/workspace/Nextstep-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/Nextstep-claimed-api-key.json

    And set the NEXTSTEP_API_KEY and NEXTSTEP_API_URL in your environment variables as specified here:
    ...
    Then set NEXTSTEP_API_KEY and NEXTSTEP_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Nextstep skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your NEXTSTEP_API_URL to the top of your skill and note the path to your NEXTSTEP_API_URL
    ...
    ${
      ...
      "Ensure Nextstep is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Nextstep base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.NextstepApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm nextstepai allowed-hostname <host>
      - then restart Nextstep and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@Nextstep.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "Nextstep", path: "/api/skills/Nextstep" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "Nextstep-create-agent",
          path: "/api/skills/Nextstep-create-agent"
        },
        {
          name: "Nextstep-converting-plans-to-tasks",
          path: "/api/skills/Nextstep-converting-plans-to-tasks"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "Nextstep",
          path: `/api/invites/${token}/skills/Nextstep`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "Nextstep") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              NextstepApiUrl: req.body.NextstepApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.NextstepApiUrl &&
          !persistedDefaults.NextstepApiUrl
        ) {
          missingPersistedFields.push("NextstepApiUrl");
        }
