...
import { AGENT_ICON_NAMES } from "@nextstepai/shared";
...
export function llmRoutes(db: Db) {
  ...
  router.get("/llms/agent-configuration.txt", async (req, res) => {
    ...
    const lines = [
      "# Nextstep Agent Configuration Index",
      "",
      "Installed adapters:",
      ...
      "- Use the Nextstep-create-agent skill for end-to-end hiring: adapter reflection, config comparison, instruction source selection, icon choice, desiredSkills, sourceIssueId/sourceIssueIds, and approval follow-up.",
