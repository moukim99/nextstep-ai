...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@nextstepai/db";
...
import { trackAgentCreated } from "@nextstepai/shared/telemetry";
...
import type { AdapterExecutionTarget } from "@nextstepai/adapter-utils/execution-target";
import type {
...
import { runClaudeLogin } from "@nextstepai/adapter-claude-local/server";
import {
...
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@nextstepai/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@nextstepai/adapter-gemini-local";
import { DEFAULT_OPENCODE_LOCAL_MODEL } from "@nextstepai/adapter-opencode-local";
...
export function agentRoutes(
   ...
   *   adapter probes the Nextstep host (legacy behavior).
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeNextstepSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readNextstepSkillSyncPreference(
