...
import { resolveNextstepInstanceId } from "../home-paths.js";
...
export function createBetterAuthInstance(db: Db, config: Config, trustedOrigins: string[]): BetterAuthInstance {
  const baseUrl = config.authBaseUrlMode === "explicit" ? config.authPublicBaseUrl : undefined;
  const secret = process.env.BETTER_AUTH_SECRET ?? process.env.NEXTSTEP_AGENT_JWT_SECRET;
  if (!secret) {
    throw new Error(
      "BETTER_AUTH_SECRET (or NEXTSTEP_AGENT_JWT_SECRET) must be set. " +
      "For local development, set BETTER_AUTH_SECRET=nextstep-dev-secret in your .env file.",
    );
  }
  const publicUrl = process.env.NEXTSTEP_PUBLIC_URL ?? baseUrl;
