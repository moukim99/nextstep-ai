...
Run this checklist on every heartbeat. This covers both your local planning/memory work and your organizational coordination via the Nextstep skill.

## 1. Identity and Context

- `GET /api/agents/me` -- confirm your id, role, budget, chainOfCommand.
- Check wake context: `NEXTSTEP_TASK_ID`, `NEXTSTEP_WAKE_REASON`, `NEXTSTEP_WAKE_COMMENT_ID`.
...
If `NEXTSTEP_APPROVAL_ID` is set:
...
- If `NEXTSTEP_TASK_ID` is set and assigned to you, prioritize that task.

## 5. Checkout and Work

- For scoped issue wakes, Nextstep may already checkout the current issue in the harness before your run starts.
...
- Use `Nextstep-create-agent` skill when hiring new agents.
...
- Always use the Nextstep skill for coordination.
- Always include `X-Nextstep-Run-Id` header on mutating API calls.
