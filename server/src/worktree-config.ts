...
import type { NextstepConfig } from "@nextstepai/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Nextstep worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.NEXTSTEP_IN_WORKTREE !== "true") return null;

  const configPath = resolveNextstepConfigPath(overrideConfigPath);
  ...
  const worktreeName =
    nonEmpty(persistedEnv.NEXTSTEP_WORKTREE_NAME) ??
    nonEmpty(env.NEXTSTEP_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.NEXTSTEP_INSTANCE_ID) ??
    nonEmpty(env.NEXTSTEP_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.NEXTSTEP_HOME) ??
      nonEmpty(env.NEXTSTEP_HOME) ??
      nonEmpty(env.NEXTSTEP_WORKTREES_DIR) ??
      "~/.Nextstep-worktrees",
  );
...
function writeConfigFile(configPath: string, config: NextstepConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".Nextstep", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as NextstepConfig;
...
): { config: NextstepConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.NEXTSTEP_HOME = context.homeDir;
  process.env.NEXTSTEP_INSTANCE_ID = context.instanceId;
  process.env.NEXTSTEP_CONFIG = context.configPath;
  process.env.NEXTSTEP_CONTEXT = context.contextPath;
  process.env.NEXTSTEP_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as NextstepConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    NEXTSTEP_HOME: context.homeDir,
    NEXTSTEP_INSTANCE_ID: context.instanceId,
    NEXTSTEP_CONFIG: context.configPath,
    NEXTSTEP_CONTEXT: context.contextPath,
    NEXTSTEP_IN_WORKTREE: "true",
    NEXTSTEP_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: NextstepConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as NextstepConfig;
  } catch {
    return;
  }
