...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.NEXTSTEP_SECRETS_PROVIDER === undefined) {
    process.env.NEXTSTEP_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.NEXTSTEP_SECRETS_STRICT_MODE === undefined) {
    process.env.NEXTSTEP_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.NEXTSTEP_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.NEXTSTEP_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.NEXTSTEP_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.NEXTSTEP_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set NEXTSTEP_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@Nextstep.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.NEXTSTEP_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "Nextstep");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "Nextstep");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: Nextstep");
    }
  
    const embeddedConnectionString = `postgres://Nextstep:Nextstep@127.0.0.1:${port}/Nextstep`;
  ...
  const configuredApiUrl = process.env.NEXTSTEP_API_URL?.trim() || runtimeApiUrl;
  ...
  process.env.NEXTSTEP_LISTEN_HOST = runtimeListenHost;
  process.env.NEXTSTEP_LISTEN_PORT = String(listenPort);
  process.env.NEXTSTEP_RUNTIME_API_URL = runtimeApiUrl;
  process.env.NEXTSTEP_RUNTIME_API_CANDIDATES_JSON = JSON.stringify(runtimeApiCandidates);
  process.env.NEXTSTEP_API_URL = configuredApiUrl;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.NEXTSTEP_OPEN_ON_LISTEN === "true") {
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Nextstep server failed to start");
    process.exit(1);
  });
}
