...
import type { Issue, NextstepPluginManifestV1 } from "@nextstepai/shared";
import { createTestHarness } from "../../../packages/plugins/sdk/src/testing.js";

function manifest(capabilities: NextstepPluginManifestV1["capabilities"]): NextstepPluginManifestV1 {
  return {
    id: "Nextstep.test-orchestration",
    apiVersion: 1,
    version: "0.1.0",
...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    expect(created.originKind).toBe("plugin:Nextstep.test-orchestration");
  ...
  it("enforces plugin origin namespaces in the test harness", async () => {
    ...
    const created = await harness.ctx.issues.create({
      companyId,
      title: "Generated issue",
      originKind: "plugin:Nextstep.test-orchestration:feature",
    });

    expect(created.originKind).toBe("plugin:Nextstep.test-orchestration:feature");
    await expect(
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:Nextstep.test-orchestration");
    await expect(
  ...
  it("supports generic plugin operation issue visibility in the test harness", async () => {
    ...
    expect(created.originKind).toBe("plugin:Nextstep.test-orchestration:operation");
