...
const ORIGINAL_NEXTSTEP_API_URL = process.env.NEXTSTEP_API_URL;
const ORIGINAL_NEXTSTEP_RUNTIME_API_URL = process.env.NEXTSTEP_RUNTIME_API_URL;
const ORIGINAL_NEXTSTEP_RUNTIME_API_CANDIDATES_JSON = process.env.NEXTSTEP_RUNTIME_API_CANDIDATES_JSON;
const ORIGINAL_NEXTSTEP_LISTEN_HOST = process.env.NEXTSTEP_LISTEN_HOST;
const ORIGINAL_NEXTSTEP_LISTEN_PORT = process.env.NEXTSTEP_LISTEN_PORT;
...
function buildTestConfig(overrides: Record<string, unknown> = {}) {
  return {
    ...
    databaseUrl: "postgres://Nextstep:Nextstep@127.0.0.1:5432/Nextstep",
    embeddedPostgresDataDir: "/tmp/Nextstep-test-db",
    embeddedPostgresPort: 54329,
    databaseBackupEnabled: false,
    databaseBackupIntervalMinutes: 60,
    databaseBackupRetentionDays: 30,
    databaseBackupDir: "/tmp/Nextstep-test-backups",
    serveUi: false,
    uiDevMiddleware: false,
    secretsProvider: "local_encrypted",
    secretsStrictMode: false,
    secretsMasterKeyFilePath: "/tmp/Nextstep-master.key",
    storageProvider: "local_disk",
    storageLocalDiskBaseDir: "/tmp/Nextstep-storage",
    storageS3Bucket: "Nextstep-test",
...
describe("startServer NEXTSTEP_API_URL handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    loadConfigMock.mockReturnValue(buildTestConfig());
    process.env.BETTER_AUTH_SECRET = "test-secret";
    delete process.env.NEXTSTEP_API_URL;
  });

  afterEach(() => {
    if (ORIGINAL_NEXTSTEP_API_URL === undefined) delete process.env.NEXTSTEP_API_URL;
    else process.env.NEXTSTEP_API_URL = ORIGINAL_NEXTSTEP_API_URL;

    if (ORIGINAL_NEXTSTEP_RUNTIME_API_URL === undefined) delete process.env.NEXTSTEP_RUNTIME_API_URL;
    else process.env.NEXTSTEP_RUNTIME_API_URL = ORIGINAL_NEXTSTEP_RUNTIME_API_URL;

    if (ORIGINAL_NEXTSTEP_RUNTIME_API_CANDIDATES_JSON === undefined) {
      delete process.env.NEXTSTEP_RUNTIME_API_CANDIDATES_JSON;
    } else {
      process.env.NEXTSTEP_RUNTIME_API_CANDIDATES_JSON = ORIGINAL_NEXTSTEP_RUNTIME_API_CANDIDATES_JSON;
    }

    if (ORIGINAL_NEXTSTEP_LISTEN_HOST === undefined) delete process.env.NEXTSTEP_LISTEN_HOST;
    else process.env.NEXTSTEP_LISTEN_HOST = ORIGINAL_NEXTSTEP_LISTEN_HOST;

    if (ORIGINAL_NEXTSTEP_LISTEN_PORT === undefined) delete process.env.NEXTSTEP_LISTEN_PORT;
    else process.env.NEXTSTEP_LISTEN_PORT = ORIGINAL_NEXTSTEP_LISTEN_PORT;
  });

  it("uses the externally set NEXTSTEP_API_URL when provided", async () => {
    process.env.NEXTSTEP_API_URL = "http://custom-api:3100";

    const started = await startServer();

    expect(started.apiUrl).toBe("http://custom-api:3100");
    expect(process.env.NEXTSTEP_API_URL).toBe("http://custom-api:3100");
    expect(JSON.parse(process.env.NEXTSTEP_RUNTIME_API_CANDIDATES_JSON ?? "[]")).toEqual(
      expect.arrayContaining(["http://custom-api:3100"]),
    );
    expect(JSON.parse(process.env.NEXTSTEP_RUNTIME_API_CANDIDATES_JSON ?? "[]")[0]).toBe("http://custom-api:3100");
  });

  it("falls back to host-based URL when NEXTSTEP_API_URL is not set", async () => {
    const started = await startServer();

    expect(started.apiUrl).toBe("http://127.0.0.1:3210");
    expect(process.env.NEXTSTEP_API_URL).toBe("http://127.0.0.1:3210");
  });
  ...
  it("rewrites explicit-port auth public URLs when detect-port selects a new port", async () => {
    ...
    expect(process.env.NEXTSTEP_RUNTIME_API_URL).toBe("http://my-host.ts.net:3110");
  });
  ...
  it("keeps no-port auth public URLs stable when detect-port selects a new port", async () => {
    loadConfigMock.mockReturnValueOnce(buildTestConfig({
      port: 3100,
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://Nextstep.example",
    }));
    detectPortMock.mockResolvedValueOnce(3110);
    ...
    expect(started.listenPort).toBe(3110);
    expect(started.apiUrl).toBe("https://Nextstep.example");
    expect(process.env.NEXTSTEP_RUNTIME_API_URL).toBe("https://Nextstep.example");
  });
});
