...
describe("heartbeat comment wake batching", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("Nextstep-heartbeat-comment-wake-");
  ...
  it("defers approval-approved wakes for a running issue so the assignee resumes after the run", async () => {
    ...
    expect((deferred?.payload as Record<string, unknown>)._NextstepWakeContext).toMatchObject({
      issueId,
      taskId: issueId,
      approvalId: "approval-1",
      approvalStatus: "approved",
  ...
  it("batches deferred comment wakes and forwards the ordered batch to the next run", async () => {
    ...
    try {
      ...
      const deferredContext = (deferredWake?.payload as Record<string, unknown> | null)?._NextstepWakeContext as
      ...
      expect(secondPayload.Nextstep).toMatchObject({
        wake: {
  ...
  it("promotes deferred comment wakes with their comments after the active run is cancelled", async () => {
    ...
    try {
      ...
      expect(promotedPayload.Nextstep).toMatchObject({
        wake: {
  ...
  it("queues exactly one follow-up run when an issue-bound run exits without a comment", async () => {
    ...
    try {
      ...
      expect(firstPayload.Nextstep).toMatchObject({
        wake: {
          reason: "issue_assigned",
          issue: {
            id: issueId,
      ...
      expect(String(firstPayload.message ?? "")).toContain("## Nextstep Wake Payload");
