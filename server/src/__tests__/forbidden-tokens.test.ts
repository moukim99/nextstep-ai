...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "Nextstep", LOGNAME: "Nextstep", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "Nextstep" }),
      },
    );

    expect(tokens).toEqual(["Nextstep", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "Nextstep" }, {
        userInfo: () => ({ username: "Nextstep" }),
      });

      expect(tokens).toEqual(["Nextstep", "custom-token"]);
    } finally {
