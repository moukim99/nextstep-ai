...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Nextstep Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@Nextstep.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `Nextstep-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "Nextstep-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "Nextstep-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "Nextstep-close-check",
      baseRef: "main",
      metadata: {
