...
import type { NextstepPluginManifestV1 } from "@nextstepai/shared";
import {
...
function manifest(): NextstepPluginManifestV1 {
  return {
    id: "Nextstep.managed-agents-test",
    apiVersion: 1,
    version: "0.1.0",
...
describeEmbeddedPostgres("plugin-managed agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-plugin-managed-agents-");
  ...
  async function seedCompanyAndPlugin(options: { requireApproval?: boolean; manifest?: NextstepPluginManifestV1 } = {}) {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@nextstepai/plugin-managed-agents-test",
  ...
  it("materializes declared managed agent instructions with local folder paths", async () => {
    const previousHome = process.env.NEXTSTEP_HOME;
    const previousInstance = process.env.NEXTSTEP_INSTANCE_ID;
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-managed-agent-home-"));
    const wikiRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-managed-agent-wiki-")));
    process.env.NEXTSTEP_HOME = tempHome;
    process.env.NEXTSTEP_INSTANCE_ID = "test";
    try {
      const pluginManifest = manifest();
      ...
      if (previousHome === undefined) delete process.env.NEXTSTEP_HOME;
      else process.env.NEXTSTEP_HOME = previousHome;
      if (previousInstance === undefined) delete process.env.NEXTSTEP_INSTANCE_ID;
      else process.env.NEXTSTEP_INSTANCE_ID = previousInstance;
  ...
  it("respects board approval policy for new managed agents", async () => {
    ...
    expect(approval?.payload).toMatchObject({
      agentId: created.agentId,
      sourcePluginKey: "Nextstep.managed-agents-test",
