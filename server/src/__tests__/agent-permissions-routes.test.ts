...
function registerModuleMocks() {
  vi.doMock("@nextstepai/adapter-opencode-local/server", async () => {
    const actual = await vi.importActual<typeof import("@nextstepai/adapter-opencode-local/server")>("@nextstepai/adapter-opencode-local/server");
    return {
      ...actual,
  ...
  vi.doMock("@nextstepai/shared/telemetry", () => ({
...
describe.sequential("agent permission routes", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.doUnmock("@nextstepai/shared/telemetry");
    ...
    vi.doUnmock("@nextstepai/adapter-opencode-local/server");
    registerModuleMocks();
    vi.resetAllMocks();
    ...
    const res = await requestApp(app, (baseUrl) => request(baseUrl)
      ...
      .send({
        adapterConfig: {
          workspaceStrategy: {
            type: "git_worktree",
            provisionCommand: "touch /tmp/Nextstep-rce",
          },
        },
      }));
