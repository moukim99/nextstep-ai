...
describe("feedback trace share client", () => {
  ...
  it("defaults to telemetry.Nextstep.ing when no backend url is configured", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://telemetry.Nextstep.ing/feedback-traces",
  ...
  it("wraps the feedback trace payload as gzip+base64 json before upload", async () => {
    const client = createFeedbackTraceShareClientFromConfig({
      feedbackExportBackendUrl: "https://telemetry.Nextstep.ing",
    ...
    expect(call?.[0]).toBe("https://telemetry.Nextstep.ing/feedback-traces");
