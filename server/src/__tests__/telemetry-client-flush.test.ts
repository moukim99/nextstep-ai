...
describe("TelemetryClient periodic flush", () => {
  ...
  it("falls back to the api gateway ingest url when the default hostname fails", async () => {
    vi.mocked(fetch)
      .mockRejectedValueOnce(new TypeError("getaddrinfo ENOTFOUND telemetry.Nextstep.ing"))
    ...
    expect(fetch).toHaveBeenCalledTimes(2);
    expect(vi.mocked(fetch).mock.calls[0]?.[0]).toBe("https://telemetry.Nextstep.ing/ingest");
