...
import { companies, companySkills, createDb } from "@nextstepai/db";
import {
...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-company-skills-service-");
  ...
  it("lists skills without exposing markdown content", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-heavy-skill-"));
