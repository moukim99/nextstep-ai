...
import { runChildProcess } from "@nextstepai/adapter-utils/server-utils";
import { execute } from "@nextstepai/adapter-codex-local/server";

async function writeFakeCodexCommand(commandPath: string): Promise<void> {
  const script = `#!/usr/bin/env node
const fs = require("node:fs");

const capturePath = process.env.NEXTSTEP_TEST_CAPTURE_PATH;
const payload = {
  argv: process.argv.slice(2),
  prompt: fs.readFileSync(0, "utf8"),
  codexHome: process.env.CODEX_HOME || null,
  NextstepWakePayloadJson: process.env.NEXTSTEP_WAKE_PAYLOAD_JSON || null,
  NextstepApiUrl: process.env.NEXTSTEP_API_URL || null,
  NextstepApiKey: process.env.NEXTSTEP_API_KEY || null,
  NextstepApiBridgeMode: process.env.NEXTSTEP_API_BRIDGE_MODE || null,
  NextstepEnvKeys: Object.keys(process.env)
...
type CapturePayload = {
  ...
  NextstepWakePayloadJson: string | null;
  NextstepApiUrl?: string | null;
  NextstepApiKey?: string | null;
  NextstepApiBridgeMode?: string | null;
  NextstepEnvKeys: string[];
};
...
describe("codex execute", () => {
  it("uses a Nextstep-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-default-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const NextstepHome = path.join(root, "Nextstep-home");
    ...
    const previousNextstepHome = process.env.NEXTSTEP_HOME;
    const previousNextstepInstanceId = process.env.NEXTSTEP_INSTANCE_ID;
    const previousNextstepInWorktree = process.env.NEXTSTEP_IN_WORKTREE;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.NEXTSTEP_HOME = NextstepHome;
    delete process.env.NEXTSTEP_INSTANCE_ID;
    delete process.env.NEXTSTEP_IN_WORKTREE;
    ...
    try {
      ...
      const result = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            NEXTSTEP_TEST_CAPTURE_PATH: capturePath,
          },
          promptTemplate: "Follow the Nextstep heartbeat.",
        },
        context: {},
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using Nextstep-managed Codex home"),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousNextstepHome === undefined) delete process.env.NEXTSTEP_HOME;
      else process.env.NEXTSTEP_HOME = previousNextstepHome;
      if (previousNextstepInstanceId === undefined) delete process.env.NEXTSTEP_INSTANCE_ID;
      else process.env.NEXTSTEP_INSTANCE_ID = previousNextstepInstanceId;
      if (previousNextstepInWorktree === undefined) delete process.env.NEXTSTEP_IN_WORKTREE;
      else process.env.NEXTSTEP_IN_WORKTREE = previousNextstepInWorktree;
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-notes-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-meta-"));
    const workspace = path.join(root, "workspace");
    ...
    try {
      ...
      expect(loggedEnv.NEXTSTEP_RESOLVED_COMMAND).toBe(commandPath);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("injects bridge env into sandbox-managed remote runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-sandbox-"));
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.codexHome).toBe(path.join(remoteWorkspace, ".Nextstep-runtime", "codex", "home"));
      expect(capture.NextstepApiUrl).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(capture.NextstepApiKey).not.toBe("run-jwt-token");
      expect(capture.NextstepApiBridgeMode).toBe("queue_v1");
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousPath === undefined) delete process.env.PATH;
  ...
  it("injects structured Nextstep wake payloads into env and prompt", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.NextstepEnvKeys).toContain("NEXTSTEP_WAKE_PAYLOAD_JSON");
      expect(capture.NextstepWakePayloadJson).not.toBeNull();
      expect(JSON.parse(capture.NextstepWakePayloadJson ?? "{}")).toMatchObject({
        reason: "issue_commented",
        latestCommentId: "comment-2",
        commentIds: ["comment-1", "comment-2"],
      });
      expect(capture.prompt).toContain("## Nextstep Wake Payload");
  ...
  it("classifies remote-compaction high-demand failures as retryable transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-transient-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("persists retry-not-before metadata for codex usage-limit failures", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-usage-limit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingCodexCommand(
  ...
  it("uses safer invocation settings and a fresh-session handoff for codex transient fallback retries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-fallback-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(capture.prompt).toContain("Nextstep session handoff:");
  ...
  it("renders execution-stage wake instructions for reviewer and executor roles", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-stage-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const executorResult = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            NEXTSTEP_TEST_CAPTURE_PATH: executorCapturePath,
          },
          promptTemplate: "Follow the Nextstep heartbeat.",
        },
        context: {
  ...
  it("renders an issue-scoped wake prompt even when the wake has no comments yet", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-issue-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    ...
    try {
      ...
      expect(capture.prompt).toContain("## Nextstep Resume Delta");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      expect(capture.prompt).not.toContain("Follow the Nextstep heartbeat.");
      expect(capture.prompt).not.toContain("You are managed instructions.");
      expect(invocationPrompt).toContain("## Nextstep Resume Delta");
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const homeSkill = path.join(isolatedCodexHome, "skills", "Nextstep");
    await fs.mkdir(workspace, { recursive: true });
    await fs.mkdir(sharedCodexHome, { recursive: true });
    await fs.writeFile(path.join(sharedCodexHome, "auth.json"), '{"token":"shared"}\n', "utf8");
    await fs.writeFile(path.join(sharedCodexHome, "config.toml"), 'model = "codex-mini-latest"\n', "utf8");
    ...
    process.env.HOME = root;
    process.env.NEXTSTEP_HOME = NextstepHome;
    process.env.NEXTSTEP_INSTANCE_ID = "worktree-1";
    process.env.NEXTSTEP_IN_WORKTREE = "true";
    process.env.CODEX_HOME = sharedCodexHome;
    ...
    try {
      ...
      expect(capture.argv).toEqual(expect.arrayContaining(["exec", "--json", "-"]));
      expect(capture.prompt).toContain("Follow the Nextstep heartbeat.");
      expect(capture.NextstepEnvKeys).toEqual(
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining('Injected Codex skill "Nextstep"'),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
  ...
  it("respects an explicit CODEX_HOME config override even in worktree mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-codex-execute-explicit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    try {
      ...
      expect((await fs.lstat(path.join(explicitCodexHome, "skills", "Nextstep"))).isSymbolicLink()).toBe(true);
      await expect(fs.lstat(path.join(NextstepHome, "instances", "worktree-1", "codex-home"))).rejects.toThrow();
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousNextstepHome === undefined) delete process.env.NEXTSTEP_HOME;
