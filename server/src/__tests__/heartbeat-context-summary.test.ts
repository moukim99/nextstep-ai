...
import {
  buildNextstepTaskMarkdown,
  mergeCoalescedContextSnapshot,
  summarizeHeartbeatRunContextSnapshot,
  summarizeHeartbeatRunListResultJson,
} from "../services/heartbeat.js";

describe("buildNextstepTaskMarkdown", () => {
  it("adds planning directives for assignment and comment task context", () => {
    const assignment = buildNextstepTaskMarkdown({
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
        title: "Plan first",
    ...
    const commentWake = buildNextstepTaskMarkdown({
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
        title: "Plan first",
    ...
    const acceptedConfirmation = buildNextstepTaskMarkdown({
      issue: {
        id: "issue-1",
        identifier: "PAP-3404",
        title: "Plan first",
...
describe("summarizeHeartbeatRunContextSnapshot", () => {
  ...
  it("returns null when no allowed fields are present", () => {
    expect(
      summarizeHeartbeatRunContextSnapshot({
        NextstepWake: { comments: [{ body: "hello" }] },
      }),
    ).toBeNull();
  });
});
