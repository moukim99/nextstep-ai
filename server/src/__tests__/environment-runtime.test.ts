...
describeEmbeddedPostgres("environmentRuntimeService", () => {
  ...
  it("acquires and releases an SSH run lease through the runtime seam", async () => {
    ...
    const fixtureRoot = await mkdtemp(path.join(os.tmpdir(), "Nextstep-environment-runtime-ssh-"));
  ...
  it("uses plugin-backed sandbox config for execute and release", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "Nextstep.fake-plugin-sandbox-provider",
      packageName: "@nextstepai/plugin-fake-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "Nextstep.fake-plugin-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("delegates plugin environment leases through the plugin worker manager", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "acme.environments",
      packageName: "@acme/Nextstep-environments",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
