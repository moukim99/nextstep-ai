...
describe.sequential("cli auth routes", () => {
  ...
  it.sequential("creates a CLI auth challenge with approval metadata", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        command: "nextstepai company import",
        clientName: "nextstepai cli",
  ...
  it.sequential("rejects anonymous access to generic skill documents", async () => {
    ...
    const skillRes = await request(skillApp).get("/api/skills/Nextstep");
  ...
  it.sequential("serves the invite-scoped Nextstep skill anonymously for active invites", async () => {
    const invite = {
      id: "invite-1",
      companyId: "company-1",
    ...
    const res = await request(app).get("/api/invites/token-123/skills/Nextstep");

    expect(res.status).toBe(200);
    expect(res.headers["content-type"]).toContain("text/markdown");
    expect(res.text).toContain("# Nextstep Skill");
  });
