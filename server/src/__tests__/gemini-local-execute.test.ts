...
import { execute } from "@nextstepai/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects Nextstep env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the Nextstep heartbeat.");
      expect(promptArg).toContain("Nextstep runtime note:");
      ...
      expect(invocationPrompt).toContain("Nextstep runtime note:");
      expect(invocationPrompt).toContain("NEXTSTEP_API_URL");
      expect(invocationPrompt).toContain("Nextstep API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes turn-limit exhaustion into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-gemini-max-turns-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes Gemini exit code 53 as max-turn exhaustion", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-gemini-exit-53-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("does not normalize unstructured turn-limit text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-gemini-max-turn-text-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-gemini-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(promptArg).toContain("## Nextstep Resume Delta");
      ...
      expect(promptArg).not.toContain("Follow the Nextstep heartbeat.");
    } finally {
      if (previousHome === undefined) {
        delete process.env.HOME;
      } else {
