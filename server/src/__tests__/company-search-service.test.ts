...
import { companySearchQuerySchema, COMPANY_SEARCH_MAX_QUERY_LENGTH } from "@nextstepai/shared";
import {
...
describeEmbeddedPostgres("companySearchService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-company-search-");
  ...
  async function createCompany(name = "Nextstep") {
