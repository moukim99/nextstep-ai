...
import { agents, companies, companySkills, createDb } from "@nextstepai/db";
import {
...
describeEmbeddedPostgres("companySkillService.detail", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-company-skills-detail-");
  ...
  it("reports attached agents without probing adapter runtime skill state", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-reflection-skill-"));
  ...
  it("uses explicit company skill column selections when resolving detail usage", async () => {
    ...
    await db.insert(companySkills).values([
      ...
      },
      {
        ...
        sourceType: "catalog",
        sourceLocator: "Nextstep://catalog/large-reference-skill",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
