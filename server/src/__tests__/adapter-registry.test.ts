...
describe("server adapter registry", () => {
  ...
  it("injects the local agent JWT and Nextstep API auth guidance into Hermes", async () => {
    ...
    expect(patchedCtx.agent.adapterConfig).toMatchObject({
      env: {
        OPENAI_API_KEY: "llm-token",
        NEXTSTEP_API_KEY: "agent-run-jwt",
        NEXTSTEP_RUN_ID: "run-123",
      },
    });
    expect(patchedCtx.agent.adapterConfig.promptTemplate).toContain(
      "Authorization: Bearer $NEXTSTEP_API_KEY",
    );
    expect(patchedCtx.agent.adapterConfig.promptTemplate).toContain(
      "X-Nextstep-Run-Id: $NEXTSTEP_RUN_ID",
    );
  ...
  it("preserves Hermes command normalization while injecting auth", async () => {
    ...
    expect(patchedCtx.agent.adapterConfig.env.NEXTSTEP_API_KEY).toBe("agent-run-jwt");
  });
  ...
  it("passes the original Hermes context through when authToken is absent", async () => {
    ...
    const ctx = {
      ...
      agent: {
        ...
        adapterConfig: {
          env: {
            NEXTSTEP_API_KEY: "server-level-key",
          },
          promptTemplate: "Existing prompt",
        },
      },
  ...
  it("preserves an explicit Hermes Nextstep API key and does not set promptTemplate when none was configured", async () => {
    const adapter = requireServerAdapter("hermes_local");
    ...
    await adapter.execute({
      ...
      agent: {
        ...
        adapterConfig: {
          env: {
            NEXTSTEP_API_KEY: "explicit-agent-key",
            NEXTSTEP_RUN_ID: "stale-run-id",
          },
        },
      },
      runtime: {},
    ...
    const [patchedCtx] = hermesExecuteMock.mock.calls[0];
    expect(patchedCtx.agent.adapterConfig.env.NEXTSTEP_API_KEY).toBe("explicit-agent-key");
    expect(patchedCtx.agent.adapterConfig.env.NEXTSTEP_RUN_ID).toBe("run-123");
