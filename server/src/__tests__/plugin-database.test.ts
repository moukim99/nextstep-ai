...
const multiMigrationPluginKey = "Nextstep.dbfixture";
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["Nextstep.dbtest", "Nextstep.escape", "Nextstep.refresh", multiMigrationPluginKey]) {
  ...
  async function createPluginPackage(manifest: NextstepPluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "Nextstep-plugin-package-"));
  ...
  async function createInstallablePluginPackage(
    pluginManifest: NextstepPluginManifestV1,
    migrationSql: string,
  ) {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        type: "module",
        NextstepPlugin: { manifest: "./manifest.js" },
      }),
      "utf8",
    );
    await writeFile(
  ...
  async function installPluginRecord(manifest: NextstepPluginManifestV1) {
  ...
  function manifest(pluginKey = "Nextstep.dbtest"): NextstepPluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("Nextstep.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
  ...
  it("refreshes persisted manifests from disk before activation", async () => {
    const staleManifest = manifest("Nextstep.refresh");
    const refreshedManifest: NextstepPluginManifestV1 = {
      ...staleManifest,
      database: {
    ...
    expect(workerManager.startWorker).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: {
          ...
          NEXTSTEP_DEPLOYMENT_EXPOSURE: "public",
        },
