...
import { buildProjectMentionHref, MAX_ISSUE_REQUEST_DEPTH } from "@nextstepai/shared";
...
describeEmbeddedPostgres("issueService.list participantAgentId", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-issues-service-");
  ...
  it("ranks comment matches ahead of description-only matches", async () => {
    ...
    await db.insert(issueComments).values({
      ...
      body: "Reference: https://github.com/nextstepai/nextstep/pull/3303",
    });
  ...
  it("hides plugin operation issues from default lists and inbox-style filters while preserving explicit retrieval", async () => {
    ...
    await db.insert(issues).values([
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:Nextstep.missions:operation",
    ...
    await expect(svc.list(companyId, { originKind: "plugin:Nextstep.missions:operation" }))
...
describeEmbeddedPostgres("issueService.create workspace inheritance", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-issues-create-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService blockers and dependency wake readiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-issues-blockers-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.findMentionedProjectIds", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-issues-mentioned-projects-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.clearExecutionRunIfTerminal", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-issues-execution-lock-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
