...
import { sessionCodec as claudeSessionCodec } from "@nextstepai/adapter-claude-local/server";
import { sessionCodec as codexSessionCodec, isCodexUnknownSessionError } from "@nextstepai/adapter-codex-local/server";
import {
...
import { sessionCodec as acpxSessionCodec } from "@nextstepai/adapter-acpx-local/server";
...
describe("adapter session codecs", () => {
  ...
  it("preserves acpx session params required for compatibility checks", () => {
    const parsed = acpxSessionCodec.deserialize({
      sessionKey: "Nextstep:company:agent:task:fingerprint",
