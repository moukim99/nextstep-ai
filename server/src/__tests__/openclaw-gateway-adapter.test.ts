...
import { execute, testEnvironment } from "@nextstepai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@nextstepai/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("Nextstep:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("NEXTSTEP_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("NEXTSTEP_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Nextstep Wake Payload");
      ...
      expect(payload?.Nextstep).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },
