...
import type { CompanyPortabilityFileEntry } from "@nextstepai/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "Nextstep",
        name: "Nextstep",
        description: "Nextstep coordination skill",
        markdown: "---\nname: Nextstep\ndescription: Nextstep coordination skill\n---\n\n# Nextstep\n",
        sourceType: "github",
        sourceLocator: "https://github.com/nextstepai/nextstep/tree/master/skills/Nextstep",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/nextstepai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "nextstepai",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/nextstepai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "nextstepai",
  ...
  it("exports referenced skills as stubs by default with sanitized Nextstep extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Nextstep"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${NextstepKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/nextstepai/nextstep/Nextstep/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/nextstepai/nextstep/Nextstep/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/nextstepai/nextstep/Nextstep/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".Nextstep.yaml"]);
    expect(extension).toContain('schema: "Nextstep/v1"');
    ...
    expect(extension).not.toContain("NextstepSkillSync");
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    expect(asTextFile(exported.files[".Nextstep.yaml"])).toContain("requireBoardApprovalForNewAgents: true");
  });

  it("exports default sidebar order into the Nextstep extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".Nextstep.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/nextstepai/nextstep/Nextstep/SKILL.md"])).toContain("# Nextstep");
    expect(asTextFile(exported.files["skills/nextstepai/nextstep/Nextstep/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/nextstepai/nextstep/Nextstep/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/nextstepai/nextstep/Nextstep/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/nextstepai/nextstep/Nextstep/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .Nextstep.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".Nextstep.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-Nextstep",
        companyId: "company-1",
        key: "nextstepai/nextstep/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/nextstepai/nextstep/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "NEXTSTEP_bundled",
          owner: "nextstepai",
          repo: "Nextstep",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/nextstepai/nextstep/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/nextstepai/nextstep/release-changelog/SKILL.md"])).toContain("nextstepai/nextstep/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/Nextstep",
            repoUrl: "https://github.com/nextstepai/nextstep.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .Nextstep-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/nextstepai/nextstep.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/Nextstep");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Nextstep",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/nextstepai/nextstep.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".Nextstep.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/nextstepai/nextstep.git"');
    expect(extension).toContain('projectWorkspaceKey: "Nextstep"');
  ...
  it("reads env inputs back from .Nextstep.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .Nextstep.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Nextstep routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "Nextstep-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Nextstep" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Nextstep"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .Nextstep.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Nextstep");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".Nextstep.yaml"] = `${exported.files[".Nextstep.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
  ...
  it("preserves issue comment presentation fields through export and import", async () => {
    ...
    issueSvc.listComments.mockResolvedValue([
      {
        ...
        authorAgentId: null,
        authorUserId: null,
        body: "Nextstep needs a disposition before this issue can continue.",
        presentation,
        metadata,
    ...
    expect(issueSvc.addComment).toHaveBeenCalledWith(
      "issue-imported",
      "Nextstep needs a disposition before this issue can continue.",
