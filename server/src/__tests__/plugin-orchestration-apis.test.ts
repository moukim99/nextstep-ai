...
describeEmbeddedPostgres("plugin orchestration APIs", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-plugin-orchestration-");
  ...
  async function makeLocalRoot() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-plugin-host-folder-"));
    tempRoots.push(root);
    return root;
  }
  ...
  it("creates plugin-origin issues with full orchestration fields and audit activity", async () => {
    ...
    const services = buildHostServices(db, "plugin-record-id", "Nextstep.missions", createEventBusStub());
    ...
    expect(stored?.originKind).toBe("plugin:Nextstep.missions");
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          details: expect.objectContaining({
            ...
            sourcePluginKey: "Nextstep.missions",
  ...
  it("enforces plugin origin namespaces", async () => {
    ...
    expect(featureIssue.originKind).toBe("plugin:Nextstep.missions:feature");
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:Nextstep.missions");
  ...
  it("creates plugin operation issues with the generic operation origin", async () => {
    ...
    expect(issue.originKind).toBe("plugin:Nextstep.missions:operation");
  ...
  it("lets bootstrap-style actions initialize required local folders from an empty root", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "nextstepai.plugin-llm-wiki",
      packageName: "@nextstepai/plugin-llm-wiki",
      version: "0.1.0",
      manifestJson: {
        id: "nextstepai.plugin-llm-wiki",
        apiVersion: 1,
        version: "0.1.0",
        ...
        localFolders: [
          {
            ...
            access: "readWrite",
            requiredDirectories: ["raw", "wiki", "wiki/concepts", ".Nextstep"],
  ...
  it("rejects worker local-folder access for undeclared manifest keys", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "Nextstep.local-folders",
      packageName: "@nextstep/plugin-local-folders",
      version: "0.1.0",
      manifestJson: {
        id: "Nextstep.local-folders",
        apiVersion: 1,
        version: "0.1.0",
  ...
  it("resolves plugin-managed projects by stable key without overwriting user edits", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "Nextstep.missions",
      packageName: "@nextstep/plugin-missions",
      version: "0.1.0",
      apiVersion: 1,
      categories: ["automation"],
      status: "ready",
      manifestJson: {
        id: "Nextstep.missions",
        apiVersion: 1,
        version: "0.1.0",
    ...
    const services = buildHostServices(db, pluginId, "Nextstep.missions", createEventBusStub());
