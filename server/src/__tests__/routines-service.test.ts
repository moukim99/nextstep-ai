...
describeEmbeddedPostgres("routine service live-execution coalescing", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-routines-service-");
  ...
  it("interpolates routine variables into the execution issue and stores resolved values", async () => {
    ...
    expect(storedIssue?.title).toBe("repo triage for Nextstep");
    expect(storedIssue?.description).toBe("Review Nextstep for high bugs");
