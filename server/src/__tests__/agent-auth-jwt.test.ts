...
describe("agent local JWT", () => {
  const secretEnv = "NEXTSTEP_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "NEXTSTEP_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "NEXTSTEP_AGENT_JWT_ISSUER";
  const audienceEnv = "NEXTSTEP_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when NEXTSTEP_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "Nextstep";
    process.env[audienceEnv] = "Nextstep-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
