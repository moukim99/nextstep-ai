...
vi.mock("@nextstepai/shared/telemetry", async () => {
  const actual = await vi.importActual<typeof import("@nextstepai/shared/telemetry")>(
    "@nextstepai/shared/telemetry",
  );
  return {
    ...actual,
...
describeEmbeddedPostgres("active-run output watchdog", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("Nextstep-active-run-output-watchdog-");
