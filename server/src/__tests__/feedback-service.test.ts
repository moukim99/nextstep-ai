...
import { writeNextstepSkillSyncPreference } from "@nextstepai/adapter-utils/server-utils";
import {
  agents,
  companies,
  companySkills,
...
describe("feedbackService.saveIssueVote", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("Nextstep-feedback-service-");
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-feedback-instructions-"));
    ...
    await db.insert(companySkills).values([
      {
        id: randomUUID(),
        companyId,
        key: "nextstepai/nextstep/Nextstep",
        slug: "Nextstep",
        name: "Nextstep",
        markdown: "# Nextstep",
        sourceType: "catalog",
    ...
    await db.insert(agents).values({
      ...
      status: "active",
      adapterType: "codex_local",
      adapterConfig: writeNextstepSkillSyncPreference(
        {
          model: "gpt-5.4",
        ...
        ["nextstepai/nextstep/Nextstep", "octo/research/public-skill"],
      ),
      runtimeConfig: {
        heartbeat: {
          enabled: true,
  ...
  it("builds a detailed sanitized shared bundle with issue and agent context", async () => {
    ...
    expect(trace?.schemaVersion).toBe("Nextstep-feedback-envelope-v2");
    expect(trace?.bundleVersion).toBe("Nextstep-feedback-bundle-v2");
  ...
  it("keeps earlier local votes local when a later vote enables sharing", async () => {
    ...
    expect(localTrace?.payloadVersion).toBe("Nextstep-feedback-v1");
    ...
    expect(sharedTrace?.payloadVersion).toBe("Nextstep-feedback-v1");
  });

  it("captures Claude project session artifacts as full traces", async () => {
    const claudeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-feedback-claude-"));
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    const opencodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "Nextstep-feedback-opencode-"));
    ...
    vi.stubEnv("NEXTSTEP_OPENCODE_STORAGE_DIR", opencodeRoot);
    const uploadTraceBundle = vi.fn().mockResolvedValue({ objectKey: "feedback-traces/test.json" });
    const flushingSvc = feedbackService(db, {
      shareClient: {
        uploadTraceBundle,
  ...
  it("flushes pending shared traces into configured object storage and marks them sent", async () => {
    ...
    expect(uploadTraceBundle.mock.calls[0]?.[0]).toMatchObject({
      ...
      envelope: {
        destination: "NEXTSTEP_labs_feedback_v1",
        exportId: traces[0]?.exportId,
      },
    });
  });
