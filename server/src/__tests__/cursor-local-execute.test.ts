...
import { execute } from "@nextstepai/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects Nextstep env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the Nextstep heartbeat.");
      ...
      expect(capture.prompt).toContain("Nextstep runtime note:");
      expect(capture.prompt).toContain("NEXTSTEP_API_KEY");
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const NextstepDir = await createSkillDir(runtimeSkillsRoot, "Nextstep");
  ...
  it("prefers ~/.local/bin/cursor-agent for remote sandbox execution when using the default command", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-cursor-sandbox-execute-"));
  ...
  it("keeps explicit command overrides for remote sandbox execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-cursor-sandbox-explicit-"));
    const homeDir = path.join(root, "home");
    const workspace = path.join(root, "workspace");
    const remoteWorkspace = path.join(root, "remote-workspace");
    const capturePath = path.join(root, "capture.json");
