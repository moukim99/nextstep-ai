...
describe("agent instructions service", () => {
  const originalNextstepHome = process.env.NEXTSTEP_HOME;
  const originalNextstepInstanceId = process.env.NEXTSTEP_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalNextstepHome === undefined) delete process.env.NEXTSTEP_HOME;
    else process.env.NEXTSTEP_HOME = originalNextstepHome;
    if (originalNextstepInstanceId === undefined) delete process.env.NEXTSTEP_INSTANCE_ID;
    else process.env.NEXTSTEP_INSTANCE_ID = originalNextstepInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const NextstepHome = await makeTempDir("Nextstep-agent-instructions-home-");
    const externalRoot = await makeTempDir("Nextstep-agent-instructions-external-");
    cleanupDirs.add(NextstepHome);
    cleanupDirs.add(externalRoot);
    process.env.NEXTSTEP_HOME = NextstepHome;
    process.env.NEXTSTEP_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("Nextstep-agent-instructions-new-external-");
    cleanupDirs.add(NextstepHome);
    cleanupDirs.add(externalRoot);
    process.env.NEXTSTEP_HOME = NextstepHome;
    process.env.NEXTSTEP_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("Nextstep-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const NextstepHome = await makeTempDir("Nextstep-agent-instructions-recover-");
    cleanupDirs.add(NextstepHome);
    process.env.NEXTSTEP_HOME = NextstepHome;
    process.env.NEXTSTEP_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const NextstepHome = await makeTempDir("Nextstep-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("Nextstep-agent-instructions-stale-root-");
    cleanupDirs.add(NextstepHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const NextstepHome = await makeTempDir("Nextstep-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("Nextstep-agent-instructions-heal-write-stale-");
    cleanupDirs.add(NextstepHome);
    cleanupDirs.add(staleRoot);
    process.env.NEXTSTEP_HOME = NextstepHome;
    process.env.NEXTSTEP_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const NextstepHome = await makeTempDir("Nextstep-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("Nextstep-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(NextstepHome);
    cleanupDirs.add(staleRoot);
    process.env.NEXTSTEP_HOME = NextstepHome;
    process.env.NEXTSTEP_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const NextstepHome = await makeTempDir("Nextstep-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("Nextstep-agent-instructions-partial-root-");
    cleanupDirs.add(NextstepHome);
    cleanupDirs.add(staleRoot);
    process.env.NEXTSTEP_HOME = NextstepHome;
    process.env.NEXTSTEP_INSTANCE_ID = "test-instance";
