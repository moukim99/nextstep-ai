...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const NextstepDir = path.join(worktreeRoot, ".Nextstep");
    const configPath = path.join(NextstepDir, "config.json");
    const envPath = path.join(NextstepDir, ".env");
    const sharedRoot = path.join(tempRoot, ".Nextstep", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".Nextstep-worktrees");

    await fs.mkdir(NextstepDir, { recursive: true });
    ...
    await fs.writeFile(
      ...
      [
        ...
        "NEXTSTEP_WORKTREE_NAME=PAP-884-ai-commits-component",
        "NEXTSTEP_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.NEXTSTEP_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.NEXTSTEP_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`NEXTSTEP_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('NEXTSTEP_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`NEXTSTEP_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`NEXTSTEP_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('NEXTSTEP_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.NEXTSTEP_HOME).toBe(isolatedHome);
    expect(process.env.NEXTSTEP_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Nextstep environment variables",
        "NEXTSTEP_IN_WORKTREE=true",
        "NEXTSTEP_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.NEXTSTEP_IN_WORKTREE = "true";
    process.env.NEXTSTEP_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.NEXTSTEP_WORKTREES_DIR = isolatedHome;
    delete process.env.NEXTSTEP_HOME;
    delete process.env.NEXTSTEP_INSTANCE_ID;
    delete process.env.NEXTSTEP_CONFIG;
  ...
  it("does not persist transient runtime home overrides over repo-local worktree env", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-worktree-runtime-override-"));
    const isolatedHome = path.join(tempRoot, ".Nextstep-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        "# Nextstep environment variables",
        `NEXTSTEP_HOME=${JSON.stringify(isolatedHome)}`,
        `NEXTSTEP_INSTANCE_ID=${JSON.stringify(instanceId)}`,
        `NEXTSTEP_CONFIG=${JSON.stringify(configPath)}`,
        `NEXTSTEP_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`,
        'NEXTSTEP_IN_WORKTREE="true"',
        'NEXTSTEP_WORKTREE_NAME="PAP-989-multi-user-implementation-using-plan-from-pap-958"',
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.NEXTSTEP_IN_WORKTREE = "true";
    process.env.NEXTSTEP_WORKTREE_NAME = "PAP-989-multi-user-implementation-using-plan-from-pap-958";
    process.env.NEXTSTEP_HOME = transientHome;
    ...
    expect(repairedEnv).toContain(`NEXTSTEP_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).not.toContain(`NEXTSTEP_HOME=${JSON.stringify(transientHome)}`);
    expect(process.env.NEXTSTEP_HOME).toBe(isolatedHome);
  });

  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".Nextstep-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".Nextstep", "worktrees");
    ...
    const NextstepDir = path.join(currentWorktreeRoot, ".Nextstep");
    const configPath = path.join(NextstepDir, "config.json");
    const envPath = path.join(NextstepDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".Nextstep", "config.json");
  ...
  it("persists runtime-selected worktree ports back into explicit-port auth URLs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.NEXTSTEP_IN_WORKTREE = "true";
    process.env.NEXTSTEP_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.NEXTSTEP_HOME = isolatedHome;
    process.env.NEXTSTEP_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.NEXTSTEP_CONFIG = configPath;
  ...
  it("does not rewrite no-port public auth URLs when persisting runtime-selected ports", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Nextstep-worktree-public-ports-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify(
        {
          ...buildLegacyConfig(instanceRoot, "https://Nextstep.example"),
          database: {
            mode: "embedded-postgres",
            embeddedPostgresDataDir: path.join(instanceRoot, "db"),
            embeddedPostgresPort: 54331,
    ...
    process.chdir(worktreeRoot);
    process.env.NEXTSTEP_IN_WORKTREE = "true";
    process.env.NEXTSTEP_WORKTREE_NAME = "PAP-125-public-base-url";
    process.env.NEXTSTEP_HOME = isolatedHome;
    process.env.NEXTSTEP_INSTANCE_ID = "pap-125-public-base-url";
    process.env.NEXTSTEP_CONFIG = configPath;
    ...
    expect(writtenConfig.server.port).toBe(3103);
    expect(writtenConfig.database.embeddedPostgresPort).toBe(54335);
    expect(writtenConfig.auth.publicBaseUrl).toBe("https://Nextstep.example");
  });
  ...
  it("does not rewrite the in-memory config when auth URL has no explicit port", () => {
    const { config, changed } = applyRuntimePortSelectionToConfig(
      buildLegacyConfig("/tmp/shared", "https://Nextstep.example"),
      {
        serverPort: 3104,
        databasePort: 54340,
        allowServerPortWrite: false,
    ...
    expect(config.server.port).toBe(3100);
    expect(config.database.embeddedPostgresPort).toBe(54340);
    expect(config.auth.publicBaseUrl).toBe("https://Nextstep.example");
  });
});
