...
import { resolveNextstepConfigPath, resolveNextstepEnvPath } from "./paths.js";
import type { BindMode, DeploymentExposure, DeploymentMode } from "@nextstepai/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.NEXTSTEP_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.NEXTSTEP_AGENT_JWT_SECRET === "string" ? parsed.NEXTSTEP_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm nextstepai onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveNextstepEnvPath();
